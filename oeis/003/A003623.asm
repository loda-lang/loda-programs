; A003623: Wythoff AB-numbers: floor(floor(n*phi^2)*phi), where phi = (1+sqrt(5))/2.
; Submitted by USTL-FIL (Lille Fr)
; 3,8,11,16,21,24,29,32,37,42,45,50,55,58,63,66,71,76,79,84,87,92,97,100,105,110,113,118,121,126,131,134,139,144,147,152,155,160,165,168,173,176,181,186,189,194,199,202,207,210,215,220,223,228,231,236,241,244,249,254,257,262,265,270,275,278,283,288,291,296,299,304,309,312,317,320,325,330,333,338,343,346,351,354,359,364,367,372,377,380,385,388,393,398,401,406,409,414,419,422
; Formula: a(n) = a(n-1)+gcd((-c(n-2)+b(n-2)-2)/2+2*c(n-2)+2,4)+1, a(3) = 16, a(2) = 11, a(1) = 8, a(0) = 3, b(n) = (2*((-c(n-1)+b(n-1)-2)/2))/gcd((-c(n-1)+b(n-1)-2)/2+2*c(n-1)+2,4), b(3) = -9, b(2) = -5, b(1) = -4, b(0) = -1, c(n) = 2*c(n-1)+2, c(3) = 62, c(2) = 30, c(1) = 14, c(0) = 6

mov $2,2
mov $3,1
mov $4,1
add $0,1
lpb $0
  sub $0,1
  add $2,1
  add $4,$3
  add $4,1
  sub $1,$2
  sub $1,1
  div $1,2
  mul $2,2
  mov $3,$2
  add $3,$1
  gcd $3,4
  mul $1,2
  div $1,$3
lpe
mov $0,$4
