; A140343: a(n)=4a(n-1)-7a(n-2)+6a(n-3)-3a(n-4), n>4.
; Submitted by Christian Krause
; 0,0,0,0,1,4,9,14,14,0,-41,-122,-243,-364,-364,0,1093,3280,6561,9842,9842,0,-29525,-88574,-177147,-265720,-265720,0,797161,2391484,4782969,7174454,7174454,0,-21523361,-64570082,-129140163,-193710244,-193710244,0,581130733
; Formula: a(n) = truncate(b(max(n-3,0))/5), b(n) = 2*b(n-1)+2*d(n-1)-3*d(n-2)+c(n-1)+10, b(4) = 70, b(3) = 45, b(2) = 20, b(1) = 5, b(0) = 0, c(n) = c(n-1)+d(n-1), c(3) = 10, c(2) = 10, c(1) = 5, c(0) = 0, d(n) = -b(n-1)+d(n-1), d(3) = -20, d(2) = 0, d(1) = 5, d(0) = 5

mov $3,5
sub $0,3
lpb $0
  sub $0,1
  add $2,$3
  mul $4,3
  sub $4,$3
  sub $3,$1
  add $4,$3
  add $4,$2
  add $1,$3
  add $1,$4
  sub $1,5
lpe
mov $0,$1
div $0,5
