; A139800: a(n)=a(n-1)+a(n-2)+a(n-3)+2a(n-4).
; Submitted by Simon Strandgaard
; 1,0,0,0,2,2,4,8,18,34,68,136,274,546,1092,2184,4370,8738,17476,34952,69906,139810,279620,559240,1118482,2236962,4473924,8947848,17895698,35791394,71582788,143165576,286331154,572662306,1145324612,2290649224
; Formula: a(n) = 2*b(n-2)+2*c(n-2)-2*b(n-2), a(4) = 2, a(3) = 0, a(2) = 0, a(1) = 0, a(0) = 1, b(n) = 4*b(n-3)+4*c(n-3)+2*b(n-2)-b(n-2)-2*b(n-3), b(4) = 2, b(3) = 1, b(2) = 0, b(1) = 1, b(0) = 0, c(n) = b(n-1)+c(n-1), c(4) = 2, c(3) = 1, c(2) = 1, c(1) = 0, c(0) = 0

mov $1,1
lpb $0
  sub $0,1
  add $1,$5
  sub $3,$4
  mov $4,$2
  mov $2,$3
  add $2,$1
  mul $3,2
  add $5,$4
  mov $1,$3
  mov $3,$5
lpe
mov $0,$1
