; A139800: a(n)=a(n-1)+a(n-2)+a(n-3)+2a(n-4).
; Submitted by Mads Nissen
; 1,0,0,0,2,2,4,8,18,34,68,136,274,546,1092,2184,4370,8738,17476,34952,69906,139810,279620,559240,1118482,2236962,4473924,8947848,17895698,35791394,71582788,143165576,286331154,572662306,1145324612,2290649224
; Formula: a(n) = -a(n-2)+c(n-2), a(4) = 2, a(3) = 0, a(2) = 0, a(1) = 0, a(0) = 1, b(n) = 2*b(n-2)+2*d(n-2), b(4) = 4, b(3) = 4, b(2) = 0, b(1) = 2, b(0) = 0, c(n) = b(n-1)+d(n-1), c(4) = 6, c(3) = 2, c(2) = 2, c(1) = 0, c(0) = 1, d(n) = b(n-1)+d(n-1), d(4) = 6, d(3) = 2, d(2) = 2, d(1) = 0, d(0) = 0

mov $3,1
mov $4,1
lpb $0
  sub $0,1
  add $5,$1
  mov $1,$3
  mul $1,2
  sub $3,$4
  mov $4,$2
  mov $2,$3
  mov $3,$5
lpe
mov $0,$4
