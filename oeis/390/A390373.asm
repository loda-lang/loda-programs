; A390373: Expansion of e.g.f. (1/2) * (exp(1/(1 - x)^2 - 1) + 1).
; Submitted by Science United
; 1,1,5,34,290,2956,34892,466600,6956168,114195280,2044297232,39593226784,824198178080,18339085306816,434119727399360,10888176248977024,288314558327931008,8034883301194942720,235007894463566519552,7196007297036317893120,230161142318670903607808,7673996503266988619262976
; Formula: a(n) = b(n-1)*(n-1)+a(n-1)*(n-1)+c(n-1), a(3) = 34, a(2) = 5, a(1) = 1, a(0) = 1, b(n) = b(n-1)*(n-1)+c(n-1), b(3) = 24, b(2) = 4, b(1) = 1, b(0) = 0, c(n) = 2*b(n-1)*(n-1)+2*a(n-1)*(n-1)+n*c(n-1)+2*c(n-1), c(3) = 116, c(2) = 16, c(1) = 3, c(0) = 1

mov $3,1
mov $4,1
lpb $0
  sub $0,1
  mul $2,$1
  add $2,$4
  mul $3,$1
  add $3,$2
  add $1,1
  mul $4,$1
  add $4,$3
  add $4,$3
lpe
mov $0,$3
