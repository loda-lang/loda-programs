...
; Formula: a(n) = floor(floor(b(4*n+4)/2)/floor((c(4*n+4)+d(4*n+4))/(10^(n+1))))%10, b(n) = floor((4*c(n-1)+2*b(n-1)+2*d(n-1))/3), b(2) = 5, b(1) = 1, b(0) = 0, c(n) = 3*c(n-1)+b(n-1)+d(n-1), c(2) = 11, c(1) = 3, c(0) = 1, d(n) = c(n-1)+d(n-1), d(2) = 4, d(1) = 1, d(0) = 0
