; A323845: Number of inequivalent height 1 degree n polynomials with nonzero constant term.
; Submitted by Dark Angel
; 1,4,6,21,45,144,378,1161,3321,10044,29646,89181,266085,798984,2392578,7179921,21526641,64586484,193720086,581179941,1743421725,5230324224,15690618378,47072032281,141215033961,423645633324,1270933711326,3812802728301,11438398618965,34315200639864
; Formula: a(n) = d(n+1)/6+1, b(n) = 3*b(n-1)+2, b(3) = 53, b(2) = 17, b(1) = 5, b(0) = 1, c(n) = 3*b(n-3)+3*c(n-2)+3*c(n-3)-c(n-1)+15, c(3) = 17, c(2) = 1, c(1) = -1, c(0) = 1, d(n) = 3*b(n-2)+3*c(n-2)+3*d(n-2)-3*c(n-2)+15, d(3) = 30, d(2) = 18, d(1) = 0, d(0) = 0

mov $1,1
mov $2,1
mov $4,-2
add $0,1
lpb $0
  sub $0,1
  sub $3,$2
  mul $2,3
  add $4,$2
  mov $5,$4
  sub $5,1
  mul $1,3
  add $1,2
  mov $4,$2
  add $4,$1
  add $4,14
  mov $2,$3
  mov $3,$5
lpe
mov $0,$3
div $0,6
add $0,1
