...
; Formula: a(n) = b(max(n-1,0)), b(n) = b(n-1)^2+b(n-1)+c(n-1), b(3) = 28, b(2) = 5, b(1) = 2, b(0) = 1, c(n) = (b(n-3)^2+b(n-3)+c(n-3))*((b(n-3)^2+b(n-3)+c(n-3))^2+b(n-3)^2+b(n-3)+c(n-2)+c(n-3))-((b(n-3)^2+b(n-3)+c(n-3))^2+b(n-3)^2+b(n-3)+c(n-2)+c(n-3))^2-(b(n-3)^2+b(n-3)+c(n-3))^2-b(n-3)^2-b(n-3)-c(n-1)-c(n-2)-c(n-3)+b(n-3), c(3) = -17, c(2) = -2, c(1) = -1, c(0) = 0
