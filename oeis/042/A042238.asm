; A042238: Numerators of continued fraction convergents to sqrt(645).
; Submitted by kpmonaghan
; 25,51,76,127,1600,16127,195124,211251,406375,1024001,51606425,104236851,155843276,260080127,3276804800,33028128127,399614342324,432642470451,832256812775,2097156096001,105690061612825,213477279321651,319167340934476,532644620256127,6710902784008000,67641672460336127,818410972308041524,886052644768377651,1704463617076419175,4294979878921216001,216453457563137219225,437201895005195654451,653655352568332873676,1090857247573528528127,13743942323450675211200,138530280482080280640127
; Formula: a(n) = b(n+1), b(n) = b(n-1)*(((sign(n-1)*((abs(n-1)-1)%10+1))==8)+((sign(n-1)*((abs(n-1)-1)%10+1))==7)+((sign(n-1)*((abs(n-1)-1)%10+1))==3)+((sign(n-1)*((abs(n-1)-1)%10+1))==2)+50*((sign(n-1)*((abs(n-1)-1)%10+1))==10)+25*((sign(n-1)*((abs(n-1)-1)%10+1))==0)+12*((sign(n-1)*((abs(n-1)-1)%10+1))==6)+12*((sign(n-1)*((abs(n-1)-1)%10+1))==4)+10*((sign(n-1)*((abs(n-1)-1)%10+1))==5)+2*((sign(n-1)*((abs(n-1)-1)%10+1))==9)+2*((sign(n-1)*((abs(n-1)-1)%10+1))==1))+b(n-2), b(2) = 51, b(1) = 25, b(0) = 1

mov $1,1
add $0,1
lpb $0
  sub $0,1
  mov $4,$2
  mov $2,$1
  mov $1,$3
  dgr $1,11
  mov $6,$1
  equ $6,0
  mul $6,25
  mov $5,$6
  mov $6,$1
  equ $6,1
  mul $6,2
  add $5,$6
  mov $6,$1
  equ $6,2
  add $5,$6
  mov $6,$1
  equ $6,3
  add $5,$6
  mov $6,$1
  equ $6,4
  mul $6,12
  add $5,$6
  mov $6,$1
  equ $6,5
  mul $6,10
  add $5,$6
  mov $6,$1
  equ $6,6
  mul $6,12
  add $5,$6
  mov $6,$1
  equ $6,7
  add $5,$6
  mov $6,$1
  equ $6,8
  add $5,$6
  mov $6,$1
  equ $6,9
  mul $6,2
  add $5,$6
  mov $6,$1
  equ $6,10
  mul $6,50
  add $5,$6
  mov $1,$5
  mul $1,$2
  add $1,$4
  add $3,1
lpe
mov $0,$1
