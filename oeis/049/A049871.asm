; A049871: a(n)=Sum{a(k): k=0,1,2,...,n-4,n-2,n-1}; a(n-3) is not a summand; 3 initial terms required.
; Submitted by Christian Krause
; 1,3,3,6,10,20,37,70,130,243,453,846,1579,2948,5503,10273,19177,35799,66828,124752,232882,434735,811546,1514962,2828071,5279331,9855246,18397383,34343506,64111097,119680057,223413991,417060391
; Formula: a(n) = c(n-1)+a(n-1), a(4) = 10, a(3) = 6, a(2) = 3, a(1) = 3, a(0) = 1, b(n) = -c(n-2)+b(n-1)+d(n-1), b(4) = 7, b(3) = 4, b(2) = 1, b(1) = 0, b(0) = 0, c(n) = b(n-1)+d(n-1), c(4) = 10, c(3) = 4, c(2) = 3, c(1) = 0, c(0) = 2, d(n) = c(n-1)+a(n-1), d(4) = 10, d(3) = 6, d(2) = 3, d(1) = 3, d(0) = 0

mov $2,2
mov $5,1
lpb $0
  sub $0,1
  sub $3,$4
  add $1,$3
  mov $3,$4
  mov $4,$2
  add $5,$2
  mov $2,$3
  add $2,$1
  mov $3,$5
lpe
mov $0,$5
