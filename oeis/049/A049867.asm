; A049867: a(n) = Sum{a(k): k=0,1,2,...,n-4,n-2,n-1}; a(n-3) is not a summand; 3 initial terms required.
; Submitted by stoneageman
; 1,1,4,5,10,17,33,61,115,214,400,746,1393,2600,4854,9061,16915,31576,58945,110036,205411,383453,715815,1336255,2494468,4656574,8692708,16227203,30292300,56548466,105562437,197059777,367863388
; Formula: a(n) = c(n-1), a(4) = 10, a(3) = 5, a(2) = 4, a(1) = 1, a(0) = 1, b(n) = c(n-2), b(4) = 5, b(3) = 4, b(2) = 1, b(1) = 1, b(0) = 0, c(n) = e(n-1)+3, c(4) = 17, c(3) = 10, c(2) = 5, c(1) = 4, c(0) = 1, d(n) = b(n-1)+d(n-1), d(4) = 6, d(3) = 2, d(2) = 1, d(1) = 0, d(0) = 0, e(n) = b(n-1)+c(n-1)+d(n-1)+e(n-1), e(4) = 30, e(3) = 14, e(2) = 7, e(1) = 2, e(0) = 1

mov $1,1
mov $3,1
mov $5,1
lpb $0
  sub $0,1
  add $4,$2
  mov $2,$1
  mov $1,$3
  mov $3,$5
  add $3,3
  add $5,$1
  add $5,$4
lpe
mov $0,$1
