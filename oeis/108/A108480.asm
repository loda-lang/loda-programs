; A108480: Expansion of (1-x-2x^2)/(1-2x-3x^2-4x^3+4x^4).
; Submitted by Science United
; 1,1,3,13,35,117,379,1197,3859,12357,39563,126845,406371,1302101,4172443,13369293,42838835,137266917,439837739,1409354397,4515934339,14470215157,46366299963,148569565165,476055153491,1525403341701
; Formula: a(n) = c(2*n+2), b(n) = 2*e(n-3), b(4) = 2, b(3) = 2, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = e(n-2), c(4) = 1, c(3) = 1, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = b(n-1)+d(n-1), d(4) = 2, d(3) = 0, d(2) = 0, d(1) = 0, d(0) = 0, e(n) = b(n-1)+d(n-1)+e(n-1), e(4) = 3, e(3) = 1, e(2) = 1, e(1) = 1, e(0) = 1

mov $5,1
mul $0,2
add $0,2
lpb $0
  sub $0,1
  add $4,$1
  mov $1,$3
  mul $1,2
  mov $3,$2
  mov $2,$5
  add $5,$4
lpe
mov $0,$3
