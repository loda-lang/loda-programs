; A108177: Integers of the form 2^(4n-1) or 2^(4n), n>0 and their immediate neighbors, together with -1, 0 and 1.
; Submitted by USTL-FIL (Lille Fr)
; -1,0,1,7,8,9,15,16,17,127,128,129,255,256,257,2047,2048,2049,4095,4096,4097,32767,32768,32769,65535,65536,65537,524287,524288,524289,1048575,1048576,1048577,8388607,8388608,8388609,16777215,16777216,16777217,134217727,134217728,134217729,268435455
; Formula: a(n) = b(n)+min(n,n%3)-1, b(n) = 4*d(n-3)+4*2, b(11) = 128, b(10) = 128, b(9) = 128, b(8) = 16, b(7) = 16, b(6) = 16, b(5) = 8, b(4) = 8, b(3) = 8, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = 3*b(n-3)-3*c(n-3)+c(n-3), c(11) = 0, c(10) = 0, c(9) = 0, c(8) = 24, c(7) = 24, c(6) = 24, c(5) = 0, c(4) = 0, c(3) = 0, c(2) = 0, c(1) = 0, c(0) = 0, d(n) = 3*b(n-3)+2*d(n-3)-3*c(n-3)+c(n-3)+2, d(11) = 62, d(10) = 62, d(9) = 62, d(8) = 30, d(7) = 30, d(6) = 30, d(5) = 2, d(4) = 2, d(3) = 2, d(2) = 0, d(1) = 0, d(0) = 0

lpb $0
  sub $0,3
  sub $1,$2
  mul $1,3
  add $2,$1
  add $3,1
  mov $1,$3
  mul $3,2
  add $3,$2
  add $1,1
  mul $1,4
lpe
sub $1,2
add $0,$1
add $0,1
