...
; Formula: a(n) = d(n+1)-1, b(n) = truncate((-c(n-1)+b(n-1)+1)/2), b(2) = -5, b(1) = -2, b(0) = 0, c(n) = gcd(d(n-1)+truncate((-c(n-1)+b(n-1)+1)/2),4)*c(n-1), c(2) = 40, c(1) = 10, c(0) = 5, d(n) = truncate(gcd(d(n-1)+truncate((-c(n-1)+b(n-1)+1)/2),4)/2), d(2) = 2, d(1) = 1, d(0) = 0
