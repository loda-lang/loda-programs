; A109545: a(n) = 2*a(n-1) + a(n-2) + a(n-3).
; Submitted by pelpolaris
; 1,1,2,6,15,38,97,247,629,1602,4080,10391,26464,67399,171653,437169,1113390,2835602,7221763,18392518,46842401,119299083,303833085,773807654,1970747476,5019135691,12782826512,32555536191,82913034585
; Formula: a(n) = 2*c(n-1)+b(n-1)+a(n-1), a(2) = 2, a(1) = 1, a(0) = 1, b(n) = 2*b(n-1)+b(n-2)+b(n-3), b(3) = 5, b(2) = 2, b(1) = 1, b(0) = 0, c(n) = 2*c(n-1)+c(n-2)+c(n-3), c(5) = 13, c(4) = 5, c(3) = 2, c(2) = 1, c(1) = 0, c(0) = 0

mov $4,1
lpb $0
  sub $0,1
  mov $2,$3
  add $4,$3
  add $4,$1
  mov $3,$1
  mov $1,$4
  add $4,$2
lpe
mov $0,$4
