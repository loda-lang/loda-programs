...
; Formula: a(n) = (n+1)*(-1)^(if((3^2)==1,3^(-binomial(floor(sqrtint(8*n)/2)+1,2)+n-1),if((-binomial(floor(sqrtint(8*n)/2)+1,2)+n-1)<=(-1),0,3^(-binomial(floor(sqrtint(8*n)/2)+1,2)+n-1)))+floor((3^(floor(sqrtint(8*n)/2)+1))/2))+a(n-1), a(0) = 1
