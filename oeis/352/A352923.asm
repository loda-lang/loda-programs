...
; Formula: a(n) = (2*d(n+2)-4)/2+1, b(n) = (-c(n-1)+b(n-1))/2-1, b(3) = -24, b(2) = -10, b(1) = -2, b(0) = 0, c(n) = gcd((-c(n-1)+b(n-1))/2+c(n-1)-1,4)*(c(n-1)+2), c(3) = 152, c(2) = 36, c(1) = 16, c(0) = 2, d(n) = gcd((-c(n-2)+b(n-2))/2+c(n-2)-1,4)/2+d(n-1), d(3) = 3, d(2) = 2, d(1) = 0, d(0) = 0
