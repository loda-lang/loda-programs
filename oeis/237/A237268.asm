; A237268: a(1)=1; for n > 1, a(n) is the smallest F(m) > F(n) such that F(n) divides F(m), where F(k) denotes the k-th Fibonacci number.
; Submitted by Ralfy
; 1,2,8,21,55,144,377,987,2584,6765,17711,46368,121393,317811,832040,2178309,5702887,14930352,39088169,102334155,267914296,701408733,1836311903,4807526976,12586269025,32951280099,86267571272,225851433717,591286729879,1548008755920,4052739537881,10610209857723,27777890035288,72723460248141,190392490709135,498454011879264,1304969544928657,3416454622906707,8944394323791464,23416728348467685,61305790721611591,160500643816367088,420196140727489673,1100087778366101931,2880067194370816120
; Formula: a(n) = e(n-1)+1, b(n) = b(n-1)+d(n-1)+2, b(4) = 53, b(3) = 19, b(2) = 6, b(1) = 1, b(0) = -1, c(n) = min(c(n-1)+1,1), c(4) = 1, c(3) = 1, c(2) = 1, c(1) = 1, c(0) = 0, d(n) = b(n-1)+c(n-1)+d(n-1)+f(n-1)+4, d(4) = 87, d(3) = 32, d(2) = 11, d(1) = 3, d(0) = 0, e(n) = b(n-1)+c(n-1)+d(n-1)+2, e(4) = 54, e(3) = 20, e(2) = 7, e(1) = 1, e(0) = 0, f(n) = b(n-1)+c(n-1)+d(n-1)+f(n-1)+3, f(4) = 86, f(3) = 31, f(2) = 10, f(1) = 2, f(0) = 0

#offset 1

mov $1,-1
sub $0,1
lpb $0
  sub $0,1
  sub $3,$4
  add $4,1
  add $1,$3
  add $1,$4
  add $1,1
  mov $4,$2
  add $4,$1
  add $2,1
  min $2,1
  add $5,1
  add $5,$4
  mov $3,$5
  add $3,1
lpe
mov $0,$4
add $0,1
