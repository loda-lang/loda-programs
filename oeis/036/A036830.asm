; A036830: Schoenheim bound L_1(n,n-4,n-5).
; Submitted by [SG]KidDoesCrunch
; 3,7,14,26,44,70,105,152,213,291,388,508,654,829,1037,1281,1566,1896,2276,2710,3203,3761,4388,5091,5875,6746,7710,8774,9944,11228,12632,14164,15831,17641,19602,21722,24009,26472,29120,31961,35005
; Formula: a(n) = c(n+2)/2, b(n) = 2*(((b(n-1)+d(n-1))*(-n+2))/n)+2*c(n-1), b(4) = 14, b(3) = 8, b(2) = 4, b(1) = 2, b(0) = 1, c(n) = 2*(((b(n-1)+d(n-1))*(-n+2))/n)+2*c(n-1)+c(n-1), c(4) = 28, c(3) = 14, c(2) = 6, c(1) = 2, c(0) = 0, d(n) = c(n-1), d(4) = 14, d(3) = 6, d(2) = 2, d(1) = 0, d(0) = 0

mov $1,1
mov $2,1
add $0,2
lpb $0
  sub $0,1
  add $1,$5
  mul $1,$2
  sub $2,1
  add $4,1
  mov $5,$3
  div $1,$4
  add $1,$3
  mul $1,2
  add $3,$1
lpe
mov $0,$3
div $0,2
