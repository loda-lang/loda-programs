; A040367: Continued fraction for sqrt(387).
; Submitted by andrew
; 19,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1,2,19,2,1,38,1
; Formula: a(n) = truncate((6*gcd(10*truncate((truncate(3^(gcd(2*n,max(-2*n+2,0)+6)-3))+1)/10)-truncate(3^(gcd(2*n,max(-2*n+2,0)+6)-3))-1,2)*gcd(-n,2)*(-10*truncate((truncate(3^(gcd(2*n,max(-2*n+2,0)+6)-3))+1)/10)+truncate(3^(gcd(2*n,max(-2*n+2,0)+6)-3))+1))/5)

sub $1,$0
gcd $1,2
mul $0,2
mov $4,2
trn $4,$0
add $4,6
gcd $0,$4
sub $0,3
mov $3,3
pow $3,$0
mov $0,$3
add $0,1
mod $0,10
sub $2,$0
gcd $2,2
mul $2,$0
mov $0,$2
mul $0,$1
mul $0,6
div $0,5
