; A040560: Continued fraction for sqrt(585).
; Submitted by loader3229
; 24,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5,48,5,2,1,4,1,2,5
; Formula: a(n) = ((sign(n)*((n-1)%8+1))==5)+((sign(n)*((n-1)%8+1))==3)+48*((sign(n)*((n-1)%8+1))==8)+24*((sign(n)*((n-1)%8+1))==0)+5*((sign(n)*((n-1)%8+1))==7)+5*((sign(n)*((n-1)%8+1))==1)+4*((sign(n)*((n-1)%8+1))==4)+2*((sign(n)*((n-1)%8+1))==6)+2*((sign(n)*((n-1)%8+1))==2)

dgr $0,9
mov $1,$0
equ $1,0
mul $1,24
mov $2,$1
mov $1,$0
equ $1,1
mul $1,5
add $2,$1
mov $1,$0
equ $1,2
mul $1,2
add $2,$1
mov $1,$0
equ $1,3
add $2,$1
mov $1,$0
equ $1,4
mul $1,4
add $2,$1
mov $1,$0
equ $1,5
add $2,$1
mov $1,$0
equ $1,6
mul $1,2
add $2,$1
mov $1,$0
equ $1,7
mul $1,5
add $2,$1
mov $1,$0
equ $1,8
mul $1,48
add $2,$1
mov $0,$2
