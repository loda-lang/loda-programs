...
; Formula: a(n) = -2*truncate(b(n-1)/2)+b(n-1)+1, b(n) = c(n-1)+truncate(b(n-1)/2), b(2) = 9, b(1) = 2, b(0) = 0, c(n) = gcd(truncate(b(n-1)/2)+truncate(d(n-1)/2),4)*c(n-1), c(2) = 8, c(1) = 8, c(0) = 2, d(n) = gcd(truncate(b(n-1)/2)+truncate(d(n-1)/2),4), d(2) = 1, d(1) = 4, d(0) = 0
