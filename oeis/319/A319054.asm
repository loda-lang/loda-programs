; A319054: Maximum product of an aperiodic integer partition of n.
; Submitted by Jamie Morken(w3)
; 1,2,3,4,6,8,12,18,24,36,54,72,108,162,216,324,486,648,972,1458,1944,2916,4374,5832,8748,13122,17496,26244,39366,52488,78732,118098,157464,236196,354294,472392,708588,1062882,1417176,2125764,3188646,4251528,6377292
; Formula: a(n) = (2*min(n+3,(n+3)%3)*c(n+3)+2*b(n+3)+34)/3-11, b(n) = 2*b(n-3)+2*c(n-3), b(8) = 6, b(7) = 6, b(6) = 6, b(5) = 2, b(4) = 2, b(3) = 2, b(2) = 1, b(1) = 1, b(0) = 1, c(n) = b(n-3)+c(n-3), c(8) = 3, c(7) = 3, c(6) = 3, c(5) = 1, c(4) = 1, c(3) = 1, c(2) = 0, c(1) = 0, c(0) = 0

mov $1,1
add $0,3
lpb $0
  sub $0,3
  add $1,$2
  mov $2,$1
  mul $1,2
lpe
mul $0,$2
add $0,$1
add $0,17
mul $0,2
div $0,3
sub $0,11
