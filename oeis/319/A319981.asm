...
; Formula: a(n) = -2*truncate((b(n)+d(n))/2)+b(n)+d(n)+1, b(n) = truncate((-c(n-1)+b(n-1))/16), b(2) = 1, b(1) = 0, b(0) = 0, c(n) = -gcd(d(n-1)+truncate((-c(n-1)+b(n-1))/16),4)*c(n-1), c(2) = 16, c(1) = -16, c(0) = 4, d(n) = gcd(d(n-1)+truncate((-c(n-1)+b(n-1))/16),4), d(2) = 1, d(1) = 4, d(0) = 0
