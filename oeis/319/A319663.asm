; A319663: Irregular triangle read by rows: T(n,k) = 5^k mod 2^n, n >= 2, 0 <= k <= 2^(n-2) - 1.
; Submitted by fzs600
; 1,1,5,1,5,9,13,1,5,25,29,17,21,9,13,1,5,25,61,49,53,9,45,33,37,57,29,17,21,41,13,1,5,25,125,113,53,9,45,97,101,121,93,81,21,105,13,65,69,89,61,49,117,73,109,33,37,57,29,17,85,41,77
; Formula: a(n) = 4*truncate(c(n+2)/10)+1, b(n) = truncate((n-1)/b(n-1))*b(n-1)+b(n-1), b(3) = 4, b(2) = 2, b(1) = 1, b(0) = 1, c(n) = -5*truncate(c(n-1)/b(n-1))*b(n-1)+5*c(n-1)+2, c(3) = 2, c(2) = 2, c(1) = 2, c(0) = 0

mov $1,1
add $0,2
lpb $0
  sub $0,1
  mod $2,$1
  div $3,$1
  mul $3,$1
  add $4,1
  add $1,$3
  mul $2,5
  add $2,2
  mov $3,$4
lpe
mov $0,$2
div $0,10
mul $0,4
add $0,1
