...
; Formula: a(n) = b(n)/2+1, b(n) = 2*b(n-1)-c(n-1)-d(n-1)-e(n-1)-f(n-1), b(5) = 180, b(4) = 42, b(3) = 10, b(2) = 2, b(1) = 0, b(0) = 0, c(n) = 3*c(n-2)+3*f(n-2)+2*e(n-2)-b(n-2)+c(n-1)+c(n-2)+d(n-2)+e(n-2)+f(n-2), c(5) = -76, c(4) = -14, c(3) = -2, c(2) = 0, c(1) = 0, c(0) = 0, d(n) = -b(n-1)+d(n-1)-2, d(5) = -64, d(4) = -20, d(3) = -8, d(2) = -4, d(1) = -2, d(0) = 0, e(n) = 2*e(n-1)-b(n-1)+c(n-1)+d(n-1)+e(n-1)+f(n-1), e(5) = -250, e(4) = -56, e(3) = -12, e(2) = -2, e(1) = 0, e(0) = 0, f(n) = 3*c(n-1)+3*f(n-1), f(5) = -60, f(4) = -6, f(3) = 0, f(2) = 0, f(1) = 0, f(0) = 0
