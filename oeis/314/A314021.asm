...
; Formula: a(n) = -truncate((7*n-1)/(2*n+floor((2*max(2*truncate((7*n-3*truncate(floor(n/3)/3)+floor(n/3))/3)-1,0)+3)/3)))*(2*n+floor((2*max(2*truncate((7*n-3*truncate(floor(n/3)/3)+floor(n/3))/3)-1,0)+3)/3))+10*n
