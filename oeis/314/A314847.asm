...
; Formula: a(n) = -truncate((7*n-1)/(max(truncate((16*n+2*floor((5*n)/6)-4*truncate(floor((5*n)/6)/2))/3)-1,0)+1))*(max(truncate((16*n+2*floor((5*n)/6)-4*truncate(floor((5*n)/6)/2))/3)-1,0)+1)+10*n
