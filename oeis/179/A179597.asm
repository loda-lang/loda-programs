; A179597: Eight white kings and one red king on a 3 X 3 chessboard. G.f.: (1 + 5*x + 2*x^2)/(1 - 2*x - 11*x^2 - 6*x^3).
; Submitted by Christian Krause
; 1,7,27,137,613,2895,13355,62233,288741,1342175,6233899,28964169,134554277,625117807,2904117675,13491856889,62679715045,291194561919,1352817130667,6284852732713,29197861274277,135646005392399
; Formula: a(n) = c(n+1)+e(n+1), b(n) = d(n-1), b(4) = 117, b(3) = 20, b(2) = 7, b(1) = 0, b(0) = 0, c(n) = d(n-1), c(4) = 117, c(3) = 20, c(2) = 7, c(1) = 0, c(0) = 1, d(n) = 6*b(n-1)+6*c(n-1)+6*f(n-1)+d(n-1)+1, d(4) = 496, d(3) = 117, d(2) = 20, d(1) = 7, d(0) = 0, e(n) = c(n-1), e(4) = 20, e(3) = 7, e(2) = 0, e(1) = 1, e(0) = 0, f(n) = 2*c(n-1)+b(n-1)+f(n-1), f(4) = 83, f(3) = 23, f(2) = 2, f(1) = 2, f(0) = 0

mov $2,1
add $0,1
lpb $0
  sub $0,1
  add $1,$2
  add $5,$1
  mov $1,$3
  mov $3,$5
  mul $3,6
  add $3,1
  add $3,$1
  mov $4,$2
  mov $2,$1
  add $5,$4
lpe
add $2,$4
mov $0,$2
