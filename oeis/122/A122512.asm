; A122512: Expansion of 1/(1 - x^2 - 2 x^3 + x^4).
; Submitted by vaughan
; 1,0,1,2,0,4,3,2,11,4,12,24,9,44,45,38,124,84,155,294,199,520,632,624,1473,1368,2089,3690,3352,6500,8643,9514,18291,20300,28676,47368,50985,84420,117045,139022,234900,288692,395899,619470,738383,1122576,1581424
; Formula: a(n) = d(n+1), b(n) = b(n-1)+c(n-1), b(3) = 2, b(2) = 1, b(1) = 1, b(0) = 1, c(n) = b(n-2)+e(n-2), c(3) = 0, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = b(n-1)+e(n-1), d(3) = 1, d(2) = 0, d(1) = 1, d(0) = 0, e(n) = -b(n-1)-e(n-1)+c(n-1), e(3) = 0, e(2) = 0, e(1) = -1, e(0) = 0

mov $1,1
add $0,1
lpb $0
  sub $0,1
  add $4,$1
  mov $5,$4
  add $1,$2
  mov $4,$2
  sub $4,$5
  mov $2,$3
  mov $3,$5
lpe
mov $0,$3
