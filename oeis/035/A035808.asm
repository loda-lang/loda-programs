; A035808: Coordination sequence for lattice D*_46 (with edges defined by l_1 norm = 1).
; Submitted by iBezanilla
; 1,92,4232,129812,2987792,55053260,846141848,11160045188,128975126048,1327092434748,12312430500520,104062494360052,808072815510832,5806722035765932,38851092934042552,243319195759791460,1433080081936088128
; Formula: a(n) = b(2*n), b(n) = truncate((92*b(n-2)+23*c(n-2))/(d(n-2)+1)), b(7) = 129812, b(6) = 129812, b(5) = 4232, b(4) = 4232, b(3) = 92, b(2) = 92, b(1) = 1, b(0) = 1, c(n) = 4*b(n-4)+c(n-4), c(8) = 16932, c(7) = 368, c(6) = 368, c(5) = 4, c(4) = 4, c(3) = 0, c(2) = 0, c(1) = 0, c(0) = 0, d(n) = d(n-2)+1, d(7) = 3, d(6) = 3, d(5) = 2, d(4) = 2, d(3) = 1, d(2) = 1, d(1) = 0, d(0) = 0

mov $1,1
mul $0,2
lpb $0
  sub $0,2
  mul $1,4
  add $1,$2
  mov $2,$3
  mov $3,$1
  add $4,1
  mul $1,23
  div $1,$4
lpe
mov $0,$1
