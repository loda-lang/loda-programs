; A111645: Expansion of (x+1)*(1-3*x)/((x^2+4*x+1)*(x^2-2*x-1)).
; Submitted by [AF>Libristes]Maeda
; -1,8,-37,156,-625,2436,-9341,35464,-133809,502896,-1885317,7056580,-26384961,98589388,-368228797,1374944336,-5133041825,19160828056,-71518973861,266936079404,-996276071249,3718290672596,-13877182280637,51791152239960,-193289149920721
; Formula: a(n) = c(n+1), b(n) = -2*b(n-1)+b(n-2), b(3) = -24, b(2) = 10, b(1) = -4, b(0) = 2, c(n) = b(n-1)+d(n-1), c(3) = -37, c(2) = 8, c(1) = -1, c(0) = 0, d(n) = -b(n-2)-d(n-2)-4*d(n-1), d(3) = 180, d(2) = -47, d(1) = 12, d(0) = -3

mov $1,2
mov $4,-3
add $0,1
lpb $0
  sub $0,1
  sub $3,$1
  mov $5,$1
  mov $6,$4
  add $6,$1
  mul $4,4
  add $4,$2
  mul $4,-1
  mul $1,-1
  add $1,$3
  mov $2,$6
  mov $3,$5
lpe
mov $0,$2
