; A111989: G.f.: 1/(1-6*x+8*x^3).
; Submitted by misaki@med
; 1,6,36,208,1200,6912,39808,229248,1320192,7602688,43782144,252131328,1451966464,8361541632,48152199168,277297463296,1596892446720,9196137086976,52958442815488,304975517319168,1756284007219200
; Formula: a(n) = truncate(d(n+3)/8), b(n) = 8*c(n-1)+2*b(n-1)-8*c(n-2), b(3) = 40, b(2) = 8, b(1) = 0, b(0) = 1, c(n) = 4*c(n-1)+b(n-1), c(2) = 4, c(1) = 1, c(0) = 0, d(n) = b(n-1)+d(n-1), d(2) = 2, d(1) = 2, d(0) = 1

mov $1,1
mov $4,1
add $0,3
lpb $0
  sub $0,1
  mov $2,$1
  mul $3,4
  add $4,$1
  mov $1,$3
  sub $1,1
  mul $1,2
  add $1,$4
  add $3,$2
lpe
mov $0,$4
div $0,8
