; A117085: Decimal value of binary number whose n-th 1 is followed by L(n) 0's.
; Submitted by Simon Strandgaard
; 9,37,593,18977,4858113,19898830849,10432718228160513,11202045899583117393395713,3153095608696857509125380749498911817729,476482336633392067336089761008918511215119000704844672205324289
; Formula: a(n) = b(n+1), b(n) = 2*e(n-1)*b(n-1)+1, b(3) = 593, b(2) = 37, b(1) = 9, b(0) = 1, c(n) = (e(n-1)*d(n-1)+c(n-1))%2, c(3) = 0, c(2) = 0, c(1) = 0, c(0) = 0, d(n) = e(n-1)*d(n-1)+c(n-1), d(3) = 128, d(2) = 16, d(1) = 8, d(0) = 2, e(n) = d(n-1), e(3) = 16, e(2) = 8, e(1) = 2, e(0) = 4

mov $1,1
mov $3,2
mov $4,4
add $0,1
lpb $0
  sub $0,1
  mul $1,2
  mul $1,$4
  add $1,1
  mul $4,$3
  add $2,$4
  mov $4,$3
  mov $3,$2
  mod $2,2
lpe
mov $0,$1
