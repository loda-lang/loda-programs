...
; Formula: a(n) = -2*truncate(d(n-1)/2)+d(n-1)+1, b(n) = c(n-1)+truncate(b(n-1)/2), b(2) = 5, b(1) = 2, b(0) = 0, c(n) = gcd(d(n-1)+truncate(b(n-1)/2),2)*c(n-1), c(2) = 4, c(1) = 4, c(0) = 2, d(n) = gcd(d(n-1)+truncate(b(n-1)/2),2), d(2) = 1, d(1) = 2, d(0) = 0
