...
; Formula: a(n) = truncate((5*c(2*n+1)*(if((c(2*n+1)^2)==1,c(2*n+1)^d(2*n+1),if(d(2*n+1)<=(-1),0,c(2*n+1)^d(2*n+1)))-1)*d(2*n+1)^2-405)/27)+15, b(n) = b(n-1)+truncate((c(n-1)+d(n-1))/2), b(2) = -6, b(1) = -3, b(0) = -3, c(n) = 2*b(n-1)-truncate((c(n-1)+d(n-1))/2), c(2) = -3, c(1) = -6, c(0) = 0, d(n) = truncate((c(n-1)+d(n-1))/2), d(2) = -3, d(1) = 0, d(0) = 0
