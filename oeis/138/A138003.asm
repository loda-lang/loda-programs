; A138003: Binomial transform of 1, 1, 0, -1, -1 (periodically continued).
; Submitted by USTL-FIL (Lille Fr)
; 1,2,3,3,0,-8,-21,-34,-34,0,89,233,377,377,0,-987,-2584,-4181,-4181,0,10946,28657,46368,46368,0,-121393,-317811,-514229,-514229,0,1346269,3524578,5702887,5702887,0,-14930352,-39088169,-63245986,-63245986
; Formula: a(n) = b(n+1), b(n) = b(n-1)+c(n-1)+d(n-1)+e(n-1), b(3) = 3, b(2) = 2, b(1) = 1, b(0) = 0, c(n) = -b(n-1)-c(n-1)-d(n-1)+c(n-1), c(3) = -3, c(2) = -1, c(1) = 0, c(0) = 0, d(n) = b(n-1)+c(n-1)+d(n-1), d(3) = 2, d(2) = 1, d(1) = 0, d(0) = 0, e(n) = c(n-1)+d(n-1)+e(n-1), e(3) = 1, e(2) = 1, e(1) = 1, e(0) = 1

mov $4,1
add $0,1
lpb $0
  sub $0,1
  add $3,$2
  add $4,$3
  add $3,$1
  add $1,$4
  sub $2,$3
lpe
mov $0,$1
