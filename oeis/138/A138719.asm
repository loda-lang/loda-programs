...
; Formula: a(n) = truncate(((if(truncate((min(n,n%2)*c(n)+b(n))/3)<=(-1),0,10^truncate((min(n,n%2)*c(n)+b(n))/3))^2+1)*(if(truncate((min(n,n%2)*c(n)+b(n))/3)<=(-1),0,10^truncate((min(n,n%2)*c(n)+b(n))/3))-1))/9), b(n) = 2*b(n-2)+c(n-2), b(3) = 3, b(2) = 3, b(1) = 0, b(0) = 0, c(n) = 3*c(n-2)-c(n-4), c(6) = 15, c(5) = 6, c(4) = 6, c(3) = 3, c(2) = 3, c(1) = 3, c(0) = 3
