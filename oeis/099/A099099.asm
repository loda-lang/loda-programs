; A099099: Quadrisection of a generalized Padovan sequence.
; Submitted by USTL-FIL (Lille Fr)
; 1,1,1,2,6,16,37,80,172,377,839,1874,4175,9274,20577,45665,101393,225193,500162,1110790,2466760,5477917,12164896,27015092,59993817,133231279,295872778,657057431,1459155634,3240410561,7196122817
; Formula: a(n) = b(n-1)+c(n-1)+a(n-1), a(3) = 2, a(2) = 1, a(1) = 1, a(0) = 1, b(n) = b(n-1)+c(n-1), b(3) = 1, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = c(n-1)+d(n-1), c(3) = 3, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = 2*d(n-1)-d(n-2)+b(n-2)+c(n-3)+d(n-3), d(4) = 5, d(3) = 3, d(2) = 2, d(1) = 1, d(0) = 0

mov $3,1
lpb $0
  sub $0,1
  add $1,$2
  add $2,$4
  add $4,$3
  add $3,$1
lpe
mov $0,$3
