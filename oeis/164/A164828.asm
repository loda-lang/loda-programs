; A164828: Generalized Fibonacci numbers.
; Submitted by loader3229
; 1,1,11,120,122,1331,1462,15960,17413,190081,207503,2265120,2472614,26991251,29463874,321630000,351093865,3832568641,4183662515,45669193800,49852856306,544197756851,594050613166,6484703888520,7078754501677,77272248905281
; Formula: a(n) = b(n-1), b(n) = 11*b(n-2)+11*b(n-4)-b(n-6), b(11) = 2265120, b(10) = 207503, b(9) = 190081, b(8) = 17413, b(7) = 15960, b(6) = 1462, b(5) = 1331, b(4) = 122, b(3) = 120, b(2) = 11, b(1) = 1, b(0) = 1

#offset 1

mov $1,1
mov $2,1
mov $3,11
mov $4,120
mov $5,122
mov $6,1331
sub $0,1
lpb $0
  mul $1,-1
  rol $1,6
  mov $7,$2
  mul $7,11
  add $6,$7
  mov $7,$4
  mul $7,11
  sub $0,1
  add $6,$7
lpe
mov $0,$1
