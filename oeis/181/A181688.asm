; A181688: Number of maximal self-avoiding walks from NW to SW corners of a 4-by-n grid
; Submitted by shift
; 1,1,4,8,23,55,144,360,921,2329,5924,15024,38159,96847,245888,624176,1584593,4022609,10211940,25924088,65811431,167069767,424126160,1076693080,2733310377,6938824361,17615009476,44717740000,113521160607,288186606623
; Formula: a(n) = b(n-1), b(n) = 3*b(n-2)+b(n-1)+b(n-3)+c(n-2), b(4) = 23, b(3) = 8, b(2) = 4, b(1) = 1, b(0) = 1, c(n) = 2*d(n-1)+c(n-1), c(3) = 4, c(2) = 2, c(1) = 0, c(0) = 0, d(n) = 3*d(n-2)+c(n-3)+d(n-1)+d(n-3), d(5) = 23, d(4) = 8, d(3) = 4, d(2) = 1, d(1) = 1, d(0) = 0

#offset 1

mov $1,1
sub $0,1
lpb $0
  sub $0,1
  mov $5,$1
  mov $6,$4
  add $1,$3
  add $1,$4
  add $1,$4
  add $2,$4
  mov $4,$5
  add $5,$2
  add $2,$6
  mov $3,$5
lpe
mov $0,$1
