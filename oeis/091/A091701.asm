; A091701: Row sums of triangle A091700.
; Submitted by Jon Maiga
; 1,3,14,63,287,1306,5945,27061,123180,560707,2552301,11617904,52883925,240724103,1095760078,4987826867,22704255571,103348258626,470434387405,2141385987489,9747446339348,44369726286423,201968037801353
; Formula: a(n) = d(n)+1, b(n) = b(n-1)+d(n-1)+1, b(3) = 18, b(2) = 4, b(1) = 1, b(0) = 0, c(n) = d(n-1)+1, c(3) = 14, c(2) = 3, c(1) = 1, c(0) = 0, d(n) = 3*d(n-1)+2*b(n-1)+2*c(n-1)+e(n-1)+2, d(3) = 62, d(2) = 13, d(1) = 2, d(0) = 0, e(n) = 2*d(n-1)+b(n-1)+c(n-1)+1, e(3) = 34, e(2) = 7, e(1) = 1, e(0) = 0

lpb $0
  sub $0,1
  add $3,1
  add $1,$3
  sub $2,1
  add $2,$1
  add $4,1
  add $4,$2
  mov $5,$4
  mov $4,$2
  add $4,$3
  add $5,$4
  mov $2,$3
  mov $3,$5
lpe
mov $0,$3
add $0,1
