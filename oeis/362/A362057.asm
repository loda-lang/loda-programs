; A362057: Number of compositions of n that are anti-palindromic modulo 3.
; Submitted by Science United
; 1,1,1,3,5,7,15,27,43,81,147,249,449,809,1409,2507,4485,7903,14015,24963,44187,78329,139203,246801,437601,776881,1378081,2444083,4337125,7694487,13648655,24215947,42962283,76213761,135212947,239883849,425562849,754987929
; Formula: a(n) = 2*b(n)+1, b(n) = e(n-1), b(5) = 3, b(4) = 2, b(3) = 1, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = -c(n-1)+d(n-1)+e(n-2)+1, c(5) = 2, c(4) = 2, c(3) = 1, c(2) = 1, c(1) = 1, c(0) = 0, d(n) = -c(n-1)+d(n-1)+1, d(5) = 0, d(4) = 1, d(3) = 1, d(2) = 1, d(1) = 1, d(0) = 0, e(n) = 3*e(n-3)-c(n-2)+d(n-2)+e(n-1)+1, e(5) = 7, e(4) = 3, e(3) = 2, e(2) = 1, e(1) = 0, e(0) = 0

lpb $0
  sub $0,1
  mov $7,$6
  add $7,$2
  add $4,1
  sub $4,$3
  mov $6,$4
  add $6,$1
  mov $2,$1
  mul $2,2
  mov $3,$6
  mov $1,$5
  add $5,$7
lpe
mov $0,$1
mul $0,2
add $0,1
