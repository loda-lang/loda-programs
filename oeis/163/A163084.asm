; A163084: Number of reduced words of length n in Coxeter group on 13 generators S_i with relations (S_i)^2 = (S_i S_j)^4 = I.
; Submitted by loader3229
; 1,13,156,1872,22386,267696,3201198,38280528,457766166,5474058876,65459882202,782782264836,9360665698098,111936698410680,1338561257884422,16006781212446024,191412266920077918,2288945881199387124,27371669179630649874,327315853045229826492
; Formula: a(n) = 2*c(n)+2*d(n)+2*e(n)+b(n)+f(n), b(n) = f(n-4), b(5) = 11, b(4) = 1, b(3) = 0, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = f(n-3), c(4) = 11, c(3) = 1, c(2) = 0, c(1) = 0, c(0) = 0, d(n) = f(n-2), d(4) = 132, d(3) = 11, d(2) = 1, d(1) = 0, d(0) = 0, e(n) = f(n-1), e(4) = 1584, e(3) = 132, e(2) = 11, e(1) = 1, e(0) = 0, f(n) = 11*f(n-1)+11*f(n-2)+11*f(n-3)-66*f(n-4), f(6) = 2707254, f(5) = 226391, f(4) = 18931, f(3) = 1584, f(2) = 132, f(1) = 11, f(0) = 1

mov $5,1
lpb $0
  sub $0,1
  mov $1,$2
  mul $2,-66
  mov $6,$2
  mov $2,$3
  mul $3,11
  add $6,$3
  mov $3,$4
  mul $4,11
  add $6,$4
  mov $4,$5
  mul $5,11
  add $5,$6
lpe
mov $0,$1
add $0,$2
add $0,$2
add $0,$3
add $0,$3
add $0,$4
add $0,$4
add $0,$5
