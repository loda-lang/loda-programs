...
; Formula: a(n) = d(n+1), b(n) = truncate(truncate((-c(n-1)+b(n-1))/2)/gcd(2*c(n-1)+truncate((-c(n-1)+b(n-1))/2),4)), b(3) = -1, b(2) = -1, b(1) = -1, b(0) = 0, c(n) = 2*c(n-1), c(3) = 16, c(2) = 8, c(1) = 4, c(0) = 2, d(n) = d(n-1)+truncate((gcd(2*c(n-2)+truncate((-c(n-2)+b(n-2))/2),4)-3)/2)+2, d(3) = 5, d(2) = 3, d(1) = 2, d(0) = 0
