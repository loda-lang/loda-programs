; A057078: Periodic sequence 1,0,-1,...; expansion of (1+x)/(1+x+x^2).
; Submitted by USTL-FIL (Lille Fr)
; 1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0,-1,1,0
; Formula: a(n) = if(((-2*n-3*truncate((-2*n)/3))^2)==1,(-2*n-3*truncate((-2*n)/3))^(-2*n-3*truncate((-2*n)/3)),if((-2*n-3*truncate((-2*n)/3))<=(-1),0,(-2*n-3*truncate((-2*n)/3))^(-2*n-3*truncate((-2*n)/3))))

mul $0,-2
mod $0,3
pow $0,$0
