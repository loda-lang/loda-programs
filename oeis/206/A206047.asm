...
; Formula: a(n) = 3*truncate((d(n+2)+e(n+2))/2), b(n) = 2*c(n-1)+2*e(n-1)+2*f(n-1)+b(n-1)+d(n-1), b(4) = 722, b(3) = 138, b(2) = 26, b(1) = 6, b(0) = 2, c(n) = c(n-1)+f(n-1), c(4) = 170, c(3) = 32, c(2) = 6, c(1) = 0, c(0) = 0, d(n) = 2*c(n-1)+2*e(n-1)+2*f(n-1), d(4) = 488, d(3) = 96, d(2) = 16, d(1) = 4, d(0) = 0, e(n) = b(n-1)+c(n-1)+d(n-1)+f(n-1), e(4) = 404, e(3) = 74, e(2) = 16, e(1) = 2, e(0) = 2, f(n) = 2*c(n-1)+2*e(n-1)+2*f(n-1)+b(n-1)+d(n-1), f(4) = 722, f(3) = 138, f(2) = 26, f(1) = 6, f(0) = 0
