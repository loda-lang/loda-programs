; A153286: a(n) = n^3 + sum((-1)^j*a(j)); for j=1 to n-1; a(1)=1.
; Submitted by loader3229
; 1,7,33,37,135,91,309,169,555,271,873,397,1263,547,1725,721,2259,919,2865,1141,3543,1387,4293,1657,5115,1951,6009,2269,6975,2611,8013,2977,9123,3367,10305,3781,11559,4219,12885,4681,14283,5167,15753,5677,17295
; Formula: a(n) = b(n-1), b(n) = 3*b(n-2)-3*b(n-4)+b(n-6), b(13) = 547, b(12) = 1263, b(11) = 397, b(10) = 873, b(9) = 271, b(8) = 555, b(7) = 169, b(6) = 309, b(5) = 91, b(4) = 135, b(3) = 37, b(2) = 33, b(1) = 7, b(0) = 1

#offset 1

mov $1,1
mov $2,7
mov $3,33
mov $4,37
mov $5,135
mov $6,91
mov $7,309
sub $0,1
lpb $0
  mov $1,0
  rol $1,7
  mov $8,$3
  mul $8,-3
  add $7,$1
  add $7,$8
  mov $8,$5
  mul $8,3
  sub $0,1
  add $7,$8
lpe
mov $0,$1
