; A133681: a(n)=9*a(n-1)+90*a(n-2) for n>=3, a(0)=1, a(1)=9, a(2)=162 .
; Submitted by GolfSierra
; 1,9,162,2268,34992,519048,7820712,117100728,1757770632,26359001208,395430367752,5931183418488,88969383864072,1334530962440568,20018023209731592,300269995507235448,4504052048440962312
; Formula: a(n) = d(n)+1, b(n) = 15*b(n-1), b(2) = 900, b(1) = 60, b(0) = 4, c(n) = (-3)*(c(n-1)+1)+((45*b(n-2)+(-3)*(c(n-1)+1))-1), c(2) = 143, c(1) = 5, c(0) = 0, d(n) = (45*b(n-2)+(-3)*((-3)*(c(n-2)+1)+((3*b(n-2)+(-3)*(c(n-2)+1))-1)+1))-1, d(2) = 161, d(1) = 8, d(0) = 0

mov $1,4
lpb $0
  sub $0,1
  mul $1,3
  mov $3,$1
  mul $1,5
  add $2,1
  mul $2,-3
  add $3,$2
  sub $3,1
  add $2,$3
lpe
mov $0,$3
add $0,1
