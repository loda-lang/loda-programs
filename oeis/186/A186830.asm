; A186830: Keith sequence for the number 197.
; Submitted by Jon Maiga
; 1,9,7,17,33,57,107,197,361,665,1223,2249,4137,7609,13995,25741,47345,87081,160167,294593,541841,996601,1833035,3371477,6201113,11405625,20978215,38584953,70968793,130531961
; Formula: a(n) = d(n-1), b(n) = b(n-2)+b(n-3)+c(n-3), b(4) = 4, b(3) = 3, b(2) = 6, b(1) = -5, b(0) = 2, c(n) = c(n-1)+c(n-2)+c(n-3), c(3) = 13, c(2) = 4, c(1) = 3, c(0) = 6, d(n) = b(n-1)+c(n-1)+d(n-1), d(2) = 7, d(1) = 9, d(0) = 1

#offset 1

mov $1,2
mov $2,6
mov $3,1
sub $0,1
lpb $0
  sub $0,1
  add $2,$1
  add $1,$3
  sub $1,$2
  add $3,$2
  add $2,$1
lpe
mov $0,$3
