...
; Formula: a(n) = A167277(A000720(6*A002822(floor(max(2*n-1,0)/2))+2*gcd(max(2*n-1,0)-1,2)-7)*truncate((2*n-1)/A000720(6*A002822(floor(max(2*n-1,0)/2))+2*gcd(max(2*n-1,0)-1,2)-7))-2*n+A000720(6*A002822(floor(max(2*n-1,0)/2))+2*gcd(max(2*n-1,0)-1,2)-7))+1
