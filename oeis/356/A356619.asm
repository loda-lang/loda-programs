; A356619: a(n) = number of k-tuples (u(1), u(2), ..., u(k)) with 1 <= u(1) < u(2) < ... < u(k) <= n such that u(i) - u(i-1) <= 3 for i = 2,...,k.
; Submitted by Dark Angel
; 0,1,4,11,25,52,103,198,374,699,1298,2401,4431,8166,15037,27676,50924,93685,172336,316999,583077,1072472,1972611,3628226,6673378,12274287,22575966,41523709,76374043,140473802,258371641,475219576,874065112,1607656425
; Formula: a(n) = a(n-1)+c(n-1)+1, a(4) = 25, a(3) = 11, a(2) = 4, a(1) = 1, a(0) = 0, b(n) = b(n-1)+b(n-2)+b(n-3)+7, b(5) = 93, b(4) = 49, b(3) = 25, b(2) = 12, b(1) = 5, b(0) = 1, c(n) = 2*d(n-1)+b(n-1)+1, c(4) = 26, c(3) = 13, c(2) = 6, c(1) = 2, c(0) = 0, d(n) = 0, d(4) = 0, d(3) = 0, d(2) = 0, d(1) = 0, d(0) = 0

mov $1,1
lpb $0
  sub $0,1
  add $3,1
  add $4,1
  mov $5,$1
  add $5,$4
  add $5,$4
  add $6,$5
  mov $1,1
  add $1,$3
  add $1,$6
  add $2,$3
  mov $4,0
  sub $5,1
  mov $6,$3
  mov $3,$5
lpe
mov $0,$2
