; A356050: a(n) = 2*A135318(n+1) - A135318(n).
; Submitted by stoneageman
; 1,1,3,4,5,6,11,14,21,26,43,54,85,106,171,214,341,426,683,854,1365,1706,2731,3414,5461,6826,10923,13654,21845,27306,43691,54614,87381,109226,174763,218454,349525,436906,699051,873814,1398101,1747626,2796203,3495254,5592405
; Formula: a(n) = c(n)+1, b(n) = 2*d(n-2)-b(n-1)+2, b(4) = 1, b(3) = 1, b(2) = 1, b(1) = 2, b(0) = 0, c(n) = b(n-1)+c(n-1), c(4) = 4, c(3) = 3, c(2) = 2, c(1) = 0, c(0) = 0, d(n) = 2*d(n-4)-b(n-3)+d(n-1)+2, d(6) = 5, d(5) = 4, d(4) = 3, d(3) = 2, d(2) = 0, d(1) = 0, d(0) = 0

mov $3,1
lpb $0
  sub $0,1
  mov $4,$2
  mul $6,2
  mov $2,2
  sub $2,$4
  add $2,$1
  add $3,$6
  mul $6,0
  mov $1,$3
  mov $3,$6
  mov $6,$5
  add $5,$4
lpe
add $5,1
mov $0,$5
