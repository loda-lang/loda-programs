; A083336: a(n)=4a(n-2)-a(n-4).
; Submitted by Stony666
; 3,3,9,12,33,45,123,168,459,627,1713,2340,6393,8733,23859,32592,89043,121635,332313,453948,1240209,1694157,4628523,6322680,17273883,23596563,64467009,88063572,240594153,328657725,897909603
; Formula: a(n) = min(n+1,(n+1)%2)*c(n+1)+b(n+1), b(n) = 2*b(n-2)+c(n-2), b(5) = 12, b(4) = 12, b(3) = 3, b(2) = 3, b(1) = 0, b(0) = 0, c(n) = 2*b(n-2)+b(n-2)+2*c(n-2), c(5) = 21, c(4) = 21, c(3) = 6, c(2) = 6, c(1) = 3, c(0) = 3

mov $2,3
add $0,1
lpb $0
  sub $0,2
  add $2,$1
  add $1,$2
  add $2,$1
lpe
mul $0,$2
add $0,$1
