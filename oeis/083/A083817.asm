; A083817: Interleaving of (1,2,3,4,5,...), (2,2,2,2,...) and (0,1,3,6,10,...).
; Submitted by loader3229
; 1,2,0,2,2,1,3,2,3,4,2,6,5,2,10,6,2,15,7,2,21,8,2,28,9,2,36,10,2,45,11,2,55,12,2,66,13,2,78,14,2,91,15,2,105,16,2,120,17,2,136,18,2,153,19,2,171,20,2,190,21,2,210,22,2,231,23,2,253,24,2,276,25,2,300,26,2
; Formula: a(n) = b(n-3), a(8) = 3, a(7) = 2, a(6) = 3, a(5) = 1, a(4) = 2, a(3) = 2, a(2) = 0, a(1) = 2, a(0) = 1, b(n) = c(n-3), b(8) = 6, b(7) = 2, b(6) = 4, b(5) = 3, b(4) = 2, b(3) = 3, b(2) = 1, b(1) = 2, b(0) = 2, c(n) = 3*c(n-3)-3*b(n-3)+b(n-6), c(9) = 6, c(8) = 10, c(7) = 2, c(6) = 5, c(5) = 6, c(4) = 2, c(3) = 4, c(2) = 3, c(1) = 2, c(0) = 3

mov $1,1
mov $2,2
mov $4,2
mov $5,2
mov $6,1
mov $7,3
mov $8,2
mov $9,3
lpb $0
  rol $1,9
  mov $10,$3
  mul $10,-3
  add $9,$10
  mov $10,$6
  mul $10,3
  sub $0,1
  add $9,$10
lpe
mov $0,$1
