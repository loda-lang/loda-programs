; A004528: Ratios of successive terms are 1,2,2,2,3,4,4,4,5,6,6,6,7,...
; Submitted by Christian Krause
; 1,1,2,4,8,24,96,384,1536,7680,46080,276480,1658880,11612160,92897280,743178240,5945425920,53508833280,535088332800,5350883328000,53508833280000,588597166080000,7063165992960000
; Formula: a(n) = a(n-1)*(-2*truncate(b(n-1)/2)+b(n-1)+c(n-1)), a(3) = 4, a(2) = 2, a(1) = 1, a(0) = 1, b(n) = 3*n+b(n-1)+3, b(3) = 28, b(2) = 16, b(1) = 7, b(0) = 1, c(n) = -2*truncate(b(n-1)/2)+b(n-1)+c(n-1), c(3) = 2, c(2) = 2, c(1) = 1, c(0) = 0

mov $1,3
mov $2,1
mov $4,1
lpb $0
  sub $0,1
  add $1,3
  mov $3,$2
  mod $3,2
  add $5,$3
  add $2,$1
  mul $4,$5
lpe
mov $0,$4
