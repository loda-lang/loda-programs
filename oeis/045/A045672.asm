; A045672: Extension of Beatty sequence; complement of A045671 (apart from the initial 0).
; Submitted by PDW
; 0,4,8,12,18,22,26,32,36,40,46,50,54,58,62,68,72,76,82,86,90,96,100,104,108,112,118,122,126,132,136,140,146,150,154,158,162,168,172,176,182,186,190,196,200,204,210,214,218,224,228,232,236,240,246,250
; Formula: a(n) = d(n-1)+a(n-1)+4, a(3) = 12, a(2) = 8, a(1) = 4, a(0) = 0, b(n) = truncate((-2*c(n-1)+b(n-1))/2), b(3) = -73, b(2) = -18, b(1) = -4, b(0) = 0, c(n) = 4*gcd(d(n-1)+truncate((-2*c(n-1)+b(n-1))/2)-3,4)*c(n-1), c(3) = 1024, c(2) = 64, c(1) = 16, c(0) = 4, d(n) = truncate(gcd(d(n-1)+truncate((-2*c(n-1)+b(n-1))/2)-3,4)/2), d(3) = 2, d(2) = 0, d(1) = 0, d(0) = 0

mov $2,4
lpb $0
  sub $0,1
  mul $2,2
  add $4,$3
  add $4,4
  sub $1,$2
  div $1,2
  add $3,$1
  sub $3,3
  gcd $3,4
  mul $2,2
  mul $2,$3
  div $3,2
lpe
mov $0,$4
