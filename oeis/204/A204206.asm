...
; Formula: a(n) = b(n-1)+3, b(n) = -binomial(max(-binomial(truncate((sqrtint(8*n)-1)/2)+1,2)+n-10,0),2)+b(n-1)+max(binomial(truncate((sqrtint(8*n)-1)/2)+1,-binomial(truncate((sqrtint(8*n)-1)/2)+1,2)+n),2), b(0) = 0
