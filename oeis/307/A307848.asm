; A307848: The number of exponential infinitary divisors of n.
; Submitted by shiva
; 1,1,1,2,1,1,1,2,2,1,1,2,1,1,1,2,1,2,1,2,1,1,1,2,2,1,2,2,1,1,1,2,1,1,1,4,1,1,1,2,1,1,1,2,2,1,1,2,2,2,1,2,1,2,1,2,1,1,1,2,1,1,2,4,1,1,1,2,1,1,1,4,1,1,2,2,1,1,1,2
; Formula: a(n) = A037445(gcd(0,floor((n-1)/A019554(max(0,n-1)+1))+1))*((if(if(((5*n-40)^2)==1,(5*n-40)^(5*n-40),if((5*n-40)<=(-1),0,(5*n-40)^(5*n-40)))==0,0,valuation(if(((5*n-40)^2)==1,(5*n-40)^(5*n-40),if((5*n-40)<=(-1),0,(5*n-40)^(5*n-40))),2))+1)%10)

#offset 1

mov $5,$0
sub $5,1
mov $3,$0
max $4,$5
add $4,1
seq $4,19554 ; Smallest number whose square is divisible by n.
sub $0,8
sub $3,1
div $3,$4
add $3,1
gcd $1,$3
seq $1,37445 ; Number of infinitary divisors (or i-divisors) of n.
mov $2,$0
mul $0,2
add $2,$0
add $0,$2
pow $0,$0
lex $0,2
add $0,1
mod $0,10
mul $0,$1
