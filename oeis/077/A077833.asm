; A077833: Expansion of 1/(1-2*x-3*x^2-2*x^3).
; Submitted by Conan
; 1,2,7,22,69,218,687,2166,6829,21530,67879,214006,674709,2127194,6706527,21144054,66662077,210169370,662613079,2089058422,6586294821,20764991066,65466983439,206401529718,650733991885,2051606539802,6468218114695,20392723832566
; Formula: a(n) = (d(n+1)-2)/2+1, b(n) = 2*d(n-1)-c(n-1)-2*b(n-1)+b(n-1), b(2) = 3, b(1) = -1, b(0) = 1, c(n) = 2*b(n-1)+c(n-1), c(2) = 0, c(1) = 2, c(0) = 0, d(n) = 2*b(n-1)+2*d(n-1)+c(n-1), d(2) = 4, d(1) = 2, d(0) = 0

mov $1,1
add $0,1
lpb $0
  sub $0,1
  add $2,$1
  add $2,$1
  mul $3,2
  sub $1,$2
  add $1,$3
  add $3,$2
lpe
mov $0,$3
sub $0,2
div $0,2
add $0,1
