; A077838: Expansion of (1-x)/(1-3*x-2*x^2-3*x^3).
; Submitted by shiva
; 1,2,8,31,115,431,1616,6055,22690,85028,318629,1194013,4474381,16767056,62831969,235453162,882324592,3306376007,12390136691,46430135863,173989808992,652000108775,2443270351898,9155780700220,34309883130781,128571021848477
; Formula: a(n) = c(n)+1, b(n) = 6*b(n-1)+3*c(n-1)+3*d(n-1)-3*b(n-1)-3*d(n-1)+3, b(2) = 33, b(1) = 9, b(0) = 2, c(n) = 2*b(n-1)+2*d(n-1)-b(n-1)-c(n-1)-d(n-1)+c(n-1)-1, c(2) = 7, c(1) = 1, c(0) = 0, d(n) = -b(n-1)-c(n-1)-d(n-1)+b(n-1)+d(n-1)-1, d(2) = -2, d(1) = -1, d(0) = 0

mov $1,2
lpb $0
  sub $0,1
  add $3,$1
  add $2,$3
  sub $3,$2
  sub $3,1
  sub $1,$3
  mul $1,3
  add $2,$3
lpe
add $2,1
mov $0,$2
