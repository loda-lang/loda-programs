; A227376: G.f.: 1/(1 - x - x^2 - x^3 + x^5 + x^6 + x^7).
; Submitted by Vester
; 1,1,2,4,7,12,21,36,62,106,181,309,527,898,1530,2606,4438,7557,12867,21907,37297,63497,108100,184032,313298,533359,907988,1545751,2631469,4479778,7626309,12982911,22101900,37625912,64053725,109043981,185634620,316021206,537989084,915863373
; Formula: a(n) = b(n+2), b(n) = c(n-1), b(6) = 7, b(5) = 4, b(4) = 2, b(3) = 1, b(2) = 1, b(1) = 0, b(0) = 0, c(n) = -c(n-5)+c(n-1)+c(n-2)+c(n-3)+truncate(c(n-6)/(-1))+truncate(c(n-7)/(-1)), c(9) = 62, c(8) = 36, c(7) = 21, c(6) = 12, c(5) = 7, c(4) = 4, c(3) = 2, c(2) = 1, c(1) = 1, c(0) = 0

mov $5,1
add $0,2
lpb $0
  sub $0,1
  mov $7,$6
  div $4,-1
  mov $6,$4
  add $6,$8
  mov $4,$2
  mov $2,$1
  add $5,$7
  sub $7,$4
  mov $1,$3
  mov $3,$8
  mov $8,$5
  add $5,$7
lpe
mov $0,$3
