...
; Formula: a(n) = floor(d(n-1)/2)+1, b(n) = truncate((-d(n-1)*c(n-1)+b(n-1))/2), b(2) = -4, b(1) = -1, b(0) = 0, c(n) = d(n-1)*c(n-1), c(2) = 8, c(1) = 2, c(0) = 2, d(n) = gcd(-2*truncate((d(n-1)+truncate((-d(n-1)*c(n-1)+b(n-1))/2))/2)+d(n-1)+truncate((-d(n-1)*c(n-1)+b(n-1))/2),4), d(2) = 4, d(1) = 4, d(0) = 1
