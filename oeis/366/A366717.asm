; A366717: Smallest prime dividing 12^n - 1.
; Submitted by loader3229
; 11,11,11,5,11,7,11,5,11,11,11,5,11,11,11,5,11,7,11,5,11,11,11,5,11,11,11,5,11,7,11,5,11,11,11,5,11,11,11,5,11,7,11,5,11,11,11,5,11,11,11,5,11,7,11,5,11,11,11,5,11,11,11,5,11,7,11,5,11,11,11
; Formula: a(n) = 11*(((n-1)%12)==10)+11*(((n-1)%12)==9)+11*(((n-1)%12)==8)+11*(((n-1)%12)==6)+11*(((n-1)%12)==4)+11*(((n-1)%12)==2)+11*(((n-1)%12)==1)+11*(((n-1)%12)==0)+7*(((n-1)%12)==5)+5*(((n-1)%12)==11)+5*(((n-1)%12)==7)+5*(((n-1)%12)==3)

#offset 1

sub $0,1
mod $0,12
mov $1,$0
equ $1,0
mul $1,11
mov $2,$1
mov $1,$0
equ $1,1
mul $1,11
add $2,$1
mov $1,$0
equ $1,2
mul $1,11
add $2,$1
mov $1,$0
equ $1,3
mul $1,5
add $2,$1
mov $1,$0
equ $1,4
mul $1,11
add $2,$1
mov $1,$0
equ $1,5
mul $1,7
add $2,$1
mov $1,$0
equ $1,6
mul $1,11
add $2,$1
mov $1,$0
equ $1,7
mul $1,5
add $2,$1
mov $1,$0
equ $1,8
mul $1,11
add $2,$1
mov $1,$0
equ $1,9
mul $1,11
add $2,$1
mov $1,$0
equ $1,10
mul $1,11
add $2,$1
mov $1,$0
equ $1,11
mul $1,5
add $2,$1
mov $0,$2
