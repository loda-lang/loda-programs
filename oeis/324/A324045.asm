; A324045: a(n) = A000010(n) - A106316(n).
; Submitted by Christian Krause
; 1,0,0,1,0,2,0,2,5,0,0,0,0,-2,-4,6,0,-6,0,-8,-8,-6,0,-4,7,-8,17,12,0,-16,0,13,17,-12,15,-3,0,-14,19,6,0,6,0,8,-12,-18,0,4,9,-1,23,6,0,0,36,-8,25,-24,0,-32,0,-26,33,29,40,-10,0,2,29,-34,0,12,0,-32,37,0,40,-18,0,-24
; Formula: a(n) = truncate((-A000203(n+1)*truncate(A094471(n+1)/A000203(n+1))+A094471(n+1))/(n+1))*(n+1)+A000203(n+1)*truncate(A094471(n+1)/A000203(n+1))-A094471(n+1)+A039649(n)-1

mov $3,$0
add $3,1
mov $4,$0
add $4,1
seq $4,203 ; a(n) = sigma(n), the sum of the divisors of n. Also called sigma_1(n).
mov $1,$0
add $1,1
seq $1,94471 ; a(n) = Sum_{(n - k)|n, 0 <= k <= n} k.
mod $1,$4
mov $2,$1
mod $2,$3
seq $0,39649 ; a(n) = phi(n)+1.
sub $0,1
sub $0,$2
mov $1,$2
