; A384673: Expansion of (1+x) / (1-2*x-5*x^2+2*x^3).
; Submitted by Science United
; 1,3,11,35,119,391,1307,4331,14415,47871,159155,528835,1757703,5841271,19413387,64517723,214419839,712601519,2368266787,7870701491,26157533879,86932041639,288910349691,960165839819,3191019344815,10605047189343,35244859423123
; Formula: a(n) = e(n)+1, b(n) = 2*c(n-1)-b(n-1), b(3) = 14, b(2) = 2, b(1) = 2, b(0) = 0, c(n) = 4*c(n-3)+3*c(n-1)+2*b(n-1)-2*b(n-2)-2*b(n-3)-2*c(n-2), c(4) = 84, c(3) = 24, c(2) = 8, c(1) = 2, c(0) = 1, d(n) = 2*b(n-1)+d(n-1), d(3) = 8, d(2) = 4, d(1) = 0, d(0) = 0, e(n) = 2*b(n-1)+2*c(n-1)+d(n-1)+e(n-1), e(3) = 34, e(2) = 10, e(1) = 2, e(0) = 0

mov $2,1
lpb $0
  sub $0,1
  mul $2,2
  add $3,$1
  add $3,$1
  mul $1,-1
  add $1,$2
  add $2,$3
  add $4,$2
lpe
mov $0,$4
add $0,1
