...
; Formula: a(n) = truncate(truncate(b(2*floor(n/2)+1)/gcd(c(2*floor(n/2)+1),b(2*floor(n/2)+1)))/gcd(truncate(b(2*floor(n/2)+1)/gcd(c(2*floor(n/2)+1),b(2*floor(n/2)+1))),n+1)), b(n) = n*b(n-1), b(2) = 2, b(1) = 1, b(0) = 1, c(n) = -n*c(n-1)+b(n-1), c(2) = -1, c(1) = 1, c(0) = 0
