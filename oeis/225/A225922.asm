; A225922: a(n) is the least k such that f(a(n-1)+1) + ... + f(k) > f(a(n-2)+1) + ... + f(a(n-1)) for n > 1, where f(n) = 1/(n+7) and a(1) = 1.
; Submitted by Science United
; 1,16,58,176,507,1436,4043,11359,31890,89506,251193,704933,1978258,5551574,15579326,43720081,122691130,344306598,966223316,2711500429,7609249779,21353742568,59924740904,168166051476,471922288540,1324349620283,3716505787761,10429583743512
; Formula: a(n) = c(n)-7, b(n) = 2*b(n-1)+floor((c(n-1)+d(n-1))/c(n-1))+10, b(2) = 61, b(1) = 19, b(0) = 4, c(n) = b(n-1)+4, c(2) = 23, c(1) = 8, c(0) = 4, d(n) = (floor((c(n-1)+d(n-1))/c(n-1))+4)*(2*b(n-1)+floor((c(n-1)+d(n-1))/c(n-1))+8)+2*b(n-1)+floor((c(n-1)+d(n-1))/c(n-1))+8, d(2) = 1062, d(1) = 102, d(0) = 0

#offset 1

mov $1,4
mov $2,4
lpb $0
  sub $0,1
  add $1,2
  add $3,$2
  div $3,$2
  add $3,4
  mov $2,$1
  add $2,2
  mul $1,2
  add $1,$3
  mul $3,$1
  add $3,$1
  add $1,2
lpe
mov $0,$2
sub $0,7
