; A074047: a(n)=a(n-1)*a(n-2)*a(n-3)*(1/a(n-1)+1/a(n-2)+1/a(n-3)) starting with a(1)=a(2)=a(3)=1.
; Submitted by Penguin
; 1,1,1,3,7,31,331,12795,4642051,60935796571,283646808320375611,17285560913056915909539455163,4902995236325455290013100337511909917402705547
; Formula: a(n) = b(n-1)+1, a(3) = 3, a(2) = 1, a(1) = 1, a(0) = 1, b(n) = (a(n-1)+1)*(b(n-1)+1)+d(n-1)*c(n-1)-b(n-1)-2, b(3) = 6, b(2) = 2, b(1) = 0, b(0) = 0, c(n) = a(n-1), c(3) = 1, c(2) = 1, c(1) = 1, c(0) = 0, d(n) = (a(n-1)+1)*(b(n-1)+1)+d(n-1)*c(n-1), d(3) = 10, d(2) = 4, d(1) = 2, d(0) = 2

mov $3,1
mov $4,1
add $0,1
lpb $0
  sub $0,1
  add $1,1
  add $2,1
  mul $4,$3
  mov $3,$2
  mov $2,$1
  mul $1,$3
  add $1,$4
  sub $3,1
  mov $4,$1
  sub $1,$2
  sub $1,1
lpe
mov $0,$2
