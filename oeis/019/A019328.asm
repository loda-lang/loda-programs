; A019328: Cyclotomic polynomials at x=10.
; Submitted by LCB001
; 9,11,111,101,11111,91,1111111,10001,1001001,9091,11111111111,9901,1111111111111,909091,90090991,100000001,11111111111111111,999001,1111111111111111111,99009901,900900990991,9090909091,11111111111111111111111,99990001
; Formula: a(n) = c(n+1)/d(n+1), b(n) = (10*c(n-1)+9)*(b(n-1)/gcd(20*c(n-1)+18,b(n-1))), b(2) = -99, b(1) = -9, b(0) = -1, c(n) = 10*c(n-1)+9, c(2) = 99, c(1) = 9, c(0) = 0, d(n) = gcd(20*c(n-1)+18,b(n-1)), d(2) = 9, d(1) = 1, d(0) = 0

mov $1,-1
add $0,1
lpb $0
  sub $0,1
  mul $2,10
  add $2,9
  mov $3,2
  mul $3,$2
  gcd $3,$1
  div $1,$3
  mul $1,$2
lpe
div $2,$3
mov $0,$2
