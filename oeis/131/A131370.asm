; A131370: a(n) = 3a(n-1) - 3a(n-2) + 2a(n-3), a(0) = 3, a(1) = 2, a(2) = 0.
; Submitted by fzs600
; 3,2,0,0,4,12,24,44,84,168,340,684,1368,2732,5460,10920,21844,43692,87384,174764,349524,699048,1398100,2796204,5592408,11184812,22369620,44739240,89478484,178956972,357913944,715827884,1431655764,2863311528
; Formula: a(n) = b(2*n)/3, b(n) = d(n-2), b(5) = 0, b(4) = 0, b(3) = 6, b(2) = 6, b(1) = 9, b(0) = 9, c(n) = 2*c(n-2), c(5) = 12, c(4) = 12, c(3) = 6, c(2) = 6, c(1) = 3, c(0) = 3, d(n) = 2*c(n-4)-d(n-4)+d(n-2), d(5) = 0, d(4) = 0, d(3) = 0, d(2) = 0, d(1) = 6, d(0) = 6

mov $1,9
mov $2,3
mov $3,6
mul $0,2
lpb $0
  sub $0,2
  sub $3,$1
  add $1,$3
  add $3,$2
  mul $2,2
lpe
mov $0,$1
div $0,3
