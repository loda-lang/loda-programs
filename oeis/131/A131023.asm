; A131023: First subdiagonal of triangular array T: T(j,1) = 1 for ((j-1) mod 6) < 3, else 0; T(j,k) = T(j-1,k-1) + T(j-1,k) for 2 <= k <= j.
; Submitted by Jon Maiga
; 1,2,3,4,5,7,14,37,101,256,593,1267,2534,4825,8921,16384,30581,58975,117950,242461,504605,1048576,2156201,4371451,8742902,17308657,34085873,67108864,132623405,263652487,527304974,1059392917,2133134741
; Formula: a(n) = d(n)/2, b(n) = 3*b(n-1)-c(n-2)-3*b(n-2)+c(n-1), b(5) = 46, b(4) = 32, b(3) = 22, b(2) = 14, b(1) = 8, b(0) = 4, c(n) = 2*c(n-1)+1, c(3) = 15, c(2) = 7, c(1) = 3, c(0) = 1, d(n) = c(n-1)+e(n-1), d(3) = 8, d(2) = 6, d(1) = 4, d(0) = 2, e(n) = 2*e(n-1)-b(n-1)+c(n-1), e(3) = -5, e(2) = 1, e(1) = 3, e(0) = 3

mov $1,4
mov $2,1
mov $3,2
mov $4,3
lpb $0
  sub $0,1
  mov $3,$2
  add $3,$4
  mul $2,2
  add $2,1
  sub $4,$1
  add $4,$3
  add $1,$3
lpe
mov $0,$3
div $0,2
