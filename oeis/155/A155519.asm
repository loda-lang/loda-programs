; A155519: a(n) = Sum (J(p): p is a permutation of {1,2,...,n}), where J(p) is the number of j <= ceiling(n/2) such that p(j) + p(n+1-j) = n+1.
; Submitted by loader3229
; 1,2,4,16,72,432,2880,23040,201600,2016000,21772800,261273600,3353011200,46942156800,697426329600,11158821273600,188305108992000,3389491961856000,64023737057280000,1280474741145600000
; Formula: a(n) = d(n-1)*(c(n-1)+1)*(min(n-1,(n-1)%2)*(b(n-1)+1)+1), b(n) = b(n-2)+2, b(5) = 4, b(4) = 4, b(3) = 2, b(2) = 2, b(1) = 0, b(0) = 0, c(n) = c(n-2)+1, c(5) = 2, c(4) = 2, c(3) = 1, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = d(n-2)*(b(n-2)+1)*(b(n-2)+2), d(5) = 24, d(4) = 24, d(3) = 2, d(2) = 2, d(1) = 1, d(0) = 1

#offset 1

mov $3,1
sub $0,1
lpb $0
  sub $0,2
  add $1,1
  add $2,1
  mul $3,$1
  add $1,1
  mul $3,$1
lpe
add $1,1
mul $1,$0
add $1,1
add $2,1
mul $2,$3
mul $2,$1
mov $0,$2
