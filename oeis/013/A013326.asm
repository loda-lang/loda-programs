; A013326: Expansion of -(2*x^3-x^2+x-1)/(x^4-3*x^3+3*x^2-3*x+1).
; Submitted by yasiwo
; 1,2,4,7,14,31,68,146,313,674,1453,3130,6740,14515,31262,67331,145012,312314,672637,1448674,3120041,6719698,14472356,31169423,67130254,144579863,311384740,670635970,1444363025
; Formula: a(n) = a(n-1)+c(n-1)+1, a(3) = 7, a(2) = 4, a(1) = 2, a(0) = 1, b(n) = a(n-1)+b(n-1), b(3) = 7, b(2) = 3, b(1) = 1, b(0) = 0, c(n) = a(n-1)+b(n-1)+c(n-1)+d(n-1), c(3) = 6, c(2) = 2, c(1) = 1, c(0) = 0, d(n) = -c(n-1)-2, d(3) = -4, d(2) = -3, d(1) = -2, d(0) = 0

mov $1,1
lpb $0
  sub $0,1
  add $2,$1
  add $4,$2
  add $1,$3
  add $1,1
  add $3,$4
  sub $4,$3
  sub $4,2
lpe
mov $0,$1
