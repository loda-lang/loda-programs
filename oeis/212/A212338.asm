; A212338: Sequence of coefficients of x in marked mesh pattern generating function Q_{n,132}^(0,0,2,0)(x).
; Submitted by BrandyNOW
; 2,7,21,53,124,273,577,1181,2358,4614,8880,16854,31612,58691,108003,197203,357596,644463,1155059,2059897,3656988,6465660,11388480,19990140,34976870,61019071,106160481,184228193,318948124,550962717,949781269,1634103701,2806342578,4811193954,8234899632,14073292530,24016000192,40926633851,69653363343,118395981919,201009553724,340883137563,577465404839,977235169777,1652131130616,2790491010744,4708956149760,7939492148280,13375192305290,22514424728695,37869417607533,63649650756173,106904261219644
; Formula: a(n) = c(n-2), b(n) = 2*b(n-1)-b(n-4)-2*b(n-3)+b(n-2), b(7) = 180, b(6) = 96, b(5) = 50, b(4) = 25, b(3) = 12, b(2) = 5, b(1) = 2, b(0) = 0, c(n) = 2*b(n-1)-d(n-4)-e(n-4)-3*b(n-3)+b(n-5)+c(n-1)+c(n-2)+d(n-2)+e(n-3), c(9) = 2358, c(8) = 1181, c(7) = 577, c(6) = 273, c(5) = 124, c(4) = 53, c(3) = 21, c(2) = 7, c(1) = 2, c(0) = 0, d(n) = 2*d(n-1)-d(n-3)-f(n-3)+d(n-2)+f(n-1)+2, d(7) = 149, d(6) = 82, d(5) = 44, d(4) = 23, d(3) = 11, d(2) = 5, d(1) = 1, d(0) = 0, e(n) = b(n-1)+d(n-1)+e(n-1)+2, e(6) = 190, e(5) = 94, e(4) = 44, e(3) = 19, e(2) = 7, e(1) = 2, e(0) = 0, f(n) = -b(n-1), f(6) = -50, f(5) = -25, f(4) = -12, f(3) = -5, f(2) = -2, f(1) = 0, f(0) = 0

#offset 3

sub $0,2
lpb $0
  sub $0,1
  add $2,$4
  mov $6,$4
  add $6,$8
  add $6,1
  sub $4,$2
  sub $7,$1
  sub $7,$4
  mov $8,$4
  add $2,2
  mov $4,$1
  add $5,$2
  add $1,$3
  mov $3,$5
  add $3,$4
  add $4,$7
  add $4,$6
  mov $7,$6
lpe
mov $0,$3
