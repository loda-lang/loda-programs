; A220186: Numbers n >= 0 such that n^2 + n*(n+1)/2 is a square.
; Submitted by arkiss
; 0,8,800,78408,7683200,752875208,73774087200,7229107670408,708378777612800,69413891098384008,6801852948864020000,666512175097575576008,65311391306613542428800,6399849835873029582446408,627119972524250285537319200
; Formula: a(n) = c(2*n), b(n) = 8*d(n-1)-c(n-1)+b(n-1)+1, b(3) = 90, b(2) = 9, b(1) = 1, b(0) = 0, c(n) = 8*d(n-1)-c(n-1)+1, c(3) = 81, c(2) = 8, c(1) = 1, c(0) = 0, d(n) = 8*d(n-1)+2*e(n-1)-c(n-1)+b(n-1)+1, d(3) = 110, d(2) = 11, d(1) = 1, d(0) = 0, e(n) = 8*d(n-1)-c(n-1)+b(n-1)+e(n-1)+1, e(3) = 100, e(2) = 10, e(1) = 1, e(0) = 0

mul $0,2
lpb $0
  sub $0,1
  sub $2,1
  mul $3,8
  sub $3,$2
  mov $5,$4
  add $1,$3
  mov $2,$3
  add $4,$1
  add $5,$4
  mov $3,$5
lpe
mov $0,$2
