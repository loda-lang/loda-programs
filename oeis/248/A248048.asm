; A248048: Numerator of u(n) where u(n) = (u(n-1) + u(n-2)) * (u(n-2) + u(n-3)) / u(n-4) with u(0) = -1, u(1) = u(2) = u(3) = 1.
; Submitted by kotenok2000
; -1,1,1,1,-4,-6,30,-240,1260,35700,1256640,-199020360,-202839655480,1124753048683264,-181696576073176468304,1026657060420588391021488976,919637161132261232937508950440493056,839430326120023909391651548323223480699275649536
; Formula: a(n) = truncate((-e(n)*d(n))/2), b(n) = truncate((b(n-2)*(b(n-1)+b(n-3)))/b(n-4)), b(3) = -3, b(2) = -4, b(1) = -2, b(0) = 1, c(n) = b(n-1), c(3) = -4, c(2) = -2, c(1) = 1, c(0) = -2, d(n) = c(n-1), d(3) = -2, d(2) = 1, d(1) = -2, d(0) = 1, e(n) = d(n-1), e(3) = 1, e(2) = -2, e(1) = 1, e(0) = 2

mov $1,1
mov $2,-2
mov $3,1
mov $4,2
lpb $0
  sub $0,1
  mov $5,$1
  add $5,$3
  mul $5,$2
  div $5,$4
  mov $4,$3
  mov $3,$2
  mov $2,$1
  mov $1,$5
lpe
mul $3,$4
mov $0,$3
mul $0,-1
div $0,2
