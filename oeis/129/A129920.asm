; A129920: Expansion of -1/(1 - x + 3*x^2 - 2*x^3 + x^4 - 2*x^5 + x^6).
; Submitted by Jamie Morken(w3)
; -1,-1,2,3,-4,-10,5,29,2,-76,-45,178,212,-361,-750,565,2282,-306,-6206,-2428,15176,14353,-32719,-55104,57933,176234,-61524,-499047,-97429,1271400,921652,-2887641,-3948938,5590078,13380187,-7828378,-39536779,108416,104810904
; Formula: a(n) = b(n+1), b(n) = -d(n-2)-d(n-3)-3*b(n-2)+b(n-4)+c(n-2)+d(n-4)-1, b(7) = 5, b(6) = -10, b(5) = -4, b(4) = 3, b(3) = 2, b(2) = -1, b(1) = -1, b(0) = 0, c(n) = b(n-1), c(5) = 3, c(4) = 2, c(3) = -1, c(2) = -1, c(1) = 0, c(0) = 0, d(n) = 2*b(n-1)-c(n-1)+d(n-1), d(5) = 6, d(4) = 2, d(3) = -3, d(2) = -2, d(1) = 0, d(0) = 0

add $0,1
lpb $0
  sub $0,1
  sub $1,$3
  sub $1,$6
  sub $2,$5
  sub $4,$6
  add $6,$2
  mov $7,$4
  mov $4,$2
  sub $1,$2
  add $2,$1
  sub $2,1
  add $5,$4
  add $6,$5
  mov $1,$3
  mov $3,$5
  sub $3,$7
lpe
mov $0,$2
