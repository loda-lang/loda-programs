; A120752: Numbers k such that {rk} <= c, where r = (1/2)^(1/2), c = 1/2 and { } denotes fractional part.
; Submitted by Penguin
; 2,3,6,9,10,12,13,16,17,19,20,23,26,27,30,33,34,36,37,40,43,44,47,50,51,53,54,57,58,60,61,64,67,68,70,71,74,75,77,78,81,84,85,88,91,92,94,95,98,99,101,102,105,108,109,111,112,115,116,118,119,122,125,126,129,132
; Formula: a(n) = c(2*n), b(n) = b(n-1)==0, b(4) = 1, b(3) = 0, b(2) = 1, b(1) = 0, b(0) = 1, c(n) = -b(n-1)-2*truncate(truncate(((2*c(n-1)+2)*(3*d(n-1)-2*d(n-2)))/(4*d(n-1)-2*d(n-2)))/2)+c(n-1)+truncate(((2*c(n-1)+2)*(3*d(n-1)-2*d(n-2)))/(4*d(n-1)-2*d(n-2)))+1, c(5) = 4, c(4) = 3, c(3) = 2, c(2) = 2, c(1) = 0, c(0) = 0, d(n) = 4*d(n-1)-2*d(n-2), d(5) = 328, d(4) = 96, d(3) = 28, d(2) = 8, d(1) = 2, d(0) = 0

#offset 1

mov $1,1
mov $2,1
mul $0,2
lpb $0
  sub $0,1
  mul $1,2
  add $1,$3
  add $4,1
  add $5,$1
  mov $3,$4
  mul $3,2
  mul $3,$1
  div $3,$5
  mod $3,2
  sub $4,$2
  add $4,$3
  equ $2,0
  mov $3,$5
lpe
mov $0,$4
