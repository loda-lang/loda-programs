; A318946: Column 1 of triangle A318945.
; Submitted by Simon Strandgaard
; 0,0,0,1,5,19,64,201,603,1752,4973,13871,38176,103985,280947,754216,2014469,5358823,14209456,37580841,99188427,261360696,687777245,1808000351,4748806720,12464634209,32699621859,85747477576,224777691893,589072137367,1543445353168
; Formula: a(n) = c(max(n-2,0)), b(n) = b(n-1)+c(n-1), b(4) = 26, b(3) = 7, b(2) = 2, b(1) = 1, b(0) = 1, c(n) = d(n-1), c(4) = 64, c(3) = 19, c(2) = 5, c(1) = 1, c(0) = 0, d(n) = 6*d(n-1)+4*d(n-3)-11*d(n-2)+b(n-2)+c(n-2)-1, d(6) = 1752, d(5) = 603, d(4) = 201, d(3) = 64, d(2) = 19, d(1) = 5, d(0) = 1

mov $1,1
mov $5,1
sub $0,2
lpb $0
  sub $0,1
  add $1,$3
  sub $3,$4
  add $2,1
  sub $2,$3
  mov $3,$5
  add $4,$1
  add $4,$2
  mul $5,3
  add $5,1
  add $5,$2
lpe
mov $0,$3
