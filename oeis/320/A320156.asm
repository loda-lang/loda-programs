...
; Formula: a(n) = -10*truncate(truncate((d(max(3*n-3,0))+2)/truncate(c(max(3*n-3,0))/(10^(n-1))))/10)+truncate((d(max(3*n-3,0))+2)/truncate(c(max(3*n-3,0))/(10^(n-1)))), b(n) = 4*c(n-1)+2*b(n-1)+2*d(n-1), b(2) = 28, b(1) = 4, b(0) = 0, c(n) = 4*c(n-1)+b(n-1)+d(n-1), c(2) = 22, c(1) = 4, c(0) = 1, d(n) = 2*c(n-1)+d(n-1), d(2) = 10, d(1) = 2, d(0) = 0
