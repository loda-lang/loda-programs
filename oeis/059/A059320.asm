...
; Formula: a(n) = -sumdigits(c(n+1),2)*sign(c(n+1))+logint(c(n+1),2), b(n) = bitxor(b(n-1),2*c(n-1)), b(2) = 6, b(1) = 2, b(0) = 0, c(n) = bitxor(4*d(n-1),bitxor(b(n-1),2*c(n-1))), c(2) = 14, c(1) = 2, c(0) = 1, d(n) = bitxor(4*d(n-1),bitxor(b(n-1),2*c(n-1))), d(2) = 14, d(1) = 2, d(0) = 0
