; A303631: Number of n X 3 0..1 arrays with every element unequal to 2 or 3 horizontally or vertically adjacent elements, with upper left element zero.
; Submitted by Simon Strandgaard
; 0,1,0,1,0,3,2,9,12,33,56,129,244,523,1034,2153,4336,8921,18104,37065,75456,154171,314266,641569,1308492,2670345,5447424,11115417,22677188,46269875,94401330,192609073,392973504,801784337,1635862800,3337639217,6809729248,13893818515,28347355474,57836768921,118003569900,240761197009,491221873768,1002233681233,2044844229748,4172069219323,8512218355578,17367368601689,35434415861360,72296378054313,147505358913288,300953265268025,614031030354272,1252799523083275,2556070564650794,5215117517172081
; Formula: a(n) = f(n-1), b(n) = -d(n-1)-f(n-1)+c(n-1)+e(n-1)+e(n-3), b(5) = -1, b(4) = -4, b(3) = 1, b(2) = -2, b(1) = 1, b(0) = 0, c(n) = b(n-1)+e(n-1), c(5) = -3, c(4) = -2, c(3) = -1, c(2) = 0, c(1) = 1, c(0) = 0, d(n) = b(n-1), d(5) = -4, d(4) = 1, d(3) = -2, d(2) = 1, d(1) = 0, d(0) = 1, e(n) = -d(n-1)-f(n-1)+b(n-1)+c(n-1), e(5) = -7, e(4) = 1, e(3) = -3, e(2) = 1, e(1) = -1, e(0) = 1, f(n) = -c(n-1)+d(n-1)+f(n-1), f(5) = 3, f(4) = 0, f(3) = 1, f(2) = 0, f(1) = 1, f(0) = 0

#offset 1

mov $1,1
mov $4,1
mov $5,1
sub $0,1
lpb $0
  sub $0,1
  add $1,$5
  sub $3,$4
  mov $4,$2
  mov $2,$3
  add $2,$1
  sub $2,$6
  add $5,$4
  mov $1,$3
  mov $3,$5
  sub $6,$1
  mov $5,$4
  sub $5,$6
lpe
mov $0,$6
