...
; Formula: a(n) = d(n+1)+3, b(n) = (-c(n-1)+b(n-1)+1)/2, b(2) = -3, b(1) = 0, b(0) = 0, c(n) = gcd(((-((-c(n-1)+b(n-1)+1)/2)+d(n-1))/2)*((-c(n-1)+b(n-1)+1)/2),4)*c(n-1), c(2) = 32, c(1) = 8, c(0) = 2, d(n) = gcd(((-((-c(n-1)+b(n-1)+1)/2)+d(n-1))/2)*((-c(n-1)+b(n-1)+1)/2),4)/(-2), d(2) = -2, d(1) = -2, d(0) = 0
