; A106206: Coefficients of (1 + 144*x)^(1/24).
; Submitted by Jon Maiga
; 1,6,-414,38916,-4144554,472479156,-56225019564,6891580969416,-863170516419354,109910379090731076,-14178438902704308804,1848352853316179893176,-243058400211077655952644
; Formula: a(n) = b(2*n), b(n) = truncate((12*d(n-2)*b(n-2))/(c(n-2)-2)), b(5) = -414, b(4) = -414, b(3) = 6, b(2) = 6, b(1) = 1, b(0) = 1, c(n) = c(n-2)-2, c(5) = -4, c(4) = -4, c(3) = -2, c(2) = -2, c(1) = 0, c(0) = 0, d(n) = d(n-2)+24, d(5) = 47, d(4) = 47, d(3) = 23, d(2) = 23, d(1) = -1, d(0) = -1

mul $0,2
mov $1,1
mov $4,-1
mov $3,$0
lpb $3
  sub $3,2
  sub $2,2
  mul $1,$4
  mul $1,12
  div $1,$2
  add $4,24
lpe
mov $0,$1
