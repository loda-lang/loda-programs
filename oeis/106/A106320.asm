; A106320: Values b of a Bhaskara pair (a,b), a<=b, sorted on values of b. A Bhaskara pair (a,b) is such that a^2 + b^2 = X^3 and a^3 + b^3 = Y^2.
; Submitted by Bagoda Tes-X
; 2,128,1250,1458,8192,31250,80000,93312,235298,524288,911250,1062882,2000000
; Formula: a(n) = 2*d(n+1), b(n) = ((625*b(n-1)^6)==min(625*b(n-1)^6,c(n-1)^6))+b(n-1), b(2) = 1, b(1) = 1, b(0) = 0, c(n) = ((c(n-1)^6)==min(625*b(n-1)^6,c(n-1)^6))+c(n-1), c(2) = 2, c(1) = 1, c(0) = 0, d(n) = min(625*b(n-1)^6,c(n-1)^6), d(2) = 1, d(1) = 0, d(0) = 0

#offset 1

add $0,1
lpb $0
  sub $0,1
  mov $4,$2
  pow $4,6
  mov $3,$1
  pow $3,6
  mul $3,625
  mov $5,$3
  min $5,$4
  equ $3,$5
  equ $4,$5
  add $1,$3
  add $2,$4
lpe
mov $0,$5
mul $0,2
