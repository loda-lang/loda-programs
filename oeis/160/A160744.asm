; A160744: a(n) = A151566(n)*3.
; Submitted by Science United
; 0,3,6,12,18,24,30,42,54,60,66,78,90,102,114,138,162,168,174,186,198,210,222,246,270,282,294,318,342,366,390,438,486,492,498,510,522,534,546,570,594,606,618,642,666,690,714,762,810,822,834,858,882,906,930
; Formula: a(n) = 3*b(n), b(n) = if((2^2)==1,2^(sumdigits(truncate((n-1)/2),2)*sign(truncate((n-1)/2))),if((sumdigits(truncate((n-1)/2),2)*sign(truncate((n-1)/2)))<=(-1),0,2^(sumdigits(truncate((n-1)/2),2)*sign(truncate((n-1)/2)))))+b(n-1), b(0) = 0

lpb $0
  sub $0,1
  mov $2,$0
  div $2,2
  dgs $2,2
  mov $3,2
  pow $3,$2
  add $1,$3
lpe
mov $0,$1
mul $0,3
