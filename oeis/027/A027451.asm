; A027451: First diagonal of A027447.
; Submitted by DukeBox
; 1,1,4,9,144,100,3600,11025,78400,63504,6350400,5336100,768398400,662547600,577152576,2029052025,519437318400,463325262400,150117385017600,135480939978384,122885206329600,111967718990400,54192375991353600,49770428644836900
; Formula: a(n) = b(n)^2, b(n) = if(gcd(b(n-1),n)==0,c(n-1),if((c(n-1)%gcd(b(n-1),n))==0,c(n-1)/gcd(b(n-1),n),c(n-1))), b(2) = 1, b(1) = 1, b(0) = 0, c(n) = n*if(gcd(b(n-1),n)==0,c(n-1),if((c(n-1)%gcd(b(n-1),n))==0,c(n-1)/gcd(b(n-1),n),c(n-1))), c(2) = 2, c(1) = 1, c(0) = 1

#offset 1

mov $1,1
mov $3,1
lpb $0
  sub $0,1
  gcd $2,$1
  dif $3,$2
  mov $2,$3
  mul $3,$1
  add $1,1
lpe
pow $2,2
mov $0,$2
