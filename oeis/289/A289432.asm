; A289432: Numbers b_n of Fibonacci-quilt legal decompositions of n.
; Submitted by JZD
; 0,0,0,1,1,1,1,2,3,4,6,8,11,15,21,30,42,59,82,114,159,222,311,435,608,849,1185,1655,2312,3231,4515,6308,8812,12309,17195,24022,33561,46888,65505,91512,127843,178599,249509,348575,486975,680323,950434,1327786
; Formula: a(n) = b(n-5)+1, a(8) = 3, a(7) = 2, a(6) = 1, a(5) = 1, a(4) = 1, a(3) = 1, a(2) = 0, a(1) = 0, a(0) = 0, b(n) = b(n-1)+b(n-5)+b(n-7)+2, b(9) = 20, b(8) = 14, b(7) = 10, b(6) = 7, b(5) = 5, b(4) = 3, b(3) = 2, b(2) = 1, b(1) = 0, b(0) = 0

lpb $0
  sub $0,1
  mov $7,$6
  add $3,1
  add $4,1
  sub $4,$9
  mov $6,$4
  mov $4,$2
  mov $2,$1
  add $7,$4
  mov $1,$3
  mov $3,$8
  mov $8,$5
  mov $9,1
  add $5,$7
lpe
mov $0,$4
