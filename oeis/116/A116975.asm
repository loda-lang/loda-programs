; A116975: Number of compositions of n into parts of sizes == 1 mod 5 or 4 mod 5.
; Submitted by pututu
; 1,1,1,2,3,5,7,10,15,23,35,52,77,115,173,260,389,581,869,1302,1951,2921,4371,6542,9795,14667,21959,32872,49209,73671,110297,165128,247209,370089,554057,829482,1241819,1859117,2783263,4166802
; Formula: a(n) = b(n+1), b(n) = c(n-2)+1, b(4) = 2, b(3) = 1, b(2) = 1, b(1) = 1, b(0) = 0, c(n) = b(n-2)+d(n-2), c(4) = 4, c(3) = 2, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = 2*b(n-1)+d(n-1), d(4) = 7, d(3) = 5, d(2) = 3, d(1) = 1, d(0) = 1

mov $5,1
add $0,1
lpb $0
  sub $0,1
  mov $6,$2
  add $1,1
  mov $2,$1
  add $5,$6
  mov $1,$3
  mov $3,$4
  mov $4,$5
  add $5,$6
lpe
mov $0,$2
