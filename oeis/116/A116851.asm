; A116851: Number of permutations of length n which avoid the patterns 321, 31245.
; Submitted by Jamie Morken(w2)
; 1,2,5,14,41,116,307,760,1779,3986,8641,18282,38005,78024,158791,321236,647247,1300630,2609029,5227766,10467521,20949692,41917115,83855504,167736331,335502586,671040297,1342121570,2684290669,5368636176,10737335311,21474742572
; Formula: a(n) = b(n-1)+1, b(n) = b(n-1)+d(n-1)+e(n-1), b(4) = 40, b(3) = 13, b(2) = 4, b(1) = 1, b(0) = 0, c(n) = c(n-1)+n-1, c(4) = 6, c(3) = 3, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = 2*c(n-1)+2*d(n-1)+2*n-2, d(4) = 48, d(3) = 18, d(2) = 6, d(1) = 2, d(0) = 1, e(n) = d(n-1)+e(n-1), e(4) = 27, e(3) = 9, e(2) = 3, e(1) = 1, e(0) = 0

#offset 1

mov $3,1
sub $0,1
lpb $0
  sub $0,1
  add $2,$5
  add $4,$3
  add $1,$4
  add $3,$2
  mul $3,2
  add $5,1
lpe
mov $0,$1
add $0,1
