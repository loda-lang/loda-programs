; A349089: a(n) = n! * Sum_{k=0..floor((n-1)/4)} 1 / (4*k+1)!.
; Submitted by Jamie Morken(w1)
; 0,1,2,6,24,121,726,5082,40656,365905,3659050,40249550,482994600,6278929801,87905017214,1318575258210,21097204131360,358652470233121,6455744464196178,122659144819727382,2453182896394547640,51516840824285500441,1133370498134281009702
; Formula: a(n) = truncate((b(n-1)-1)^(n-1))*truncate((-truncate((b(n-1)-1)^(n-1))+c(n-1))^(n-1))+n*a(n-1), a(3) = 6, a(2) = 2, a(1) = 1, a(0) = 0, b(n) = truncate((b(n-1)-1)^(n-1)), b(3) = 1, b(2) = 0, b(1) = 1, b(0) = 0, c(n) = truncate((-truncate((b(n-1)-1)^(n-1))+c(n-1))^(n-1)), c(3) = 0, c(2) = 1, c(1) = 1, c(0) = 0

lpb $0
  sub $0,1
  sub $2,1
  pow $2,$1
  mov $3,$4
  sub $3,$2
  pow $3,$1
  mov $4,$3
  add $1,1
  mul $3,$2
  mul $5,$1
  add $5,$3
lpe
mov $0,$5
