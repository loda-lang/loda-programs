; A048755: Partial sums of A048693.
; Submitted by VWS
; 1,7,20,52,129,315,764,1848,4465,10783,26036,62860,151761,366387,884540,2135472,5155489,12446455,30048404,72543268,175134945,422813163,1020761276,2464335720,5949432721,14363201167,34675835060,83714871292,202105577649,487926026595
; Formula: a(n) = (d(n+2)-7)/2+1, b(n) = 2*b(n-1)+b(n-2), b(4) = 32, b(3) = 13, b(2) = 6, b(1) = 1, b(0) = 0, c(n) = 2*c(n-1)+c(n-2), c(3) = 13, c(2) = 6, c(1) = 1, c(0) = 4, d(n) = b(n-1)+c(n-1)+d(n-1), d(2) = 7, d(1) = 5, d(0) = 1

mov $2,4
mov $3,1
add $0,2
lpb $0
  sub $0,1
  add $1,$3
  add $2,$1
  mov $3,$2
  mov $2,$1
lpe
mov $0,$3
sub $0,7
div $0,2
add $0,1
