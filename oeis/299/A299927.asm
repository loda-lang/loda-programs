...
; Formula: a(n) = ((-binomial(floor((sqrtint(8*max(n-1,0))+1)/2),2)+max(n-1,0))==1)+binomial(floor((sqrtint(8*max(n-1,0))+1)/2),-binomial(floor((sqrtint(8*max(n-1,0))+1)/2),2)+max(n-1,0))
