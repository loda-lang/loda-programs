...
; Formula: a(n) = d(n)+1, b(n) = 2*d(n-1)-max(c(n-1),6)+gcd(b(n-1),2)+1, b(3) = 3, b(2) = -2, b(1) = -3, b(0) = 0, c(n) = 2*d(n-1)-max(c(n-1),6)+1, c(3) = 1, c(2) = -3, c(1) = -5, c(0) = 0, d(n) = 2*d(n-2)-max(c(n-2),6)+gcd(b(n-2),2)+max(c(n-2),6)+1, d(3) = 4, d(2) = 3, d(1) = 1, d(0) = 0
