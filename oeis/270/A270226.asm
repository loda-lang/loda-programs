...
; Formula: a(n) = d(n)+1, b(n) = truncate((-c(n-1)+b(n-1))/2), b(2) = -4, b(1) = -1, b(0) = 0, c(n) = 2*gcd(truncate((-c(n-1)+b(n-1))/2)+1,2)*c(n-1), c(2) = 16, c(1) = 8, c(0) = 2, d(n) = gcd(truncate((-c(n-1)+b(n-1))/2)+1,2), d(2) = 1, d(1) = 2, d(0) = 0
