; A270693: Alternating sum of centered 25-gonal numbers.
; Submitted by Loadie
; 1,-25,51,-100,151,-225,301,-400,501,-625,751,-900,1051,-1225,1401,-1600,1801,-2025,2251,-2500,2751,-3025,3301,-3600,3901,-4225,4551,-4900,5251,-5625,6001,-6400,6801,-7225,7651,-8100,8551,-9025,9501,-10000,10501
; Formula: a(n) = d(n)+1, b(n) = -2*c(n-1)+c(n-1)-26, b(2) = 25, b(1) = -26, b(0) = 0, c(n) = -2*c(n-3)-4*c(n-1)-4*c(n-2)+c(n-1)+c(n-2)+c(n-3)-129, c(2) = 50, c(1) = -51, c(0) = 0, d(n) = -2*c(n-1)+b(n-1)-26, d(2) = 50, d(1) = -26, d(0) = 0

lpb $0
  sub $0,1
  mov $1,26
  add $1,$3
  add $1,$3
  sub $3,$1
  add $4,1
  mov $5,$4
  mov $4,$2
  sub $4,$1
  add $5,$3
  add $5,$4
  mov $2,$3
  mov $3,$5
lpe
mov $0,$4
add $0,1
