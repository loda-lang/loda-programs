...
; Formula: a(n) = d(n+4), b(n) = truncate((-c(n-1)+b(n-1))/2), b(2) = 10, b(1) = 4, b(0) = 0, c(n) = gcd(d(n-1)+truncate((-c(n-1)+b(n-1))/2),2)*c(n-1), c(2) = -32, c(1) = -16, c(0) = -8, d(n) = gcd(d(n-1)+truncate((-c(n-1)+b(n-1))/2),2), d(2) = 2, d(1) = 2, d(0) = 0
