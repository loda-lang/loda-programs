; A086291: Maximal exponent in prime factorization of 7-smooth numbers.
; Submitted by fzs600
; 0,1,1,2,1,1,1,3,2,1,2,1,1,4,2,2,1,3,2,3,2,1,5,1,2,3,1,2,4,2,2,3,3,2,2,6,1,3,2,4,4,2,2,5,2,2,1,3,4,3,3,2,7,3,2,4,2,2,5,4,3,2,2,3,6,2,3,1,3,5,2,4,5,2,3,2,8,3,3,5
; Formula: a(n) = A067132(A002473(n+1)-1)-1

add $0,1
seq $0,2473 ; 7-smooth numbers: positive numbers whose prime divisors are all <= 7.
sub $0,1
seq $0,67132 ; Number of elements in the largest set of divisors of n which are in geometric progression.
sub $0,1
