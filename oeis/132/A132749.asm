; A132749: Triangle T(n,k) = binomial(n, k) with T(n, 0) = 2, read by rows.
; Submitted by Joe
; 1,2,1,2,2,1,2,3,3,1,2,4,6,4,1,2,5,10,10,5,1,2,6,15,20,15,6,1,2,7,21,35,35,21,7,1,2,8,28,56,70,56,28,8,1,2,9,36,84,126,126,84,36,9,1,2,10,45,120,210,252,210,120,45,10,1,2,11,55,165,330,462,462,330,165,55,11,1,2,12
; Formula: a(n) = binomial(truncate((sqrtint(8*n+8)-1)/2),-binomial(truncate((sqrtint(8*n+8)-1)/2)+1,2)+n)+binomial(0,-binomial(truncate((sqrtint(8*n+8)-1)/2)+1,2)+n)+min(truncate((sqrtint(8*n+8)-1)/2),1)-1

add $0,1
mov $2,$0
mul $2,8
nrt $2,2
sub $2,1
div $2,2
mov $1,$2
add $1,1
bin $1,2
sub $0,$1
sub $0,1
mov $1,$2
bin $1,$0
min $2,1
bin $3,$0
add $3,$1
add $3,$2
mov $0,$3
sub $0,1
