...
; Formula: a(n) = truncate((A293838((max(n-1,0)+1)%2+1)*floor(binomial(2*floor(max(n-1,0)/2),floor(max(n-1,0)/2))/(floor(max(n-1,0)/2)+1))+floor(binomial(2*max(n-1,0),max(n-1,0))/(max(n-1,0)+1)))/2)
