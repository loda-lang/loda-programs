...
; Formula: a(n) = truncate(((6*binomial(floor((sqrtint(8*n)+1)/2),2)+6*floor((sqrtint(8*n)+1)/2)-6*n+5)*if((-binomial(floor((sqrtint(8*n)+1)/2),2)+n)<=(-1),0,4^(-binomial(floor((sqrtint(8*n)+1)/2),2)+n)))/6)
