...
; Formula: a(n) = -2*truncate((-2*truncate(b(max(n-1,0))/2)+b(max(n-1,0))+2)/2)-2*truncate(b(max(n-1,0))/2)+b(max(n-1,0))+2, b(n) = truncate(truncate((-c(n-1)+b(n-1))/2)/gcd(truncate((-c(n-1)+b(n-1))/2)+1,4)), b(1) = -1, b(0) = 0, c(n) = 4*c(n-1), c(1) = 40, c(0) = 10
