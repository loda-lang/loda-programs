; A284840: Number of quinternary strings avoiding consecutive digits i,i+1 and i,i+2.
; Submitted by Jamie Morken(w4)
; 1,5,18,63,224,799,2846,10132,36073,128438,457304,1628224,5797263,20641062,73492179,261667749,931663897,3317174627,11810748015,42051982296,149725420655,533095002085,1898076358760,6758070980980,24062005289281,85672390859855,305035198330433,1086073019401288,3866945880106824,13768199902358574,49021458905463096,174540132352984047,621447392264414323,2212653652463202008,7878118480664025249,28049916771333569634,99871286882756199923,355590143989706618452,1266073107189051325634,4507833357703356075469
; Formula: a(n) = e(n+1), b(n) = -e(n-1)+b(n-1)+d(n-1)+1, b(5) = 179, b(4) = 50, b(3) = 14, b(2) = 4, b(1) = 1, b(0) = 0, c(n) = -e(n-1)-f(n-1)+d(n-1), c(5) = -63, c(4) = -18, c(3) = -5, c(2) = -1, c(1) = 0, c(0) = 0, d(n) = 2*f(n-1)-e(n-1)+b(n-1)+c(n-1)+d(n-1)+e(n-1)+f1(n-1)+3, d(5) = 682, d(4) = 191, d(3) = 53, d(2) = 14, d(1) = 3, d(0) = 0, e(n) = -e(n-1)+b(n-1)+c(n-1)+d(n-1)+e(n-1)+1, e(5) = 224, e(4) = 63, e(3) = 18, e(2) = 5, e(1) = 1, e(0) = 0, f(n) = 2*f(n-1)-e(n-1)+b(n-1)+c(n-1)+d(n-1)+e(n-1)+f1(n-1)+3, f(5) = 682, f(4) = 191, f(3) = 53, f(2) = 14, f(1) = 3, f(0) = 0, f1(n) = f(n-1)+f1(n-1)+1, f1(5) = 266, f1(4) = 74, f1(3) = 20, f1(2) = 5, f1(1) = 1, f1(0) = 0

add $0,1
lpb $0
  sub $0,1
  sub $3,$4
  add $3,1
  add $5,1
  add $1,$3
  add $4,$2
  add $4,$1
  add $6,$5
  sub $3,$5
  add $5,$6
  add $5,$4
  mov $2,$3
  mov $3,$5
lpe
mov $0,$4
