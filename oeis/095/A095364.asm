; A095364: Number of walks of length n between two adjacent nodes in the cycle graph C_9.
; Submitted by Jamie Morken(l1)
; 1,0,3,0,10,0,35,1,126,11,462,78,1716,455,6435,2380,24311,11628,92398,54264,352947,245157,1354102,1081575,5215250,4686826,20156580,20030039,78152535,84672780,303906051,354822776,1184959314,1476390160
; Formula: a(n) = 2*c(n-1)-2*d(n-1)+b(n-1)+a(n-1), a(5) = 10, a(4) = 0, a(3) = 3, a(2) = 0, a(1) = 1, a(0) = 0, b(n) = -d(n-3)+b(n-1)+b(n-2)+c(n-3)+d(n-1)-1, b(6) = 7, b(5) = 2, b(4) = 2, b(3) = 1, b(2) = 1, b(1) = 1, b(0) = 1, c(n) = a(n-1), c(5) = 0, c(4) = 3, c(3) = 0, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = -d(n-1)+b(n-1)+c(n-1), d(5) = 6, d(4) = -1, d(3) = 2, d(2) = 0, d(1) = 1, d(0) = 0

#offset 1

mov $1,1
lpb $0
  sub $0,1
  sub $2,$5
  sub $4,1
  mov $5,$1
  add $5,$2
  add $1,$3
  mov $3,$4
  add $3,$5
  mov $4,$2
  mov $2,$6
  add $6,$5
  add $6,$4
  add $4,$1
lpe
mov $0,$6
