...
; 9,8,2,0,9,2,7,5,1,6,4,7,9,8,2,6,7,2,7,7,8,9,5,0,5,0,2,9,2,3,4,0,1,4,4,3,4,5,1,1,6,1,0,2,4,5,6,7,3,2,5,0,5,0,8,1,7,1,3,8,7,0,6,9,3,8,0,0,8,6,6,5,5,9,8,6,8,5,4,4
; Formula: a(n) = -10*truncate(truncate((2*b(4*n+4)-truncate(d(4*n+4)/3))/(3*truncate(c(4*n+4)/(10^(n+1)))))/10)+truncate((2*b(4*n+4)-truncate(d(4*n+4)/3))/(3*truncate(c(4*n+4)/(10^(n+1))))), b(n) = 4*b(n-1)+4*max(e(n-1),c(n-1)), b(3) = 160, b(2) = 24, b(1) = 4, b(0) = 1, c(n) = 2*b(n-1)+2*c(n-1)+2*max(e(n-1),c(n-1)), c(3) = 112, c(2) = 16, c(1) = 2, c(0) = 0, d(n) = 2*d(n-1)-b(n-1)-max(e(n-1),c(n-1))-2*e(n-1)+c(n-1), d(3) = -40, d(2) = -6, d(1) = -1, d(0) = 0, e(n) = max(e(n-1),c(n-1)), e(3) = 16, e(2) = 2, e(1) = 0, e(0) = 0
