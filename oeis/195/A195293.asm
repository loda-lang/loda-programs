...
; Formula: a(n) = -10*truncate(truncate((6*d(max(4*n-5,0))+6)/truncate(c(max(4*n-5,0))/(10^(n-1))))/10)+truncate((6*d(max(4*n-5,0))+6)/truncate(c(max(4*n-5,0))/(10^(n-1)))), b(n) = 4*c(n-1)+2*b(n-1)+2*d(n-1), b(2) = 18, b(1) = 4, b(0) = 0, c(n) = 2*c(n-1)+b(n-1)+d(n-1)+truncate(c(n-1)/2), c(2) = 10, c(1) = 2, c(0) = 1, d(n) = c(n-1)+d(n-1), d(2) = 3, d(1) = 1, d(0) = 0
