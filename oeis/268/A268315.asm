; A268315: Decimal expansion of 256/27.
; 9,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4,8,1,4
; Formula: a(n) = -truncate((4*n+truncate((-2*n-1)/3)+2)/(4*n+2))*(4*n+2)+4*n-10*truncate((-truncate((4*n+truncate((-2*n-1)/3)+2)/(4*n+2))*(4*n+2)+4*n+truncate((-2*n-1)/3)+11)/10)+truncate((-2*n-1)/3)+11

mul $0,2
add $0,1
sub $1,$0
mul $0,2
div $1,3
add $1,$0
mod $1,$0
add $1,9
mod $1,10
mov $0,$1
