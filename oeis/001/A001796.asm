; A001796: Coefficients of Legendre polynomials.
; Submitted by Aurum
; 1,3,27,143,3315,20349,260015,1710855,92116035,631165425,8775943605,61750730457,1755702867191,12587970424449,181858466731095,1322239639929719,154702037871777123,1137023085979691001,16789716964765636633
; Formula: a(n) = if(floor(floor((114*binomial(4*n+2,2*n+1))/(2*n+3))/19)==0,0,floor(floor((114*binomial(4*n+2,2*n+1))/(2*n+3))/19)/(2^valuation(floor(floor((114*binomial(4*n+2,2*n+1))/(2*n+3))/19),2)))

mul $0,2
add $0,1
mov $1,$0
mul $0,2
bin $0,$1
mul $0,114
add $1,2
div $0,$1
div $0,19
dir $0,2
