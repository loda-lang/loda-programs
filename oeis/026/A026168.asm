...
; Formula: a(n) = -max(n-1,0)+d(max(max(n-1,0)+1,0))+n+1, b(n) = (-c(n-1)+b(n-1))/2, b(3) = -9, b(2) = -2, b(1) = -1, b(0) = 0, c(n) = 2*gcd((-c(n-1)+b(n-1))/2,4)*c(n-1), c(3) = 32, c(2) = 16, c(1) = 4, c(0) = 2, d(n) = (gcd((-c(n-2)+b(n-2))/2,4)+2)/2+d(n-1), d(3) = 3, d(2) = 1, d(1) = 0, d(0) = 0
