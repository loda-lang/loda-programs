; A026278: a(n) = greatest k such that s(k) = n, where s = A026276.
; Submitted by [DPC] hansR
; 4,6,8,11,14,17,19,22,24,27,29,32,35,37,40,43,45,48,51,53,56,58,61,64,66,69,71,74,77,79,82,84,87,90,92,95,98,100,103,105,108,111,113,116,119,121,124,126,129,132,134,137,140,142,145
; Formula: a(n) = d(n-1)+a(n-1), a(3) = 11, a(2) = 8, a(1) = 6, a(0) = 4, b(n) = (-c(n-1)+b(n-1))/2-2, b(3) = -15, b(2) = -11, b(1) = -10, b(0) = -8, c(n) = c(n-1)*(((-c(n-1)+b(n-1))/2+d(n-1)/2)%2+2), c(3) = 32, c(2) = 16, c(1) = 8, c(0) = 8, d(n) = ((-c(n-1)+b(n-1))/2+d(n-1)/2)%2+3, d(3) = 3, d(2) = 3, d(1) = 2, d(0) = 2

mov $1,2
mov $2,8
mov $4,2
add $0,2
lpb $0
  sub $0,1
  sub $1,$2
  div $1,2
  add $4,$3
  div $3,2
  add $3,$1
  mod $3,2
  add $3,2
  sub $1,2
  mul $2,$3
  add $3,1
lpe
mov $0,$4
