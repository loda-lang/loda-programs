; A056016: a(n) = -2*a(n - 1) -a(n - 2) -a(n - 3), a(0) = a(1) = a(2) = 1.
; Submitted by Christian Krause
; 1,1,1,-4,6,-9,16,-29,51,-89,156,-274,481,-844,1481,-2599,4561,-8004,14046,-24649,43256,-75909,133211,-233769,410236,-719914,1263361,-2217044,3890641,-6827599,11981601,-21026244,36898486,-64752329,113632416,-199410989,349941891,-614105209
; Formula: a(n) = c(n-1)+1, a(3) = -4, a(2) = 1, a(1) = 1, a(0) = 1, b(n) = -b(n-1)+a(n-1)+d(n-1)+4, b(3) = 10, b(2) = -5, b(1) = 5, b(0) = 0, c(n) = -c(n-1)+d(n-1), c(3) = 5, c(2) = -5, c(1) = 0, c(0) = 0, d(n) = -a(n-1)-c(n-1)-d(n-1)+b(n-1)+d(n-1)-4, d(3) = -5, d(2) = 0, d(1) = -5, d(0) = 0

mov $2,1
lpb $0
  sub $0,1
  sub $2,$1
  add $2,$4
  add $2,4
  sub $4,$3
  mov $1,$2
  mov $2,$3
  add $2,1
  mov $3,$4
  sub $4,$1
lpe
mov $0,$2
