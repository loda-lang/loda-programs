; A052963: a(0)=1, a(1)=2, a(2)=5, a(n) = 3*a(n+2) - a(n+3).
; Submitted by F14Claude
; 1,2,5,14,40,115,331,953,2744,7901,22750,65506,188617,543101,1563797,4502774,12965221,37331866,107492824,309513251,891207887,2566130837,7388879260,21275429893,61260158842,176391597266,507899361905
; Formula: a(n) = 2*a(n-1)+b(n-1)+d(n-1), a(3) = 14, a(2) = 5, a(1) = 2, a(0) = 1, b(n) = c(n-1), b(3) = 3, b(2) = 1, b(1) = 0, b(0) = 0, c(n) = b(n-1)+a(n-1)+d(n-1), c(3) = 9, c(2) = 3, c(1) = 1, c(0) = 0, d(n) = b(n-1)+a(n-1)+d(n-1), d(3) = 9, d(2) = 3, d(1) = 1, d(0) = 0

mov $2,1
lpb $0
  sub $0,1
  add $4,$1
  add $4,$2
  mov $1,$3
  add $2,$4
  mov $3,$4
lpe
mov $0,$2
