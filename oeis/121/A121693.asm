; A121693: Number of deco polyominoes of height n and vertical height 3 (i.e., having 3 rows).
; Submitted by PDW
; 0,0,1,12,57,216,741,2412,7617,23616,72381,220212,666777,2012616,6062421,18236412,54807537,164619216,494250861,1483539012,4452189897,13359715416,40085437701,120268896012,360831853857,1082545893216
; Formula: a(n) = 2*b(n-2)+a(n-1)+1, a(3) = 12, a(2) = 1, a(1) = 0, a(0) = 0, b(n) = 3*b(n-1)+2*max(c(n-1),1)-2*max(c(n-2),1)+1, b(4) = 262, b(3) = 79, b(2) = 22, b(1) = 5, b(0) = 0, c(n) = 2*max(c(n-1),1)+2, c(3) = 22, c(2) = 10, c(1) = 4, c(0) = 0

lpb $0
  sub $0,1
  add $1,$3
  mul $2,2
  mov $3,1
  add $3,$2
  max $4,1
  add $4,1
  mul $4,2
  mov $2,$1
  add $2,$4
  add $2,$3
lpe
mov $0,$1
