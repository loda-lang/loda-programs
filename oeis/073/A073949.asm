; A073949: Number of strings over Z_3 of length n with trace 0 and subtrace 2.
; Submitted by loader3229
; 0,2,6,12,30,90,252,702,2106,6480,19602,58806,176904,532170,1596510,4785156,14351094,43053282,129146724,387400806,1162202418,3486725352,10460294154,31380882462,94143001680,282430067922,847290203766,2541867422652,7625599079310
; Formula: a(n) = b(n-1), b(n) = 27*b(n-3)+27*b(n-5)+6*b(n-1)-15*b(n-2)-36*b(n-4), b(11) = 58806, b(10) = 19602, b(9) = 6480, b(8) = 2106, b(7) = 702, b(6) = 252, b(5) = 90, b(4) = 30, b(3) = 12, b(2) = 6, b(1) = 2, b(0) = 0

#offset 1

mov $2,2
mov $3,6
mov $4,12
mov $5,30
sub $0,1
lpb $0
  mul $1,27
  rol $1,5
  mov $6,$1
  mul $6,-36
  add $5,$6
  mov $6,$2
  mul $6,27
  add $5,$6
  mov $6,$3
  mul $6,-15
  add $5,$6
  mov $6,$4
  mul $6,6
  sub $0,1
  add $5,$6
lpe
mov $0,$1
