; A073845: a(1)=a(2)=1, a(n+2)=a(n+1)+a(n)+(-2)^n.
; Submitted by Simon Strandgaard
; 1,1,0,5,-3,18,-17,65,-80,241,-351,914,-1485,3525,-6152,13757,-25163,54130,-102105,214169,-412224,850521,-1658855,3385970,-6661493,13501693,-26714232,53896325,-107035635,215296146,-428610401,860427569,-1715666480,3439728385,-6865872687
; Formula: a(n) = c(n+1), b(n) = -2*b(n-1), b(3) = -8, b(2) = 4, b(1) = -2, b(0) = 1, c(n) = d(n-1), c(3) = 0, c(2) = 1, c(1) = 1, c(0) = 0, d(n) = b(n-1)+c(n-1)+e(n-1), d(3) = 5, d(2) = 0, d(1) = 1, d(0) = 1, e(n) = b(n-1)+c(n-1)+e(n-1), e(3) = 5, e(2) = 0, e(1) = 1, e(0) = 0

mov $1,1
mov $3,1
add $0,1
lpb $0
  sub $0,1
  mul $1,-1
  add $4,$2
  sub $4,$1
  mul $1,2
  mov $2,$3
  mov $3,$4
lpe
mov $0,$2
