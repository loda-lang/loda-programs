; A073496: Expansion of (3 + 2*x + 3*x^2)/(1 + x + 3*x^2 - x^3).
; Submitted by USTL-FIL (Lille Fr)
; 3,-1,-5,11,3,-41,43,83,-253,47,795,-1189,-1149,5511,-3253,-14429,29699,10335,-113861,112555,239363,-690889,85355,2226675,-3173629,-3421041,15168603,-8079109,-40847741,80253671,34210443,-315819197,293441539,688226495,-1884370309,113132363,6228205059
; Formula: a(n) = c(2*n), b(n) = 2*d(n-1)-b(n-1)-d(n-1)+b(n-1)+c(n-1), b(2) = 0, b(1) = 4, b(0) = -2, c(n) = -b(n-1)-c(n-1)-2*d(n-1)+b(n-1)+c(n-1)+d(n-1), c(2) = -1, c(1) = -1, c(0) = 3, d(n) = 2*d(n-1)-c(n-1)-d(n-1)-2*b(n-1)-2*d(n-1)+b(n-1)+c(n-1), d(2) = -5, d(1) = 1, d(0) = 1

mov $1,-2
mov $2,3
mov $3,1
mul $0,2
lpb $0
  sub $0,1
  add $1,$3
  sub $3,$1
  add $1,$3
  add $1,$2
  sub $2,$1
  add $3,$2
lpe
mov $0,$2
