...
; Formula: a(n) = floor((truncate(b(max(6*n-5,0))/2)^2)/56), b(n) = truncate((gcd(-max(n-1,1)+c(n-1),2)*((truncate(3^(gcd(n-1,6)-3))+2)%10+truncate(3^(gcd(n-1,6)-3))))/2)*b(n-1)+b(n-2), b(3) = 56, b(2) = 19, b(1) = 18, b(0) = 1, c(n) = gcd(-max(n-1,1)+c(n-1),2)*((truncate(3^(gcd(n-1,6)-3))+2)%10+truncate(3^(gcd(n-1,6)-3))), c(3) = 4, c(2) = 2, c(1) = 36, c(0) = 0
