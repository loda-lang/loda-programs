; A203006: (n-1)-st elementary symmetric function of the first n Fibonacci numbers.
; Submitted by Simon Strandgaard
; 1,2,5,17,91,758,10094,215094,7378716,408057060,36439600740,5258207000160,1226732478115680,462844011818878560,282472779283129656000,278884771717353348456000,445462025196173918554440000,1151206495594319717393795136000
; Formula: a(n) = a(n-1)*(b(n-1)+d(n-1))+c(n-1), a(3) = 17, a(2) = 5, a(1) = 2, a(0) = 1, b(n) = b(n-1)+b(n-2), b(3) = 3, b(2) = 2, b(1) = 1, b(0) = 1, c(n) = c(n-1)*(2*d(n-1)+d(n-2)), c(4) = 30, c(3) = 6, c(2) = 2, c(1) = 1, c(0) = 1, d(n) = d(n-1)+d(n-2), d(4) = 3, d(3) = 2, d(2) = 1, d(1) = 1, d(0) = 0

mov $1,1
mov $2,1
mov $4,1
lpb $0
  sub $0,1
  mov $5,$1
  add $1,$3
  mov $3,$4
  mul $3,$1
  mov $4,$2
  add $4,$3
  mul $2,$1
  mov $3,$5
lpe
mov $0,$4
