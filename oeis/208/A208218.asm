; A208218: a(n)=(a(n-1)^2*a(n-3)+a(n-2))/a(n-4) with a(0)=a(1)=a(2)=a(3)=1.
; Submitted by biodoc
; 1,1,1,1,2,5,27,1463,5350936,154615586811211,1295349936263652139582251464117,6137049788665571444030885529267632764941063995324839557922175605,1088285417126782519841756661868715065176513281545210892526495014245555455304628406710217516050342166505309959867449375443155044120307447
; Formula: a(n) = e(max(n-3,0)), b(n) = (b(n-3)*e(n-2)^2+b(n-2))/b(n-4), b(4) = 27, b(3) = 5, b(2) = 2, b(1) = 1, b(0) = 1, c(n) = b(n-1), c(4) = 5, c(3) = 2, c(2) = 1, c(1) = 1, c(0) = 1, d(n) = c(n-1), d(4) = 2, d(3) = 1, d(2) = 1, d(1) = 1, d(0) = 1, e(n) = (c(n-1)*e(n-1)^2+b(n-1))/d(n-1), e(4) = 1463, e(3) = 27, e(2) = 5, e(1) = 2, e(0) = 1

mov $1,1
mov $2,1
mov $3,1
mov $4,1
mov $5,1
sub $0,3
lpb $0
  sub $0,1
  pow $5,2
  mul $5,$3
  add $5,$2
  div $5,$4
  mov $4,$3
  mov $3,$2
  mov $2,$1
  mov $1,$5
lpe
mov $0,$5
