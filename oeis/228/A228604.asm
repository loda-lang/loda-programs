; A228604: The Merrifield-Simmons index of the ortho-polyphenylene chain of length n.
; Submitted by Science United
; 1,18,299,4932,81301,1340118,22089599,364109832,6001737001,98928520218,1630669938899,26878845894732,443052477632701,7302973450020318,120377210159548199,1984215446621359632,32706447785195768401,539110673967989840418,8886330936793922917499
; Formula: a(n) = b(n-1)+d(n-1), a(3) = 4932, a(2) = 299, a(1) = 18, a(0) = 1, b(n) = 8*b(n-1)+8*d(n-1), b(3) = 39456, b(2) = 2392, b(1) = 144, b(0) = 8, c(n) = -d(n-1)+b(n-1)+c(n-1)+d(n-1), c(3) = 2543, c(2) = 151, c(1) = 7, c(0) = -1, d(n) = 8*b(n-1)+8*d(n-1)-c(n-1)+d(n-1), d(3) = 41845, d(2) = 2540, d(1) = 155, d(0) = 10

mov $3,-1
add $0,2
lpb $0
  sub $0,1
  add $2,$4
  mov $5,$2
  mov $1,$2
  sub $1,$4
  mul $2,8
  sub $4,$3
  add $4,$2
  add $3,$1
lpe
mov $0,$5
