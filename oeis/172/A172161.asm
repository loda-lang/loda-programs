; A172161: Greedy Coppersmith-Winograd sequence.
; Submitted by Gibson Praise
; 0,1,2,3,4,6,9,13,20,30,45,67,101,151,227,340,510,765,1148,1722,2583,3874,5811,8717,13075,19613,29419,44129,66193,99290,148935,223402,335103,502655,753982,1130973,1696460,2544690,3817035,5725552
; Formula: a(n) = b(n-1), b(n) = b(n-1)+d(n-1)+1, b(2) = 2, b(1) = 1, b(0) = 0, c(n) = 2*c(n-1)-c(n-2)+truncate((-d(n-3)+c(n-3)-1)/2)+1, c(3) = 3, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = truncate((-d(n-1)+c(n-1)-1)/2), d(2) = 0, d(1) = 0, d(0) = 0

#offset 1

sub $0,1
lpb $0
  sub $0,1
  add $2,$1
  add $3,1
  add $1,$3
  mov $3,$2
  sub $3,$1
  div $3,2
lpe
mov $0,$1
