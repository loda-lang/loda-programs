...
; Formula: a(n) = 2*gcd(if((-floor((sqrtint(4*n)^2)/4)+n)<=(-1),0,2^(-floor((sqrtint(4*n)^2)/4)+n)),2)*(-(-floor((sqrtint(4*n-3)^2)/4)+n)*(-floor((sqrtint(4*n-3)^2)/4)+n-1)+sqrtint(4*n-3)*(-floor((sqrtint(4*n-3)^2)/4)+n))
