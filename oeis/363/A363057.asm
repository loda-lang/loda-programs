; A363057: Run lengths of the Fibonacci word (A003849).
; Submitted by Science United
; 1,1,2,1,1,1,2,1,2,1,1,1,2,1,1,1,2,1,2,1,1,1,2,1,2,1,1,1,2,1,1,1,2,1,2,1,1,1,2,1,1,1,2,1,2,1,1,1,2,1,2,1,1,1,2,1,1,1,2,1,2,1,1,1,2,1,2,1,1,1,2,1,1,1,2,1,2,1,1,1,2,1,1,1,2,1,2
; Formula: a(n) = max(gcd((-c(n-1)+b(n-1))/2+a(n-1)+2,4),3)/2, a(2) = 2, a(1) = 1, a(0) = 1, b(n) = (-c(n-1)+b(n-1))/2, b(2) = -3, b(1) = -2, b(0) = -1, c(n) = gcd((-c(n-1)+b(n-1))/2+a(n-1)+2,4)*c(n-1), c(2) = 16, c(1) = 4, c(0) = 4

mov $2,1
add $0,2
lpb $0
  sub $0,1
  sub $1,$2
  div $1,2
  add $3,$1
  add $3,2
  gcd $3,4
  mul $2,$3
  max $3,3
  div $3,2
lpe
mov $0,$3
