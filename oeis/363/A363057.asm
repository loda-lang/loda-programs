...
; Formula: a(n) = d(n+1), b(n) = truncate((-c(n-1)+b(n-1))/2), b(2) = -1, b(1) = 0, b(0) = 0, c(n) = gcd(d(n-1)+truncate((-c(n-1)+b(n-1))/2)+2,4)*c(n-1), c(2) = 4, c(1) = 2, c(0) = 1, d(n) = floor(max(gcd(d(n-1)+truncate((-c(n-1)+b(n-1))/2)+2,4),3)/2), d(2) = 1, d(1) = 1, d(0) = 0
