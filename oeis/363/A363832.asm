...
; Formula: a(n) = truncate(c(max(2*n-2,0))/gcd(c(max(2*n-2,0))*b(max(2*n-2,0)),b(max(2*n-2,0))))+1, b(n) = b(n-1)*(n+2), b(2) = 24, b(1) = 6, b(0) = 2, c(n) = (n+2)*(c(n-1)+1)+b(n-1), c(2) = 18, c(1) = 2, c(0) = -1
