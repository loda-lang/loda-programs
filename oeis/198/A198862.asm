; A198862: Sum of the n-th antidiagonal in the triangle A192011.
; Submitted by Cruncher Pete
; -1,2,2,2,3,1,-1,-3,-6,-7,-6,-3,3,10,16,19,16,6,-10,-29,-45,-51,-41,-12,33,84,125,137,104,20,-105,-242,-346,-366,-261,-19,327,693,954,973,646,-47,-1001,-1974,-2620,-2573,-1572,402,3022
; Formula: a(n) = b(n+1), b(n) = d(n-2)+2, b(4) = 2, b(3) = 2, b(2) = 2, b(1) = -1, b(0) = 0, c(n) = b(n-1)+c(n-1), c(4) = 3, c(3) = 1, c(2) = -1, c(1) = 0, c(0) = 0, d(n) = -b(n-2)-c(n-2), d(4) = -1, d(3) = 1, d(2) = 0, d(1) = 0, d(0) = 0

add $0,1
lpb $0
  sub $0,1
  mov $4,$2
  add $6,3
  add $2,$1
  sub $3,1
  mov $1,$3
  mov $3,$6
  sub $5,$4
  mov $6,$5
  mov $5,0
lpe
mov $0,$1
