; A198279: Number of 2n X 4 0..2 arrays with values 0..2 introduced in row major order and each element equal to exactly two horizontal and vertical neighbors.
; Submitted by Christian Krause
; 1,4,14,52,190,700,2574,9476,34878,128396,472654,1739988,6405438,23580508,86807566,319567268,1176432574,4330837036,15943242574,58692346740,216065933246,795410139132,2928167715854,10779553536580,39683100749118,146086614806220,537793131681358,1979794335538708,7288277555386430,26830559503633564,98772160885611022,363612982602986212,1338579615268487358,4927754156592047468,18140692380812787534,66781886757695298740,245846206158857602750,905041172346623409468,3331755801483788850702
; Formula: a(n) = c(n+1)+1, b(n) = 4*b(n-2)+4*d(n-2)+2*b(n-2)+2*e(n-2)+b(n-1)+2, b(4) = 39, b(3) = 11, b(2) = 3, b(1) = 1, b(0) = 0, c(n) = 2*b(n-1)+2*d(n-1)+b(n-1)+e(n-1), c(4) = 51, c(3) = 13, c(2) = 3, c(1) = 0, c(0) = 0, d(n) = 2*d(n-1)+b(n-1), d(4) = 21, d(3) = 5, d(2) = 1, d(1) = 0, d(0) = 0, e(n) = 2*b(n-1)+e(n-1), e(4) = 30, e(3) = 8, e(2) = 2, e(1) = 0, e(0) = 0

add $0,1
lpb $0
  sub $0,1
  add $4,$1
  add $4,$1
  add $1,$3
  add $3,$1
  add $5,$2
  add $1,$5
  add $1,1
  mov $2,$3
  add $2,$4
  mov $5,$4
  add $5,1
lpe
mov $0,$2
add $0,1
