...
; Formula: a(n) = truncate(((-n+binomial(floor((sqrtint(8*n)+1)/2),2)+floor((sqrtint(8*n)+1)/2))*if((2*n-2*binomial(floor((sqrtint(8*n)+1)/2),2))<=(-1),0,2^(2*n-2*binomial(floor((sqrtint(8*n)+1)/2),2))))/2)+1
