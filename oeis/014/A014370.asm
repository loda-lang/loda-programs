; A014370: If n = binomial(b,2) + binomial(c,1), b > c >= 0 then a(n) = binomial(b+1,3) + binomial(c+1,2).
; Submitted by bcavnaugh
; 1,2,4,5,7,10,11,13,16,20,21,23,26,30,35,36,38,41,45,50,56,57,59,62,66,71,77,84,85,87,90,94,99,105,112,120,121,123,126,130,135,141,148,156,165,166,168,171,175,180,186,193,201,210,220,221,223,226,230,235,241,248,256,265,275,286
; Formula: a(n) = b(n-1), b(n) = -truncate((94*truncate((sqrtint(8*n+8)-1)/2)+1)/(-binomial(truncate((sqrtint(8*n+8)-1)/2)+1,2)-94*truncate((sqrtint(8*n+8)-1)/2)+n-1))*(-binomial(truncate((sqrtint(8*n+8)-1)/2)+1,2)-94*truncate((sqrtint(8*n+8)-1)/2)+n-1)+94*truncate((sqrtint(8*n+8)-1)/2)+b(n-1)+2, b(0) = 1

#offset 1

mov $1,1
sub $0,1
lpb $0
  mov $2,$0
  add $2,1
  mov $4,$2
  mul $4,8
  nrt $4,2
  sub $4,1
  div $4,2
  mov $3,$4
  add $3,1
  bin $3,2
  mul $4,94
  add $4,1
  sub $2,1
  sub $2,$3
  sub $2,$4
  mod $4,$2
  sub $0,1
  add $1,1
  add $1,$4
lpe
mov $0,$1
