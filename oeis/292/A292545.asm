; A292545: Number of 6-cycles in the n-Sierpinski tetrahedron graph.
; Submitted by loader3229
; 0,218,876,3504,14016,56064,224256,897024,3588096,14352384,57409536,229638144,918552576,3674210304,14696841216,58787364864,235149459456,940597837824,3762391351296,15049565405184,60198261620736,240793046482944,963172185931776,3852688743727104
; Formula: a(n) = c(n-1), b(n) = b(n-1), c(n) = d(n-1), c(2) = 876, c(1) = 218, c(0) = 0, d(n) = truncate((d(n-1)*(b(n-1)*(e(n-1)*b(n-1)-8)+438)+d(n-2)*(e(n-1)*b(n-1)-8))/109), d(2) = 3504, d(1) = 876, d(0) = 218, e(n) = b(n-1)*(e(n-1)*b(n-1)-8)+438, e(2) = 438, e(1) = 438, e(0) = 0

#offset 1

mov $3,218
sub $0,1
lpb $0
  mul $5,$1
  sub $5,8
  mul $2,$5
  rol $2,2
  mul $5,$1
  add $5,438
  sub $0,1
  mov $4,$2
  mul $4,$5
  add $3,$4
  div $3,109
lpe
mov $0,$2
