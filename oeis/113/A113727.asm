; A113727: A Pell convolution.
; Submitted by ckrause
; 1,0,4,4,17,32,88,200,497,1184,2876,6924,16737,40384,97520,235408,568353,1372096,3312564,7997204,19306993,46611168,112529352,271669848,655869073,1583407968,3822685036,9228778012,22280241089,53789260160
; Formula: a(n) = truncate(gcd(d(n+2),b(n+2)==0)/2), b(n) = -b(n-1)+c(n-1)+d(n-1), b(2) = 2, b(1) = 0, b(0) = 0, c(n) = -d(n-1)+b(n-1)+2, c(2) = 2, c(1) = 2, c(0) = 0, d(n) = -c(n-1)+b(n-1), d(2) = -2, d(1) = 0, d(0) = 0

add $0,2
lpb $0
  sub $0,1
  sub $2,$1
  add $2,$4
  mov $3,$1
  sub $3,$4
  mov $1,$2
  mov $2,$3
  add $2,2
  sub $4,$1
lpe
equ $1,0
gcd $4,$1
mov $0,$4
div $0,2
