; A113224: a(2n) = A002315(n), a(2n+1) = A082639(n+1).
; Submitted by DenMartin
; 1,2,7,16,41,98,239,576,1393,3362,8119,19600,47321,114242,275807,665856,1607521,3880898,9369319,22619536,54608393,131836322,318281039,768398400,1855077841,4478554082,10812186007,26102926096,63018038201
; Formula: a(n) = truncate((min(n+1,(n+1)%2)*c(n+1)+b(n+1)+min(n+1,(n+1)%2))/2), b(n) = 3*b(n-2)+2*c(n-2)+2, b(3) = 5, b(2) = 5, b(1) = 1, b(0) = 1, c(n) = 4*b(n-2)+3*c(n-2)+4, c(3) = 8, c(2) = 8, c(1) = 0, c(0) = 0

mov $1,1
add $0,1
lpb $0
  sub $0,2
  add $2,$1
  add $1,$2
  add $2,2
  add $1,$2
  add $2,$1
lpe
add $1,$0
mul $0,$2
add $0,$1
div $0,2
