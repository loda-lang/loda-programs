; A113225: a(2n) = A011900(n), a(2n+1) = A001109(n+1).
; Submitted by Science United
; 1,1,3,6,15,35,85,204,493,1189,2871,6930,16731,40391,97513,235416,568345,1372105,3312555,7997214,19306983,46611179,112529341,271669860,655869061,1583407981,3822685023,9228778026,22280241075,53789260175
; Formula: a(n) = b(n+1)/4, b(n) = 2*d(n-2)+b(n-1)+c(n-2)+d(n-2), b(3) = 12, b(2) = 6, b(1) = 4, b(0) = 0, c(n) = d(n-1), c(3) = 4, c(2) = 2, c(1) = 0, c(0) = 2, d(n) = 2*d(n-1)+d(n-2), d(3) = 10, d(2) = 4, d(1) = 2, d(0) = 0

mov $3,2
mov $4,1
add $0,1
lpb $0
  sub $0,1
  add $3,$5
  add $1,$3
  add $1,$4
  add $1,$4
  mov $2,$3
  mov $3,$5
  mul $4,0
  add $5,$2
lpe
mov $0,$1
div $0,4
