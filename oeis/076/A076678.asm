; A076678: a(0)=a(1)=1 a(n)=floor(sqrt(a(n-1)))+a(n-2).
; Submitted by Orange Kid
; 1,1,2,2,3,3,4,5,6,7,8,9,11,12,14,15,17,19,21,23,25,28,30,33,35,38,41,44,47,50,54,57,61,64,69,72,77,80,85,89,94,98,103,108,113,118,123,129,134,140,145,152,157,164,169,177,182,190,195,203,209,217,223,231,238
; Formula: a(n) = d(n)+1, b(n) = e(n-3)/(b(n-2)+1), b(4) = 1, b(3) = 0, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = -c(n-1)+b(n-1)+1, c(4) = 1, c(3) = 0, c(2) = 1, c(1) = 0, c(0) = 1, d(n) = e(n-1), d(4) = 2, d(3) = 1, d(2) = 1, d(1) = 0, d(0) = 0, e(n) = c(n-1)+e(n-1), e(4) = 2, e(3) = 2, e(2) = 1, e(1) = 1, e(0) = 0

mov $2,1
lpb $0
  sub $0,1
  mov $6,$2
  mov $2,1
  add $2,$1
  div $4,$2
  mov $1,$3
  sub $2,$6
  mov $3,$4
  mov $4,$5
  add $5,$6
lpe
add $4,1
mov $0,$4
