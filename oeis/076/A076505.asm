...
; Formula: a(n) = b(n+1), b(n) = max(b(n-1),d(n-1))*(c(n-1)+1), b(2) = 2, b(1) = 1, b(0) = 1, c(n) = -3*truncate((c(n-1)+1)/3)+c(n-1)+1, c(2) = 2, c(1) = 1, c(0) = 0, d(n) = max(-d(n-2)+d(n-1),d(n-1))*(c(n-1)+1)+d(n-1), d(3) = 12, d(2) = 3, d(1) = 1, d(0) = 0
