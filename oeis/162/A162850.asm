; A162850: Number of reduced words of length n in Coxeter group on 36 generators S_i with relations (S_i)^2 = (S_i S_j)^3 = I.
; Submitted by loader3229
; 1,36,1260,43470,1499400,51707880,1783182870,61494142500,2120662873980,73132344752670,2522013244518600,86973155625205080,2999321996442766470,103433437289822465700,3566964788136020871180,123008943076595227404270
; Formula: a(n) = 2*c(n)+2*d(n)+b(n)+e(n), b(n) = e(n-3), b(3) = 1, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = e(n-2), c(3) = 34, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = e(n-1), d(3) = 1190, d(2) = 34, d(1) = 1, d(0) = 0, e(n) = 34*e(n-1)+34*e(n-2)-595*e(n-3), e(4) = 1414944, e(3) = 41021, e(2) = 1190, e(1) = 34, e(0) = 1

mov $4,1
lpb $0
  sub $0,1
  mov $1,$2
  mul $2,-595
  mov $5,$2
  mov $2,$3
  mul $3,34
  add $5,$3
  mov $3,$4
  mul $4,34
  add $4,$5
lpe
mov $0,$1
add $0,$2
add $0,$2
add $0,$3
add $0,$3
add $0,$4
