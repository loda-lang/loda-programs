; A162937: Number of reduced words of length n in Coxeter group on 6 generators S_i with relations (S_i)^2 = (S_i S_j)^4 = I.
; Submitted by loader3229
; 1,6,30,150,735,3600,17640,86400,423210,2073000,10154040,49737000,243624060,1193330400,5845225440,28631349600,140243381160,686946520800,3364832751840,16481777119200,80731791755760,395444141299200
; Formula: a(n) = 2*b(n)-c(n)+e(n), b(n) = 5*b(n-1)-15*d(n-1), b(5) = 2990, b(4) = 610, b(3) = 125, b(2) = 25, b(1) = 5, b(0) = 1, c(n) = 4*b(n-1)-14*d(n-1), c(5) = 2384, c(4) = 486, c(3) = 100, c(2) = 20, c(1) = 4, c(0) = 1, d(n) = 4*b(n-4)-14*d(n-4), d(6) = 100, d(5) = 20, d(4) = 4, d(3) = 1, d(2) = 0, d(1) = 0, d(0) = 0, e(n) = d(n-1), e(5) = 4, e(4) = 1, e(3) = 0, e(2) = 0, e(1) = 0, e(0) = 0

mov $1,1
mov $3,1
lpb $0
  sub $0,1
  ror $3,5
  mov $2,$7
  mul $2,14
  mov $3,$1
  mul $3,4
  sub $3,$2
  sub $1,$7
  add $1,$3
lpe
mov $0,$1
add $0,$1
add $0,$7
sub $0,$3
