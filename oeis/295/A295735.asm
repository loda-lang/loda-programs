; A295735: a(n) = a(n-1) + 3*a(n-2) -2*a(n-3) - 2*a(n-4), where a(0) = 0, a(1) = -1, a(2) = 0, a(3) = 1.
; Submitted by Conan
; 0,-1,0,1,3,8,15,31,54,101,171,304,507,875,1446,2449,4023,6728,11007,18247,29766,49037,79827,130912,212787,347795,564678,920665,1493535,2430584,3940503,6403855,10377126,16846517,27289179,44266768,71687019,116215931,188165094,304905313,493594695,799548584,1294191855,2095837591,3392126598,5492158493,8888479395,14389026496,23285894499,37691698211,60994369926,98719622569,159747546927,258534278360,418348934151,677017430239,1095500582118,1772786447813,2868555465387,4641878784112,7510971120411
; Formula: a(n) = -d(n-1)-e(n-1)+c(n-1)-1, a(4) = 3, a(3) = 1, a(2) = 0, a(1) = -1, a(0) = 0, b(n) = 2*b(n-1)+2*c(n-1)-2, b(4) = -36, b(3) = -16, b(2) = -6, b(1) = -2, b(0) = 0, c(n) = e(n-1), c(4) = 0, c(3) = -1, c(2) = -1, c(1) = 0, c(0) = 0, d(n) = 2*b(n-2)+2*c(n-2)+2*e(n-2)-c(n-2)-d(n-1)-d(n-2)-2*e(n-2)+c(n-2)+d(n-2)+e(n-2)-2, d(4) = -12, d(3) = -5, d(2) = -2, d(1) = 0, d(0) = 0, e(n) = -d(n-1)-e(n-1)+c(n-1)+e(n-1)-1, e(4) = 3, e(3) = 0, e(2) = -1, e(1) = -1, e(0) = 0

lpb $0
  sub $0,1
  sub $3,1
  add $4,$5
  add $1,$3
  sub $3,$4
  mov $4,$2
  add $4,$1
  mul $1,2
  mov $2,$3
  mov $3,$5
  add $4,2
  add $4,$2
  add $5,$2
lpe
mov $0,$2
