; A251704: 4-step Fibonacci sequence starting with 1, 1, 0, 1.
; Submitted by Christian Krause
; 1,1,0,1,3,5,9,18,35,67,129,249,480,925,1783,3437,6625,12770,24615,47447,91457,176289,339808,655001,1262555,2433653,4691017,9042226,17429451,33596347,64759041,124827065,240611904,463794357,893992367,1723225693
; Formula: a(n) = -a(n-2)+c(n-1), a(4) = 3, a(3) = 1, a(2) = 0, a(1) = 1, a(0) = 1, b(n) = c(n-1), b(4) = 3, b(3) = 2, b(2) = 1, b(1) = 0, b(0) = 0, c(n) = b(n-1)+a(n-1)+d(n-1), c(4) = 6, c(3) = 3, c(2) = 2, c(1) = 1, c(0) = 0, d(n) = b(n-1)+a(n-1)+d(n-1), d(4) = 6, d(3) = 3, d(2) = 2, d(1) = 1, d(0) = 0

mov $2,1
mov $4,-1
lpb $0
  sub $0,1
  add $5,$1
  add $5,$2
  mov $1,$3
  sub $3,$4
  mov $4,$2
  mov $2,$3
  mov $3,$5
lpe
mov $0,$2
