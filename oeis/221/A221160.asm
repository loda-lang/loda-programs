; A221160: G.f.: Sum_{n>=0} (4*n+1)^n * x^n / (1 + (4*n+1)*x)^n.
; Submitted by Jamie Morken(s1.)
; 1,5,56,864,16896,399360,11059200,350945280,12551454720,499415777280,21879167385600,1046394961920000,54245114825932800,3029690116944691200,181363518724689100800,11583863454028529664000,786298610212845649920000,56523637237014847291392000
; Formula: a(n) = truncate(((truncate((c(4*n)-2)/4)+1)*(2*n+b(4*n)+3))/3), b(n) = b(n-4), b(7) = 0, b(6) = 0, b(5) = 0, b(4) = 0, b(3) = 0, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = n*c(n-4), c(7) = 21, c(6) = 18, c(5) = 15, c(4) = 12, c(3) = 3, c(2) = 3, c(1) = 3, c(0) = 3

mov $2,$0
mov $3,3
mul $0,4
lpb $0
  mul $3,$0
  sub $0,4
lpe
sub $3,2
div $3,4
add $3,1
add $1,$2
add $1,3
add $1,$2
mul $1,$3
mov $0,$1
div $0,3
