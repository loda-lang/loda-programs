; A221536: Number of 0..2 arrays of length n with each element differing from at least one neighbor by something other than 1, starting with 0.
; Submitted by Christian Krause
; 0,2,4,10,22,54,134,334,822,2014,4934,12110,29750,73086,179494,440750,1082262,2657630,6526342,16026766,39356662,96646974,237332966,582812014,1431196758,3514554142,8630600774,21193942094,52045411766,127806567678,313851282598,770716480814,1892628524310,4647678882014,11413184727814,28027062342670,68825331669622,169012585851582,415039833126758,1019202577047790,2502829391568342,6146133364015774,15092900641417670,37063245504412622,91015252796675894,223503800837265534,548852499484115494
; Formula: a(n) = truncate(f1(n-1)/3), b(n) = 2*b(n-1)+2*e(n-3)+6, b(8) = 1680, b(7) = 678, b(6) = 276, b(5) = 114, b(4) = 48, b(3) = 18, b(2) = 6, b(1) = 0, b(0) = 0, c(n) = 2*c(n-1)-2*e(n-2)+d(n-1)+e(n-1)-3, c(7) = 1071, c(6) = 438, c(5) = 177, c(4) = 72, c(3) = 27, c(2) = 12, c(1) = 3, c(0) = 3, d(n) = 2*e(n-2)+6, d(7) = 324, d(6) = 126, d(5) = 48, d(4) = 18, d(3) = 12, d(2) = 6, d(1) = 6, d(0) = 0, e(n) = 4*e(n-4)+3*e(n-1)-2*e(n-2)+9, e(9) = 5667, e(8) = 2304, e(7) = 945, e(6) = 390, e(5) = 159, e(4) = 60, e(3) = 21, e(2) = 6, e(1) = 3, e(0) = 0, f(n) = 2*c(n-1)-d(n-1)-2*b(n-1)+f(n-1), f(5) = 48, f(4) = 18, f(3) = 12, f(2) = 6, f(1) = 6, f(0) = 0, f1(n) = 2*c(n-1)+f(n-1), f1(5) = 162, f1(4) = 66, f1(3) = 30, f1(2) = 12, f1(1) = 6, f1(0) = 0

#offset 1

mov $2,3
sub $0,1
lpb $0
  sub $0,1
  mul $1,2
  mul $2,2
  add $4,$1
  add $4,3
  add $5,$2
  mov $6,$5
  add $1,$3
  mov $2,$3
  add $2,$4
  mov $3,$5
  sub $3,$1
  sub $5,$1
lpe
mov $0,$6
div $0,3
