; A180669: a(n) = a(n-1)+a(n-2)+a(n-3)+4*n^2-16*n+18 with a(0)=0, a(1)=0 and a(2)=1.
; Submitted by Simon Strandgaard
; 0,0,1,7,26,72,171,371,760,1500,2889,5475,10266,19116,35435,65495,120832,222664,410017,754671,1388650,2554784,4699707,8644907,15901336,29248068,53796617,98948523,181995914,334743972,615691547
; Formula: a(n) = b(n-1)+a(n-1), a(5) = 72, a(4) = 26, a(3) = 7, a(2) = 1, a(1) = 0, a(0) = 0, b(n) = 8*n+2*c(n-2)+b(n-2)-10, b(5) = 99, b(4) = 46, b(3) = 19, b(2) = 6, b(1) = 1, b(0) = 0, c(n) = 4*n+b(n-1)+c(n-1)-2, c(5) = 122, c(4) = 58, c(3) = 25, c(2) = 9, c(1) = 2, c(0) = 0

lpb $0
  sub $0,1
  sub $5,1
  add $5,$1
  add $6,$2
  add $1,2
  add $2,1
  add $2,$1
  sub $3,$4
  add $3,$5
  add $3,2
  mov $4,$2
  add $5,$2
  mov $2,$3
  mov $3,$5
  add $3,2
lpe
mov $0,$6
