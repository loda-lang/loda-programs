...
; Formula: a(n) = if((truncate((-d(n)+b(n)+n+2)/2)-2)<=(-1),0,2^(truncate((-d(n)+b(n)+n+2)/2)-2)), b(n) = -2*binomial(gcd(c(n-1),n+1),n+1)+b(n-1)+1, b(3) = -1, b(2) = -2, b(1) = -1, b(0) = 0, c(n) = 2*c(n-1)-n-2, c(3) = -25, c(2) = -10, c(1) = -3, c(0) = 0, d(n) = -2*binomial(gcd(c(n-1),n+1),n+1), d(3) = 0, d(2) = -2, d(1) = -2, d(0) = 0
