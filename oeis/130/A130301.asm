; A130301: Triangle read by rows: A130296 * A007318, as infinite lower triangular matrices.
; Submitted by loader3229
; 1,3,1,5,3,1,7,6,4,1,9,10,10,5,1,11,15,20,15,6,1,13,21,35,35,21,7,1,15,28,56,70,56,28,8,1,17,36,84,126,126,84,36,9,1,19,45,120,210,252,210,120,45,10,1,21,55,165,330,462,462,330,165,55,11,1
; Formula: a(n) = floor((sqrtint(8*n)-1)/2)*binomial(1,-binomial(floor((sqrtint(8*n)-1)/2)+1,2)+n)+binomial(floor((sqrtint(8*n)-1)/2)+1,-binomial(floor((sqrtint(8*n)-1)/2)+1,2)+n)

#offset 1

mov $1,$0
mul $0,8
nrt $0,2
sub $0,1
div $0,2
mov $2,$0
add $2,1
bin $2,2
sub $1,$2
mov $4,$0
add $4,1
bin $4,$1
mov $3,1
bin $3,$1
mul $0,$3
add $0,$4
