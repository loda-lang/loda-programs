; A130844: a(n) = 2*a(n-1) + a(n-2) - a(n-3) + a(n-4), with a(1) = 0, a(2) = 3, a(3) = 5 and a(4) = 17.
; Submitted by ckrause
; 0,3,5,17,36,87,198,464,1075,2503,5815,13522,31431,73072,169868,394899,918025,2134153,4961300,11533627,26812426,62331332,144902763,336858059,783099975,1820486578,4232117835,9838480332,22871691896,53170232867
; Formula: a(n) = c(n-1), b(n) = 2*b(n-2)+2*d(n-3)+b(n-1)+b(n-3)+5, b(5) = 61, b(4) = 25, b(3) = 8, b(2) = 3, b(1) = 0, b(0) = 0, c(n) = 3*b(n-2)+2*c(n-2)+2*d(n-2)+5, c(3) = 17, c(2) = 5, c(1) = 3, c(0) = 0, d(n) = b(n-1)+d(n-1)+3, d(3) = 12, d(2) = 6, d(1) = 3, d(0) = 0

#offset 1

sub $0,1
lpb $0
  sub $0,1
  add $2,3
  add $2,$1
  add $4,$1
  mov $5,1
  add $5,$2
  add $1,$3
  mov $2,$3
  add $2,$4
  add $4,2
  add $2,$4
  mov $3,$5
  sub $3,1
  add $4,1
lpe
mov $0,$3
