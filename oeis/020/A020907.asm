; A020907: Position of n-th 2 in A020906.
; Submitted by idlegnome
; 2,5,9,11,14,17,20,24,27,32,35,37,41,44,47,51,54,56,58,62,65,68,70,74,77,81,83,87,90,92,95,97,101,104,107,110,112,116,119,123,126,128,132,135,137,140,143,145,149,152,155,158,161,163,167,170,174,177

#offset 1

mov $2,$0
sub $0,1
pow $2,2
lpb $2
  mov $3,$1
  add $3,1
  seq $3,283683 ; Unique sequence with a(1)=0, a(2)=1, representing an array T(i,j) read by antidiagonals in which every row is this sequence itself.
  sub $0,$3
  add $1,1
  mov $4,$0
  max $4,0
  equ $4,$0
  mul $2,$4
  sub $2,1
lpe
mov $0,$1
add $0,1
