...
; Formula: a(n) = truncate((truncate(truncate(b(floor((2*max(n-1,0))/2)+2)/(floor((floor((2*max(n-1,0))/2)+1)/2)+1))/(floor(floor((2*max(n-1,0))/2)/2)+1))*(2*max(n-1,0)+2))/4), b(n) = n*b(n-1), b(0) = 1
