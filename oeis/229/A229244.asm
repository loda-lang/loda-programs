...
; Formula: a(n) = truncate((truncate(truncate(b(max(floor((2*max(n-1,0))/2)+2,0))/(truncate((floor((2*max(n-1,0))/2)+1)/2)+1))/(truncate(floor((2*max(n-1,0))/2)/2)+1))*(2*max(n-1,0)+2))/4), b(n) = n*b(n-1), b(0) = 1
