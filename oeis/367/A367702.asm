; A367702: Number of degree 4 vertices in the n-Menger sponge graph.
; Submitted by Dune Finkleberry
; 0,144,2784,57552,1180320,23889936,480221280,9624275280,192645717024,3854200280208,77094305873376,1541968557881808,30840030795738528,616805893363960080,12336160087905835872,246723539526229152336,4934473492678780614432,98689491470837087102352
; Formula: a(n) = 48*truncate(d(n-1)/3), b(n) = 8*b(n-1)+2*c(n-1), b(2) = 56, b(1) = 2, b(0) = 0, c(n) = 20*c(n-1), c(2) = 400, c(1) = 20, c(0) = 1, d(n) = 12*b(n-1)+6*c(n-1)+3*d(n-1)+3, d(2) = 174, d(1) = 9, d(0) = 0

#offset 1

mov $2,1
sub $0,1
lpb $0
  sub $0,1
  mul $1,4
  mul $2,2
  mov $4,$2
  add $4,$1
  add $1,$4
  mul $2,10
  add $3,1
  add $3,$4
  mul $3,3
lpe
mov $0,$3
div $0,3
mul $0,48
