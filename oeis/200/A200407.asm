; A200407: The x-values in the solution to 19*x^2 - 18 = y^2.
; Submitted by loader3229
; 1,9,131,209,3051,44539,71059,1037331,15143129,24159851,352689489,5148619321,8214278281,119913388929,1750515426011,2792830455689,40770199546371,595170096224419,949554140655979,13861747932377211,202356082200876449,322845614992577171
; Formula: a(n) = b(n-1), b(n) = c(n-3), b(5) = 44539, b(4) = 3051, b(3) = 209, b(2) = 131, b(1) = 9, b(0) = 1, c(n) = 340*c(n-3)-c(n-6), c(7) = 352689489, c(6) = 24159851, c(5) = 15143129, c(4) = 1037331, c(3) = 71059, c(2) = 44539, c(1) = 3051, c(0) = 209

#offset 1

mov $1,1
mov $2,9
mov $3,131
mov $4,209
mov $5,3051
mov $6,44539
sub $0,1
lpb $0
  sub $0,1
  mul $1,-1
  mov $7,$1
  mov $1,$2
  mov $2,$3
  mov $3,$4
  mul $4,340
  add $7,$4
  mov $4,$5
  mov $5,$6
  mov $6,$7
lpe
mov $0,$1
