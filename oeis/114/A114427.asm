; A114427: Decimal expansion of the real solution of x^3-x^2-x-4=0.
; Submitted by matali
; 2,2,4,1,8,9,6,5,6,3,0,3,4,4,7,9,8,5,5,7,8,9,4,9,2,5,0,2,6,6,9,4,9,8,5,0,3,2,4,5,6,7,6,8,4,3,1,6,6,4,9,5,6,6,2,3,5,8,8,6,5,0,1,4,0,4,2,3,9,2,2,7,4,5,7,7,4,5,6,8
; Formula: a(n) = ((d(3*n)+1)/(c(3*n)/(10^n)+1))%10, b(n) = 8*c(n-1)+6*d(n-1)+2*b(n-1), b(2) = 50, b(1) = 6, b(0) = 0, c(n) = 7*c(n-1)+4*d(n-1)+b(n-1), c(2) = 38, c(1) = 4, c(0) = 0, d(n) = 2*c(n-1)+d(n-1), d(2) = 9, d(1) = 1, d(0) = 1

mov $5,1
mov $3,$0
mul $3,3
lpb $3
  sub $3,1
  add $1,$5
  add $5,$2
  add $5,$2
  add $1,$5
  add $1,$5
  add $2,$5
  add $2,$1
  mul $1,2
lpe
mov $4,10
pow $4,$0
div $2,$4
add $2,1
mov $1,1
add $1,$5
div $1,$2
mov $0,$1
mod $0,10
