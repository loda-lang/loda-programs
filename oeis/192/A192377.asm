; A192377: Coefficient of x in the reduction by x^2->x+2 of the polynomial p(n,x) defined below in Comments.
; Submitted by Kotenok2000
; 0,2,4,20,68,262,968,3624,13512,50442,188236,702524,2621836,9784846,36517520,136285264,508623504,1898208786,7084211604,26438637668,98670339028,368242718486,1374300534872,5128959421048,19141537149272,71437189176090
; Formula: a(n) = d(max(n-1,0)), b(n) = 3*b(n-1)+2*c(n-1), b(3) = 41, b(2) = 11, b(1) = 3, b(0) = 1, c(n) = b(n-1)+c(n-1), c(3) = 15, c(2) = 4, c(1) = 1, c(0) = 0, d(n) = 2*b(n-1)+2*c(n-1)+2*e(n-1)-3*d(n-1), d(3) = 20, d(2) = 4, d(1) = 2, d(0) = 0, e(n) = -2*d(n-1)+b(n-1)+c(n-1)+e(n-1), e(3) = 8, e(2) = 1, e(1) = 1, e(0) = 0

#offset 1

mov $1,1
sub $0,1
lpb $0
  sub $0,1
  add $2,$1
  add $1,$2
  add $1,$2
  sub $4,$3
  add $4,$2
  sub $4,$3
  add $3,$4
  add $3,$4
lpe
mov $0,$3
