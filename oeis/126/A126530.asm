; A126530: Number of base 9 n-digit numbers with adjacent digits differing by five or less.
; Submitted by fzs600
; 1,9,69,541,4231,33101,258953,2025831,15848391,123984439,969949627,7588067405,59362636295,464403174075,3633098554089,28422297349753,222352070721709,1739494972761653,13608340819322255,106460175369651261
; Formula: a(n) = 2*(e(n)/3)+1, b(n) = b(n-1)+max(d(n-1),1)+1, b(3) = 136, b(2) = 17, b(1) = 2, b(0) = 0, c(n) = max(d(n-1),1)+1, c(3) = 119, c(2) = 15, c(1) = 2, c(0) = 0, d(n) = 10*max(d(n-2),1)+6*max(d(n-1),1)+3*b(n-1)+3*b(n-2)+3*max(d(n-3),1)+21, d(3) = 929, d(2) = 118, d(1) = 14, d(0) = 0, e(n) = 6*max(d(n-1),1)+3*b(n-1)+3*c(n-1)+6, e(3) = 810, e(2) = 102, e(1) = 12, e(0) = 0

lpb $0
  sub $0,1
  max $3,1
  add $3,1
  add $4,2
  add $4,$2
  mov $5,$4
  add $1,$3
  add $2,$3
  mov $4,$2
  add $4,$1
  mov $6,$4
  mov $2,$3
  mul $4,2
  add $4,$6
  add $5,$4
  mov $3,$5
lpe
mov $0,$4
div $0,3
mul $0,2
add $0,1
