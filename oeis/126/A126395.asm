; A126395: Number of base 8 n-digit numbers with adjacent digits differing by two or less.
; Submitted by Jamie Morken(w1)
; 1,8,34,150,668,2986,13362,59816,267802,1199022,5368412,24036226,107618586,481846232,2157395218,9659418342,43248619100,193639305850,866991400098,3881825980040,17380302665002,77817738944382
; Formula: a(n) = d(n)+2, b(n) = 2*b(n-1)+2*c(n-1)-2*e(n-1)+2, b(3) = 48, b(2) = 10, b(1) = 2, b(0) = 0, c(n) = 2*b(n-1)+c(n-1)+e(n-1)+1, c(3) = 18, c(2) = 5, c(1) = 1, c(0) = 0, d(n) = 8*b(n-1)+4*c(n-1)+2*c(n-1)-4*e(n-1)+6, d(3) = 148, d(2) = 32, d(1) = 6, d(0) = -1, e(n) = 2*e(n-1)-2*b(n-1)-2*c(n-1)+c(n-1)-1, e(3) = -42, e(2) = -8, e(1) = -1, e(0) = 0

mov $4,-1
lpb $0
  sub $0,1
  mov $2,$1
  add $3,1
  add $1,$3
  add $2,$1
  sub $1,$5
  mul $1,2
  mov $4,$2
  add $4,$1
  mul $4,2
  add $5,$2
  mov $2,$3
  sub $2,$1
  mov $3,$5
  mov $5,$2
lpe
mov $0,$4
add $0,2
