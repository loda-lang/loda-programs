; A118011: Complement of the Connell sequence (A001614); a(n) = 4*n - A001614(n).
; Submitted by p3d-cluster
; 3,6,8,11,13,15,18,20,22,24,27,29,31,33,35,38,40,42,44,46,48,51,53,55,57,59,61,63,66,68,70,72,74,76,78,80,83,85,87,89,91,93,95,97,99,102,104,106,108,110,112,114,116,118,120,123,125,127,129,131,133,135,137,139
; Formula: a(n) = d(n+1)+2, b(n) = c(n-1)+truncate(b(n-1)/2), b(3) = 6, b(2) = 5, b(1) = 2, b(0) = 0, c(n) = gcd(truncate(b(n-1)/2),2)*c(n-1), c(3) = 8, c(2) = 4, c(1) = 4, c(0) = 2, d(n) = d(n-1)+gcd(truncate(b(n-2)/2),2)+1, d(3) = 6, d(2) = 4, d(1) = 1, d(0) = 0

mov $2,2
add $0,1
lpb $0
  sub $0,1
  add $4,$3
  add $4,1
  div $1,2
  mov $3,$1
  gcd $3,2
  add $1,$2
  mul $2,$3
lpe
mov $0,$4
add $0,2
