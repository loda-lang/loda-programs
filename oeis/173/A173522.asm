...
; Formula: a(n) = b(max(2*n-1,0)), b(n) = if((2^2)==1,2^(sumdigits(truncate((n-1)/2),2)*sign(truncate((n-1)/2))),if((sumdigits(truncate((n-1)/2),2)*sign(truncate((n-1)/2)))<=(-1),0,2^(sumdigits(truncate((n-1)/2),2)*sign(truncate((n-1)/2)))))+b(n-1), b(0) = 0
