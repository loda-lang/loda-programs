; A127362: a(n) = Sum_{k=0..n} C(n,floor(k/2))*(-3)^(n-k).
; Submitted by loader3229
; 1,-2,8,-24,84,-272,920,-3040,10180,-33840,112968,-376224,1254696,-4181088,13939248,-46459584,154873860,-516229040,1720795880,-5735921440,19119861304,-63732624672,212442552528,-708140901184,2360471473384,-7868234639072,26227455730640,-87424838568000,291416155304400,-971387130859200,3237957206275680,-10793190480762240,35977302003261060,-119924339209429680,399747798920503080,-1332492659956868640,4441642205906319000,-14805474007587549600,49351580048855341200,-164505266782390785600
; Formula: a(n) = min(n,n%2)*c(n)+b(n), b(n) = truncate((b(n-2)*(16*d(n-2)+3)+c(n-2)*(-4*d(n-2)-7))/(3*d(n-2)+3)), b(5) = 84, b(4) = 84, b(3) = 8, b(2) = 8, b(1) = 1, b(0) = 1, c(n) = truncate((b(n-2)*(-52*d(n-2)+39)+c(n-2)*(88*d(n-2)+109))/(9*d(n-2)+9)), c(5) = -356, c(4) = -356, c(3) = -32, c(2) = -32, c(1) = -3, c(0) = -3, d(n) = d(n-2)+1, d(5) = 2, d(4) = 2, d(3) = 1, d(2) = 1, d(1) = 0, d(0) = 0

mov $1,1
mov $2,-3
lpb $0
  sub $0,2
  mov $4,$3
  mul $4,16
  add $4,3
  mov $5,$3
  mul $5,-4
  sub $5,7
  mul $5,$2
  mov $6,$3
  mul $6,-52
  add $6,39
  mul $6,$1
  mov $7,$3
  mul $7,88
  add $7,109
  mul $1,$4
  add $1,$5
  mul $2,$7
  add $2,$6
  mov $8,$3
  mul $8,3
  add $8,3
  mov $9,$3
  mul $9,9
  add $9,9
  div $1,$8
  div $2,$9
  add $3,1
lpe
mul $0,$2
add $0,$1
