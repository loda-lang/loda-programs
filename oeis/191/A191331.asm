; A191331: Positions of 2 in A191329.
; Submitted by Conan
; 2,6,12,16,18,22,28,32,34,38,42,44,48,54,58,60,64,70,74,80,84,86,90,96,100,102,106,110,112,116,122,126,128,132,138,142,148,152,154,158,164,168,170,174,180,184,190,194,196,200,206,210,212,216,220,222,226,232,236,238,242,248,252,258,262,264,268,274,278,280,284,288
; Formula: a(n) = 2*c(2*n), b(n) = b(n-1)==0, b(4) = 1, b(3) = 0, b(2) = 1, b(1) = 0, b(0) = 1, c(n) = -b(n-1)-2*truncate(truncate((d(n-2)*(2*c(n-1)+2))/(d(n-1)+d(n-2)))/2)+c(n-1)+truncate((d(n-2)*(2*c(n-1)+2))/(d(n-1)+d(n-2)))+1, c(5) = 4, c(4) = 3, c(3) = 1, c(2) = 1, c(1) = 0, c(0) = 0, d(n) = d(n-1)+d(n-2), d(5) = -5, d(4) = -3, d(3) = -2, d(2) = -1, d(1) = -1, d(0) = 0

#offset 1

mov $1,1
mov $2,1
mul $0,2
lpb $0
  sub $0,1
  mul $1,-1
  add $1,$3
  add $4,1
  add $5,$1
  mov $3,$4
  mul $3,2
  mul $3,$1
  div $3,$5
  mod $3,2
  sub $4,$2
  add $4,$3
  equ $2,0
  mov $3,$5
lpe
mov $0,$4
mul $0,2
