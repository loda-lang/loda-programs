; A008627: Molien series for A_4.
; Submitted by [TA]crashtech
; 1,1,2,3,5,6,10,12,17,21,28,33,43,50,62,72,87,99,118,133,155,174,200,222,253,279,314,345,385,420,466,506,557,603,660,711,775,832,902,966,1043,1113,1198,1275,1367,1452,1552,1644,1753,1853,1970,2079,2205,2322
; Formula: a(n) = b(n+4), b(n) = -truncate(floor((max((n-4)^2-1,0)+3)/2)/floor((max((n-4)^2-1,0)+3)/3))*floor((max((n-4)^2-1,0)+3)/3)+b(n-4)+floor((max((n-4)^2-1,0)+3)/2)+1, b(3) = 0, b(2) = 0, b(1) = 0, b(0) = 0

add $0,4
lpb $0
  sub $0,4
  mov $2,$0
  pow $2,2
  trn $2,1
  add $2,3
  mov $3,$2
  div $3,3
  div $2,2
  mod $2,$3
  add $2,1
  add $1,$2
lpe
mov $0,$1
