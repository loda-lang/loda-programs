; A079969: Number of permutations satisfying -k<=p(i)-i<=r and p(i)-i not in I, i=1..n, with k=1, r=5, I={2}.
; Submitted by Athlici
; 1,1,2,3,6,11,21,38,70,128,236,434,799,1469,2702,4969,9140,16811,30921,56872,104604,192396,353872,650872,1197141,2201885,4049898,7448923,13700706,25199527,46349157,85249390,156798074,288396620,530444084
; Formula: a(n) = a(n-2)+a(n-3)+b(n-2)+1, a(5) = 11, a(4) = 6, a(3) = 3, a(2) = 2, a(1) = 1, a(0) = 1, b(n) = 2*c(n-1)+b(n-1)+c(n-2)+c(n-3), b(5) = 20, b(4) = 11, b(3) = 5, b(2) = 2, b(1) = 0, b(0) = 0, c(n) = a(n-1), c(5) = 6, c(4) = 3, c(3) = 2, c(2) = 1, c(1) = 1, c(0) = 0

mov $1,1
lpb $0
  sub $0,1
  mov $5,$1
  mov $6,$4
  add $6,$8
  add $7,$4
  mov $8,$4
  mov $1,1
  add $1,$3
  add $2,$4
  mov $4,$5
  add $5,$2
  add $2,$7
  mov $3,$5
  mov $7,$6
lpe
mov $0,$1
