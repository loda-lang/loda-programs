; A079162: a(n) = 5a(n-2) - 2a(n-4).
; Submitted by Science United
; 0,1,2,4,10,18,46,82,210,374,958,1706,4370,7782,19934,35498,90930,161926,414782,738634,1892050,3369318,8630686,15369322,39369330,70107974,179585278,319801226,819187730,1458790182,3736768094
; Formula: a(n) = (min(n,n%2)*b(n)+c(n))/3, b(n) = 2*b(n-2)+2*c(n-2), b(5) = 24, b(4) = 24, b(3) = 6, b(2) = 6, b(1) = 3, b(0) = 3, c(n) = 2*b(n-2)+2*c(n-2)+c(n-2), c(5) = 30, c(4) = 30, c(3) = 6, c(2) = 6, c(1) = 0, c(0) = 0

mov $1,3
lpb $0
  sub $0,2
  add $1,$2
  mul $1,2
  add $2,$1
lpe
mul $0,$1
add $0,$2
div $0,3
