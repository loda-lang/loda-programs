; A079285: First differences of A079284.
; Submitted by Christian Krause
; 0,2,1,5,4,13,13,34,39,89,112,233,313,610,859,1597,2328,4181,6253,10946,16687,28657,44320,75025,117297,196418,309619,514229,815656,1346269,2145541,3524578,5637351,9227465,14799280,24157817
; Formula: a(n) = 2*d(n-2)+b(n-1), a(3) = 5, a(2) = 1, a(1) = 2, a(0) = 0, b(n) = b(n-1)+b(n-2)+c(n-2), b(3) = 4, b(2) = 3, b(1) = 1, b(0) = 1, c(n) = 2*c(n-2), c(3) = 0, c(2) = 2, c(1) = 0, c(0) = 1, d(n) = 2*d(n-2), d(3) = 2, d(2) = 0, d(1) = 1, d(0) = 0

mov $1,1
mov $2,1
lpb $0
  sub $0,1
  mov $5,$1
  add $5,$2
  add $1,$3
  mov $3,$4
  mov $4,$2
  mov $2,$3
  mul $2,2
  mov $3,$5
lpe
mov $0,$3
