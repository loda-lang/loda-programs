...
; Formula: a(n) = -sign(3*sign(n)*sign(n-1)+sign(n-1)+sign(n))*bitxor(abs(n-1),abs(n))+5*n+3*truncate((sign(3*sign(n)*sign(n-1)+sign(n-1)+sign(n))*bitxor(abs(n-1),abs(n)))/3)-1
