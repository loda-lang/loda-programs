...
; Formula: a(n) = 2*truncate(d(n)/3)+2, b(n) = truncate(truncate((-c(n-1)+b(n-1))/2)/gcd(truncate((-c(n-1)+b(n-1))/2)+1,4)), b(2) = -20, b(1) = -1, b(0) = 0, c(n) = 4*c(n-1), c(2) = 160, c(1) = 40, c(0) = 10, d(n) = gcd(truncate((-c(n-1)+b(n-1))/2)+1,4), d(2) = 1, d(1) = 4, d(0) = 0
