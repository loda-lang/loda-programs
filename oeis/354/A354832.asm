...
; Formula: a(n) = max(-c(n)+b(n),d(n))-1, b(n) = 2*b(n-1), b(2) = 4, b(1) = 2, b(0) = 1, c(n) = c(n-1)^2-2*truncate((c(n-1)^2+1)/(2*b(n-1)))*b(n-1)+1, c(2) = 2, c(1) = 1, c(0) = 0, d(n) = c(n-1)^2-2*truncate((c(n-1)^2+1)/(2*b(n-1)))*b(n-1)+2, d(2) = 3, d(1) = 2, d(0) = 0
