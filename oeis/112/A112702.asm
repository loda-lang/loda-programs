; A112702: Partial sum of Catalan numbers A000108 multiplied by powers of 8.
; Submitted by USTL-FIL (Lille Fr)
; 1,9,137,2697,60041,1436297,36039305,935717513,24927136393,677493729929,18712061405833,523679956331145,14818155751139977,423231749888535177,12185543261045516937,353292577084597987977,10305591916877658319497
; Formula: a(n) = a(n-1)+truncate((16*n*b(n-1)*(2*n-1))/(2*n+c(n-1))), a(3) = 2697, a(2) = 137, a(1) = 9, a(0) = 1, b(n) = truncate((16*n*b(n-1)*(2*n-1))/(2*n+c(n-1))), b(3) = 2560, b(2) = 128, b(1) = 8, b(0) = 1, c(n) = 2*n+c(n-1), c(3) = 12, c(2) = 6, c(1) = 2, c(0) = 0

mov $1,1
mov $3,1
lpb $0
  sub $0,1
  add $2,1
  mul $1,4
  mul $1,$2
  add $2,1
  add $4,$2
  mul $1,2
  mul $1,$2
  div $1,$4
  add $3,$1
lpe
mov $0,$3
