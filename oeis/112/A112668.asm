...
; Formula: a(n) = if((A083503(floor((sqrtint(8*n)-1)/2)+1)^2)==1,A083503(floor((sqrtint(8*n)-1)/2)+1)^(-binomial(floor((sqrtint(8*n)-1)/2)+1,2)+n-1),if((-binomial(floor((sqrtint(8*n)-1)/2)+1,2)+n-1)<=(-1),0,A083503(floor((sqrtint(8*n)-1)/2)+1)^(-binomial(floor((sqrtint(8*n)-1)/2)+1,2)+n-1)))
...
sub $0,1
add $1,1
seq $1,83503 ; Basis of the n-th power arising in A083502.
pow $1,$0
mov $0,$1
