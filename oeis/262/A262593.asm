...
; Formula: a(n) = f1(2*n), b(n) = b(n-1)+d(n-1), b(6) = -13, b(5) = -6, b(4) = -5, b(3) = -2, b(2) = -1, b(1) = 0, b(0) = 0, c(n) = f1(n-1)-2, c(6) = -8, c(5) = -5, c(4) = -5, c(3) = -3, c(2) = -2, c(1) = -1, c(0) = 0, d(n) = 2*e(n-1)-d(n-1)-f(n-1)-f1(n-1)+c(n-1), d(6) = 3, d(5) = -7, d(4) = -1, d(3) = -3, d(2) = -1, d(1) = -1, d(0) = 0, e(n) = -f(n-1)-f1(n-1)+b(n-1)+c(n-1)+d(n-1)+e(n-1), e(6) = -8, e(5) = -9, e(4) = -5, e(3) = -4, e(2) = -2, e(1) = -1, e(0) = 0, f(n) = -f(n-1)-f1(n-1)+b(n-1)+b(n-2)+c(n-1)+d(n-2)+e(n-1), f(6) = -7, f(5) = -13, f(4) = -4, f(3) = -4, f(2) = -1, f(1) = -1, f(0) = 0, f1(n) = -f(n-1)+c(n-1)+e(n-1), f1(6) = -1, f1(5) = -6, f1(4) = -3, f1(3) = -3, f1(2) = -1, f1(1) = 0, f1(0) = 1
