...
; Formula: a(n) = d(n)+2, b(n) = truncate(truncate((-c(n-1)+b(n-1))/2)/gcd(truncate((-c(n-1)+b(n-1))/2)+1,4)), b(2) = -10, b(1) = -1, b(0) = -2, c(n) = 4*c(n-1), c(2) = 80, c(1) = 20, c(0) = 5, d(n) = truncate(gcd(truncate((-c(n-1)+b(n-1))/2)+1,4)/2), d(2) = 0, d(1) = 1, d(0) = 0
