; A096406: a(1) = 4; then alternately add -4 and multiply by -2.
; Submitted by Merlin2331
; 4,0,0,-4,8,4,-8,-12,24,20,-40,-44,88,84,-168,-172,344,340,-680,-684,1368,1364,-2728,-2732,5464,5460,-10920,-10924,21848,21844,-43688,-43692,87384,87380,-174760,-174764,349528,349524,-699048,-699052,1398104,1398100,-2796200,-2796204,5592408,5592404
; Formula: a(n) = 4*b(n-1), b(n) = -2*b(n-1)+c(n-1)+1, b(2) = 0, b(1) = 0, b(0) = 1, c(n) = -d(n-1)-2*c(n-1)-2*c(n-2)-2*d(n-2)+1, c(3) = -1, c(2) = -2, c(1) = -1, c(0) = 1, d(n) = 2*c(n-1)+d(n-1)+1, d(2) = 2, d(1) = 3, d(0) = 0

#offset 1

mov $1,1
mov $2,1
sub $0,1
lpb $0
  sub $0,1
  add $3,$1
  add $3,$2
  add $3,$1
  add $2,1
  mul $1,-2
  add $1,$2
  sub $2,$3
  add $3,$1
lpe
mov $0,$1
mul $0,4
