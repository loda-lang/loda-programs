; A184675: n + floor(sqrt(n) + sqrt(n+1)); complement of A184674.
; Submitted by ChelseaOilman
; 3,5,6,8,9,11,12,13,15,16,17,19,20,21,22,24,25,26,27,29,30,31,32,33,35,36,37,38,39,41,42,43,44,45,46,48,49,50,51,52,53,55,56,57,58,59,60,61,63,64,65,66,67,68,69,71,72,73,74,75,76,77,78,80,81,82,83
; Formula: a(n) = d(n)+3, b(n) = (-c(n-1)+b(n-1))/2+2, b(2) = -17, b(1) = -6, b(0) = 0, c(n) = c(n-1)*(((-c(n-1)+b(n-1))/2+2)%2+2), c(2) = 32, c(1) = 32, c(0) = 16, d(n) = ((-c(n-1)+b(n-1))/2+2)%2+d(n-1)+2, d(2) = 3, d(1) = 2, d(0) = 0

mov $2,16
lpb $0
  sub $0,1
  sub $1,$2
  div $1,2
  add $1,1
  mov $3,1
  add $3,$1
  mod $3,2
  add $3,2
  add $4,$3
  add $1,1
  mul $2,$3
lpe
mov $0,$4
add $0,3
