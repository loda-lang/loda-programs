...
; Formula: a(n) = c(n)+2, b(n) = truncate((-2^(n-1)+b(n-1))/(-2^(n-1)-c(n-1)-2*truncate((-2^(n-1)-c(n-1)+b(n-1))/2)+b(n-1)+2)), b(3) = -3, b(2) = -3, b(1) = -1, b(0) = 0, c(n) = -2^(n-2)-c(n-2)-2*truncate((-2^(n-2)-c(n-2)+b(n-2))/2)+b(n-2)+c(n-1)+4, c(3) = 6, c(2) = 3, c(1) = 0, c(0) = 0
