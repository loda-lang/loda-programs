; A290588: Number of irredundant sets in the n-helm graph.
; Submitted by loader3229
; 5,13,35,113,355,1105,3435,10625,32723,100433,307323,937985,2856547,8682897,26349835,79850241,241679859,730700881,2207155867,6661493633,20090908803,60555714705,182419600683,549259559297,1653098625555,4973441442385,14957936920763
; Formula: a(n) = b(n-1), b(n) = 6*b(n-1)+6*b(n-5)+4*b(n-3)-5*b(n-4)-10*b(n-2), b(11) = 937985, b(10) = 307323, b(9) = 100433, b(8) = 32723, b(7) = 10625, b(6) = 3435, b(5) = 1105, b(4) = 355, b(3) = 113, b(2) = 35, b(1) = 13, b(0) = 5

#offset 1

mov $1,5
mov $2,13
mov $3,35
mov $4,113
mov $5,355
sub $0,1
lpb $0
  mul $1,6
  rol $1,5
  mov $6,$1
  mul $6,-5
  add $5,$6
  mov $6,$2
  mul $6,4
  add $5,$6
  mov $6,$3
  mul $6,-10
  add $5,$6
  mov $6,$4
  mul $6,6
  sub $0,1
  add $5,$6
lpe
mov $0,$1
