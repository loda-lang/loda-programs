; A102429: Row sums of A102427.
; Submitted by Penguin
; 1,2,6,11,23,44,85,163,311,594,1132,2158,4112,7835,14928,28441,54186,103234,196679,374707,713880,1360061,2591143,4936559,9404966,17918024,34136814,65036304,123904967,236059552,449732673,856815474,1632375854
; Formula: a(n) = e(n+1), b(n) = b(n-1)+d(n-1), b(3) = 4, b(2) = 2, b(1) = 0, b(0) = 0, c(n) = d(n-1), c(3) = 2, c(2) = 2, c(1) = 0, c(0) = 1, d(n) = 2*d(n-2)+2*d(n-3)+b(n-3)+1, d(3) = 5, d(2) = 2, d(1) = 2, d(0) = 0, e(n) = b(n-1)+c(n-1)+d(n-1), e(3) = 6, e(2) = 2, e(1) = 1, e(0) = 0

mov $2,1
add $0,1
lpb $0
  sub $0,1
  add $1,$3
  mov $5,$4
  add $5,$2
  add $5,1
  mov $4,$2
  add $4,$1
  mov $2,$3
  mov $3,$5
lpe
mov $0,$4
