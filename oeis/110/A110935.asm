; A110935: a(n) = if n mod 2 = 0 then 8*F(n)-n otherwise 8*F(n)-4, where F() = Fibonacci numbers A000045.
; Submitted by Simon Strandgaard
; 0,4,6,12,20,36,58,100,160,268,430,708,1140,1860,3002,4876,7880,12772,20654,33444,54100,87564,141666,229252,370920,600196,971118,1571340,2542460,4113828,6656290,10770148,17426440,28196620,45623062,73819716,119442780,193262532
; Formula: a(n) = 2*c(n), b(n) = truncate((b(n-1)+b(n-2))/b(n-1)), b(4) = 3, b(3) = 1, b(2) = 2, b(1) = 1, b(0) = 1, c(n) = e(n-1)+3, c(4) = 10, c(3) = 6, c(2) = 3, c(1) = 2, c(0) = 0, d(n) = b(n-1)+c(n-1)+d(n-1), d(4) = 16, d(3) = 9, d(2) = 4, d(1) = 1, d(0) = 0, e(n) = c(n-1)+d(n-1), e(4) = 15, e(3) = 7, e(2) = 3, e(1) = 0, e(0) = 0

mov $1,1
lpb $0
  sub $0,1
  sub $3,$4
  mov $5,$1
  add $6,2
  add $1,$3
  div $1,$5
  add $2,$3
  add $2,$4
  mov $3,$4
  mov $4,$2
  add $5,$2
  mov $2,$3
  add $2,$6
  mov $3,$5
  mov $6,1
lpe
mov $0,$2
mul $0,2
