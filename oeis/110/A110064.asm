; A110064: a(n+4) = a(n+1) - a(n), a(0) = 1, a(1) = -4, a(2) = 0, a(3) = 1.
; Submitted by Christian Krause
; 1,-4,0,1,-5,4,1,-6,9,-3,-7,15,-12,-4,22,-27,8,26,-49,35,18,-75,84,-17,-93,159,-101,-76,252,-260,25,328,-512,285,303,-840,797,18,-1143,1637,-779,-1161,2780,-2416,-382,3941,-5196,2034,4323,-9137,7230,2289,-13460,16367,-4941,-15749,29827,-21308,-10808
; Formula: a(n) = b(n+1), b(n) = c(n-1)+1, b(3) = 0, b(2) = -4, b(1) = 1, b(0) = 0, c(n) = -e(n-1)+d(n-1), c(3) = 0, c(2) = -1, c(1) = -5, c(0) = 0, d(n) = 2*c(n-2)-d(n-1)+e(n-2)+2, d(3) = -4, d(2) = 2, d(1) = 5, d(0) = 0, e(n) = c(n-1)+e(n-1)+1, e(3) = 2, e(2) = 2, e(1) = 6, e(0) = 5

mov $4,5
add $0,1
lpb $0
  sub $0,1
  sub $3,$4
  mov $5,$1
  mov $1,$2
  add $1,1
  mov $2,$3
  mov $3,$5
  sub $3,$2
  add $4,$1
lpe
mov $0,$1
