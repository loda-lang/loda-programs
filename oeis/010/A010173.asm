; A010173: Continued fraction for sqrt(107).
; Submitted by iBezanilla
; 10,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2,1,9,1,2,20,2
; Formula: a(n) = (if((gcd(n,max(-n+2,0)+6)-3)<=(-1),0,3^(gcd(n,max(-n+2,0)+6)-3))+gcd(gcd(n,max(-n+2,0)+6)-3,2))%10+6*floor(((if((gcd(n,max(-n+2,0)+6)-3)<=(-1),0,3^(gcd(n,max(-n+2,0)+6)-3))+gcd(gcd(n,max(-n+2,0)+6)-3,2))%10)/3)

mov $2,2
trn $2,$0
add $2,6
gcd $0,$2
sub $0,3
mov $3,3
pow $3,$0
gcd $0,2
add $3,$0
mov $0,$3
mod $0,10
mov $1,$0
div $0,3
mul $0,6
add $0,$1
