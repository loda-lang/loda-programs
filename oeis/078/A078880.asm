...
; Formula: a(n) = b(n+1), b(n) = gcd(b(n-1)+truncate((-d(n-1)+c(n-1))/2),2), b(2) = 2, b(1) = 2, b(0) = 0, c(n) = truncate((-d(n-1)+c(n-1))/2), c(2) = -2, c(1) = 0, c(0) = 1, d(n) = gcd(b(n-1)+truncate((-d(n-1)+c(n-1))/2),2)*d(n-1), d(2) = 8, d(1) = 4, d(0) = 2
