; A041445: Denominators of continued fraction convergents to sqrt(238).
; Submitted by lee
; 1,2,5,7,103,110,323,756,23003,46762,116527,163289,2402573,2565862,7534297,17634456,536567977,1090770410,2718108797,3808879207,56042417695,59851296902,175745011499,411341319900,12515984608499,25443310536898,63402605682295,88845916219193,1307245432750997,1396091348970190,4099428130691377,9594947610352944,291947856441279697,593490660492912338,1478929177427104373,2072419837920016711,30492806908307338327,32565226746227355038,95623260400762048403,223811747547751451844,6809975686833305603723
; Formula: a(n) = truncate(b(n+1)/10), b(n) = b(n-1)*(((2*truncate((((3*(3*n-3)^2)%16)%10+A040329(n-1))/2))==2)+truncate((if(((21*truncate(truncate((((3*(3*n-3)^2)%16)%10+A040329(n-1))/2)/4)+2*truncate((((3*(3*n-3)^2)%16)%10+A040329(n-1))/2))%2)==0,(21*truncate(truncate((((3*(3*n-3)^2)%16)%10+A040329(n-1))/2)/4)+2*truncate((((3*(3*n-3)^2)%16)%10+A040329(n-1))/2))/2,21*truncate(truncate((((3*(3*n-3)^2)%16)%10+A040329(n-1))/2)/4)+2*truncate((((3*(3*n-3)^2)%16)%10+A040329(n-1))/2))-2)/2)+1)+b(n-2), b(2) = 20, b(1) = 10, b(0) = 0

mov $2,10
add $0,1
lpb $0
  sub $0,1
  mov $4,$2
  mov $2,$1
  mov $6,$3
  mul $6,3
  pow $6,2
  mul $6,3
  mod $6,16
  mod $6,10
  mov $1,$3
  seq $1,40329 ; Continued fraction for sqrt(348).
  add $1,$6
  div $1,2
  mov $5,$1
  mul $5,2
  div $1,4
  mul $1,21
  add $1,$5
  dif $1,2
  sub $1,2
  div $1,2
  equ $5,2
  add $1,$5
  add $1,1
  mul $1,$2
  add $1,$4
  add $3,1
lpe
mov $0,$1
div $0,10
