; A283835: Number of length-n binary vectors beginning with 0, ending with 1, and avoiding 5 consecutive 0's and 5 consecutive 1's.
; Submitted by mmonnin
; 1,0,1,2,4,8,14,28,54,104,201,386,745,1436,2768,5336,10284,19824,38212,73656,141977,273668,527513,1016814,1959972,3777968,7282266,14037020,27057226,52154480,100530993,193779718,373522417,719987608,1387820736,2675110480,5156441240,9939360064,19158732520,36929644304,71184178129,137211915016,264484469969,509810207418,982690770532,1894197362936,3651182810854,7037881151740,13565952096062,26149213421592,50404229480249,97157276149642,187276671147545,360987390199028,695825566976464,1341246904472680
; Formula: a(n) = f(n+2), b(n) = d(n-3)+d(n-4), b(9) = 12, b(8) = 6, b(7) = 3, b(6) = 2, b(5) = 1, b(4) = 0, b(3) = 0, b(2) = 0, b(1) = 0, b(0) = 0, c(n) = d(n-1)+d(n-2), c(8) = 24, c(7) = 12, c(6) = 6, c(5) = 3, c(4) = 2, c(3) = 1, c(2) = 0, c(1) = 0, c(0) = 0, d(n) = 2*d(n-3)+2*d(n-4)+2*d(n-5)+d(n-1)+d(n-6)+d(n-7)+f1(n-3), d(11) = 216, d(10) = 112, d(9) = 58, d(8) = 30, d(7) = 16, d(6) = 8, d(5) = 4, d(4) = 2, d(3) = 1, d(2) = 1, d(1) = 0, d(0) = 0, e(n) = -d(n-1)-f1(n-1)+b(n-2)+1, e(8) = -8, e(7) = -4, e(6) = -2, e(5) = -1, e(4) = 0, e(3) = 0, e(2) = 0, e(1) = 1, e(0) = 0, f(n) = 2*b(n-1)+c(n-1)+e(n-1), f(8) = 14, f(7) = 8, f(6) = 4, f(5) = 2, f(4) = 1, f(3) = 0, f(2) = 1, f(1) = 0, f(0) = 0, f1(n) = -d(n-1)-f1(n-1)+1, f1(8) = -10, f1(7) = -5, f1(6) = -2, f1(5) = -1, f1(4) = 0, f1(3) = 0, f1(2) = 0, f1(1) = 1, f1(0) = 0

add $0,2
lpb $0
  sub $0,1
  add $6,$3
  mov $7,$6
  add $9,$5
  add $4,1
  sub $4,$9
  mov $6,$4
  mov $4,$2
  mov $2,$1
  add $7,$4
  add $7,$4
  mov $1,$3
  mov $3,$8
  add $3,$5
  mov $8,$5
  mul $9,-1
  add $9,1
  add $5,$7
lpe
mov $0,$7
