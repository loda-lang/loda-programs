; A178508: a(n) = 1, 7, A011557*(period 6: repeat 10, 13, 31, 49, 70, 97).
; Submitted by loader3229
; 1,7,10,13,31,49,70,97,100,130,310,490,700,970,1000,1300,3100,4900,7000,9700,10000,13000,31000,49000,70000,97000,100000,130000,310000,490000,700000,970000,1000000,1300000,3100000,4900000,7000000,9700000,10000000
; Formula: a(n) = b(n-1), b(n) = c(n-2), b(7) = 97, b(6) = 70, b(5) = 49, b(4) = 31, b(3) = 13, b(2) = 10, b(1) = 7, b(0) = 1, c(n) = 10*c(n-6), c(9) = 490, c(8) = 310, c(7) = 130, c(6) = 100, c(5) = 97, c(4) = 70, c(3) = 49, c(2) = 31, c(1) = 13, c(0) = 10

#offset 1

mov $1,1
mov $2,7
mov $3,10
mov $4,13
mov $5,31
mov $6,49
mov $7,70
mov $8,97
sub $0,1
lpb $0
  sub $0,1
  mul $1,0
  mov $9,$1
  mov $1,$2
  mov $2,$3
  mul $3,10
  add $9,$3
  mov $3,$4
  mov $4,$5
  mov $5,$6
  mov $6,$7
  mov $7,$8
  mov $8,$9
lpe
mov $0,$1
