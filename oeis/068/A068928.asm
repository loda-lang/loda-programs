; A068928: Number of incongruent ways to tile a 3 X 2n room with 1x2 Tatami mats. At most 3 Tatami mats may meet at a point.
; Submitted by KetamiNO [YouTube]
; 2,2,2,4,5,9,12,21,30,51,76,127,195,322,504,826,1309,2135,3410,5545,8900,14445,23256,37701,60813,98514,159094,257608,416325,673933,1089648,1763581,2852242,4615823,7466468,12082291,19546175,31628466
; Formula: a(n) = b(max(n-3,0)), b(n) = c(n-2), b(5) = 21, b(4) = 12, b(3) = 9, b(2) = 5, b(1) = 4, b(0) = 2, c(n) = 2*c(n-2)-c(n-3)-c(n-5)-c(n-6)+c(n-1), c(9) = 322, c(8) = 195, c(7) = 127, c(6) = 76, c(5) = 51, c(4) = 30, c(3) = 21, c(2) = 12, c(1) = 9, c(0) = 5

#offset 1

mov $1,1
mov $2,2
mov $3,2
mov $4,4
mov $5,5
mov $6,9
sub $0,3
lpb $0
  sub $0,1
  mul $1,-1
  mov $7,$1
  mov $1,$2
  mul $2,-1
  add $7,$2
  mov $2,$3
  mov $3,$4
  mul $4,-1
  add $7,$4
  mov $4,$5
  mul $5,2
  add $7,$5
  add $7,$6
  mov $5,$6
  mov $6,$7
lpe
mov $0,$3
